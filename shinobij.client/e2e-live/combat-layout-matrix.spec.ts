...
test('Solo-PvE combat layout viewport matrix', async ({ page, request }, testInfo) => {
    ...
    expect(
        ...
        `desktop solo card artwork must show the whole square source or its visible fallback: ${JSON.stringify(soloArtwork)}`,
    ).toBe(true);
...
test('PvP combat layout viewport matrix', async ({ page, request }, testInfo) => {
    ...
    expect(pvpArtwork.every(Boolean), 'desktop PvP card artwork must load showing the whole square source').toBe(true);
});
