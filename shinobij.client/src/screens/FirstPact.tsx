...
import { FIRST_PACT_MIN_LEVEL, FIRST_PACT_TEAM_SIZE, FIRST_PACT_MAIN_ENCOUNTERS, createFirstPactProgress, expectedFirstPactMainEncounter, expectedFirstPactTournamentEncounter, FIRST_PACT_DISTRICT_WRITS, FIRST_PACT_TRIAL_ROUNDS, firstPactEncounter, FIRST_PACT_STANDING_COURT_LENGTH, expectedFirstPactStandingCourtRound, firstPactStandingCourtOpen, firstPactVow, firstPactAvailableAftermath, type FirstPactEncounterId, type FirstPactAftermathId, type FirstPactMainBeat, type FirstPactProgress } from "../../../shared/first-pact-contract";
import {
...
} from "../lib/first-pact-interiors";
import {
    firstPactInteriorCamera,
    renderFirstPactInterior,
} from "../lib/first-pact-interior-render";
import {
    playGameSfx,
    primeGameAudio,
    startGameAmbience,
...
import cisternHeadGw from "../assets/first-pact/gateworks-v2/cistern-head.png";
...
/* The one face in this cast with no name on it. The blank steel plate at the
   Arbiter's chest is where every other Court officer wears theirs. */
import arbiterPortrait from "../assets/first-pact/portraits/court-arbiter.webp";
...
type FirstPactWorldArt = {
    ...
    cisternHead?: HTMLImageElement | null;
...
/**
 * Everything this campaign will ever play.
 *
 * The crossing is a walkable city with six interiors and it shipped silent,
 * which made a place read as a menu. The game already owns an ambience bed and
 * a story bus; the campaign only had to say which of it belongs here. Primed as
 * one list on the crossing gesture so no cue is still downloading when its
 * moment arrives.
 */
const FIRST_PACT_AUDIO_CUES: (GameSfxCue | GameAmbienceCue)[] = [
    "ambience-village",
    "ambience-interior",
    "ambience-hollow",
    "chapter-seal",
...
/** Jump a re-opened conversation to whatever it is currently offering.
 *  The dialogue frame clamps the line index, so a number past the end lands on
 *  the last line, which is the one the choices belong to. Buying a second
 *  finding should not make the player sit through the Arbiter's preamble again. */
const FIRST_PACT_DIALOGUE_LAST_LINE = Number.MAX_SAFE_INTEGER;
...
function drawLowerKennelExerciseGround(context: CanvasRenderingContext2D, camera: Camera): void {
    // Each run is squared to the fence that encloses it and to the ward's own
    // street grid. The earlier hand-plotted outlines wandered a third of a tile
    // off the masonry, which read as a decal dropped on the stone rather than a
    // yard the city built, and the east run's earth began two rows above its
    // own rail. Both now stop clear of the shelf edge so the rim cannot slice
    // a paddock in half.
    const runs = [
        { side: "west" as const, salt: 941, minX: 15.12, maxX: 17.88, minY: 46.08, maxY: 52.62 },
        { side: "east" as const, salt: 977, minX: 25.12, maxX: 27.88, minY: 48.08, maxY: 52.62 },
    ];

    context.save();
    // Keep the paddocks on ward ground; the adjacent aqueduct and public road
    // retain their own material even where the earth meets them.
    context.beginPath();
    ...
    for (const run of runs) {
        ...
        shape.roundRect(minX, minY, maxX - minX, maxY - minY, 7);

        context.save();
        context.lineJoin = "round";
        // A kerb, not an outline: earth sits a little below the stone it meets.
        context.strokeStyle = "rgba(14, 18, 18, .55)";
        context.lineWidth = 4;
        ...
        // The inner face of the kerb, so the yard reads as a shallow bed.
        const lip = context.createLinearGradient(0, minY, 0, minY + 22);
        lip.addColorStop(0, "rgba(0,0,0,.34)");
        lip.addColorStop(1, "rgba(0,0,0,0)");
        context.fillStyle = lip;
        context.fillRect(minX, minY, maxX - minX, 22);

        // A compacted loop explains how a narrow run is exercised. It is sized
        // off the run itself rather than a fixed radius, so it stays inside.
        const loopX = (minX + maxX) / 2;
        const loopY = (minY + maxY) / 2;
        context.strokeStyle = "rgba(37, 29, 21, .42)";
        context.lineWidth = 11;
        context.beginPath();
        context.ellipse(loopX, loopY, (maxX - minX) * .30, (maxY - minY) * .34, 0, 0, Math.PI * 2);
        context.stroke();
        context.strokeStyle = "rgba(124, 94, 54, .22)";
        context.lineWidth = 3.5;
        context.stroke();

        // Deterministic grit and straw give the earth enough scale variation to
        // read beside the crisp V3 architecture.
        ...
        // The outer berm is deliberate husbandry: shade-tolerant switchgrass and
        // low moss bind the shelf edge, tracked to each run's own rail.
        const bermX = (run.side === "west" ? run.minX + .35 : run.maxX - .35) * FIRST_PACT_TILE_SIZE - camera.x;
        const bermSpan = run.maxY - run.minY - 1.2;
        for (let tuft = 0; tuft < 6; tuft += 1) {
            const tuftY = (run.minY + .5 + (tuft * bermSpan) / 5) * FIRST_PACT_TILE_SIZE - camera.y;
...
function drawStableCampusGrounding(context: CanvasRenderingContext2D, camera: Camera): void {
    ...
    // south-gate jambs and runs level east to the service road. It used to fall
    // 0.9 of a tile across its 7-tile run to avoid reading as a pad; on a tiled
    // city that shallow slope reads instead as a seam cutting across the grid,
    // which is the one thing this district cannot afford. A short straight
    // branch serves the annex bay.
    const street = new Path2D();
    street.moveTo(sx(8), sy(37.45));
    street.lineTo(sx(16.9), sy(37.45));
    street.lineTo(sx(16.9), sy(38.85));
    street.lineTo(sx(8), sy(38.85));
    street.closePath();
...
function drawKennelInfirmaryGround(context: CanvasRenderingContext2D, camera: Camera): void {
    ...
    // The shoulder keeps the span it needs to cover the boulevard's lower lip,
    // but its top edge is now one straight line. It used to step five times
    // across that edge by a sixth of a tile at a time, which read as a ragged
    // seam rather than a kerb where the road meets the ward.
    ...
    serviceCourt.lineTo(sx(9.55), sy(46.2));
    ...
    // The infirmary's working yard. It used to be a skewed hexagon in a pale tan
    // under a six-pixel green outline, which read as a decal laid on the ward
    // rather than ground the ward is made of. It is now squared to the parcel and
    // finished with the same kerb the exercise runs use, so the two say the same
    // thing about where earth meets stone.
    const lot = new Path2D();
    lot.roundRect(sx(10.1), sy(46), (14.98 - 10.1) * size, (50.05 - 46) * size, 6);
    ...
    const yard = context.createLinearGradient(sx(10.1), sy(46), sx(14.98), sy(50.05));
    yard.addColorStop(0, "#302a1f");
    yard.addColorStop(.5, "#29241b");
    yard.addColorStop(1, "#352d20");
    context.fillStyle = yard;
    context.fill(lot);
    context.lineJoin = "round";
    context.strokeStyle = "rgba(14, 18, 18, .55)";
    context.lineWidth = 4;
    ...
    context.save();
    context.clip(lot);
    const yardLip = context.createLinearGradient(0, sy(46), 0, sy(46) + 20);
    yardLip.addColorStop(0, "rgba(0,0,0,.32)");
    yardLip.addColorStop(1, "rgba(0,0,0,0)");
    context.fillStyle = yardLip;
    context.fillRect(sx(10.1), sy(46), (14.98 - 10.1) * size, 20);
    // Moss at the foundation only, where run-off actually collects.
    context.strokeStyle = "rgba(56, 91, 62, .34)";
    context.lineWidth = 2;
    context.beginPath();
    context.moveTo(sx(10.35), sy(49.92));
    context.lineTo(sx(14.7), sy(49.92));
    context.stroke();
    context.restore();
    context.restore();
        ...
        const standalone = gardenStandalone ?? highCourtStandalone ?? (placement.id === "vale-stable"
            ...
            : placement.id === "kennel-house"
                ...
                : placement.id === "gateworks-keeper-rowhouse"
                    ? keeperRowhouse
                : placement.id === "gateworks-cistern-head"
                    ? cisternHead
...
/**
 * Lamplight on the six thresholds that actually open. A door the player can
 * walk through has to look different from a door painted on a wall, or the
 * rooms behind them are never found. The light is authored from the same
 * `publicThreshold` the interior is keyed to, so a door cannot be lit here and
 * shut in the world, or opened in the world and dark on the street.
 */
function drawPublicDoorways(context: CanvasRenderingContext2D, camera: Camera): void {
    const size = FIRST_PACT_TILE_SIZE;
    for (const placement of FIRST_PACT_ARCHITECTURE) {
        const threshold = placement.publicThreshold;
        if (!threshold || !firstPactInteriorAtDoor(threshold)) continue;
        const centerX = (threshold.x + .5) * size - camera.x;
        const stepY = (threshold.y + .5) * size - camera.y;
        if (centerX < -size * 4 || stepY < -size * 4 || centerX > camera.width + size * 4 || stepY > camera.height + size * 4) continue;

        context.save();
        context.globalCompositeOperation = "lighter";
        // The spill lands on the stair below the door, not on the wall above it.
        const spill = context.createRadialGradient(centerX, stepY - size * .1, size * .12, centerX, stepY + size * .35, size * 1.9);
        spill.addColorStop(0, "rgba(255, 190, 112, .24)");
        spill.addColorStop(.45, "rgba(232, 154, 74, .11)");
        spill.addColorStop(1, "rgba(220, 140, 60, 0)");
        context.fillStyle = spill;
        context.beginPath();
        context.ellipse(centerX, stepY + size * .22, size * 1.2, size * .92, 0, 0, Math.PI * 2);
        context.fill();
        context.restore();

        // A pair of door lamps, and the swept stone of a step in daily use.
        context.save();
        context.strokeStyle = "rgba(255, 216, 158, .20)";
        context.lineWidth = 2;
        context.beginPath();
        context.moveTo(centerX - size * .34, stepY + size * .34);
        context.lineTo(centerX + size * .34, stepY + size * .34);
        context.stroke();
        for (const side of [-1, 1]) {
            const lampX = centerX + side * size * .42;
            const lampY = stepY - size * .18;
            context.fillStyle = "rgba(20, 16, 12, .85)";
            context.beginPath();
            context.ellipse(lampX, lampY + 3, 4.6, 5.6, 0, 0, Math.PI * 2);
            context.fill();
            const flame = context.createRadialGradient(lampX, lampY, .5, lampX, lampY, 7.5);
            flame.addColorStop(0, "rgba(255, 226, 168, .78)");
            flame.addColorStop(.5, "rgba(255, 172, 80, .46)");
            flame.addColorStop(1, "rgba(255, 150, 50, 0)");
            context.fillStyle = flame;
            context.beginPath();
            context.arc(lampX, lampY, 7.5, 0, Math.PI * 2);
            context.fill();
        }
        context.restore();
    }
...
/**
 * The Gateworks reads as a works district rather than a field of masonry.
 *
 * Every surface here was drawn at one value: walls, lanes and yards were the
 * same blue-grey brick, so nothing told the player which of them was a street.
 * Four things fix that, all derived from the painted map rather than
 * hand-plotted, so they survive a map edit:
 *
 *   - lanes are lifted above the massing, because in this palette open ground
 *     reads by being lighter than the buildings, not darker;
 *   - a kerb wherever a lane meets something solid;
 *   - a drain down the centre line of every narrow lane, which is the one line
 *     that says built city rather than texture;
 *   - conduit along the wall faces that front a street, and a lamp on the pitch
 *     a works quarter would actually light.
 *
 * It is structure, not scatter: nothing is a prop, nothing is random, and no
 * collision changes. The lanes it draws are the lanes the player can walk.
 */
function drawGateworksStreetFabric(context: CanvasRenderingContext2D, camera: Camera): void {
    const size = FIRST_PACT_TILE_SIZE;
    const inDistrict = (x: number, y: number) => firstPactDistrictAt({ x, y }) === "gateworks";
    const isLane = (x: number, y: number) => firstPactTileAt(x, y) === FirstPactTile.Road && isFirstPactWalkable(x, y);
    const bounds = { minX: 53, maxX: 81, minY: 36, maxY: 53 };

    const sx = (x: number) => x * size - camera.x;
    const sy = (y: number) => y * size - camera.y;
    if (sx(bounds.maxX) < 0 || sy(bounds.maxY) < 0 || sx(bounds.minX) > camera.width || sy(bounds.minY) > camera.height) return;
    const offscreen = (left: number, top: number) =>
        left + size < 0 || top + size < 0 || left > camera.width || top > camera.height;

    // How far the contiguous lane runs through a tile, so a narrow corridor can
    // be told from an open yard and only the corridor earns a drain.
    const span = (x: number, y: number, dx: number, dy: number) => {
        let low = 0;
        while (isLane(x - dx * (low + 1), y - dy * (low + 1))) low += 1;
        let high = 0;
        while (isLane(x + dx * (high + 1), y + dy * (high + 1))) high += 1;
        return { start: dx ? x - low : y - low, length: low + high + 1 };
    };

    context.save();

    // 1. The lanes themselves, lifted out of the massing.
    for (let y = bounds.minY; y <= bounds.maxY; y += 1) {
        for (let x = bounds.minX; x <= bounds.maxX; x += 1) {
            if (!inDistrict(x, y) || !isLane(x, y)) continue;
            const left = sx(x);
            const top = sy(y);
            if (offscreen(left, top)) continue;

            const wear = terrainHash(x, y, 4111);
            context.fillStyle = `rgba(150, 172, 184, ${.10 + wear * .05})`;
            context.fillRect(left, top, size, size);
            // Flagging laid across the lane, tighter than the wall courses.
            context.strokeStyle = "rgba(8, 14, 18, .30)";
            context.lineWidth = 1;
            context.beginPath();
            for (let joint = 16; joint < size; joint += 16) {
                context.moveTo(left + joint + .5, top + 1);
                context.lineTo(left + joint + .5, top + size - 1);
            }
            context.moveTo(left + 1, top + 24.5);
            context.lineTo(left + size - 1, top + 24.5);
            context.stroke();
            if (wear > .84) {
                context.fillStyle = "rgba(214, 232, 240, .05)";
                context.fillRect(left + 6, top + 7, size - 14, size - 16);
            }

            // 2. A kerb wherever the lane meets something solid.
            for (const [dx, dy] of [[0, -1], [0, 1], [-1, 0], [1, 0]] as const) {
                if (isFirstPactWalkable(x + dx, y + dy)) continue;
                context.fillStyle = "rgba(0, 0, 0, .40)";
                if (dy) context.fillRect(left, dy < 0 ? top : top + size - 4, size, 4);
                else context.fillRect(dx < 0 ? left : left + size - 4, top, 4, size);
                context.fillStyle = "rgba(206, 228, 238, .16)";
                if (dy) context.fillRect(left, dy < 0 ? top + 4 : top + size - 5, size, 1.5);
                else context.fillRect(dx < 0 ? left + 4 : left + size - 5, top, 1.5, size);
            }
        }
    }

    // 3. The drain, down the centre line of every narrow lane.
    for (let y = bounds.minY; y <= bounds.maxY; y += 1) {
        for (let x = bounds.minX; x <= bounds.maxX; x += 1) {
            if (!inDistrict(x, y) || !isLane(x, y)) continue;
            const across = span(x, y, 1, 0);
            const along = span(x, y, 0, 1);
            const vertical = across.length <= 3 && along.length >= 4;
            const horizontal = along.length <= 3 && across.length >= 4;
            if (!vertical && !horizontal) continue;
            const centre = vertical
                ? across.start + Math.floor((across.length - 1) / 2)
                : along.start + Math.floor((along.length - 1) / 2);
            if (vertical ? x !== centre : y !== centre) continue;

            const left = sx(x);
            const top = sy(y);
            if (offscreen(left, top)) continue;
            const channelX = vertical ? left + size / 2 - 7 : left;
            const channelY = vertical ? top : top + size / 2 - 7;
            const channelW = vertical ? 14 : size;
            const channelH = vertical ? size : 14;

            context.fillStyle = "rgba(6, 12, 15, .92)";
            context.fillRect(channelX, channelY, channelW, channelH);
            context.fillStyle = "rgba(214, 236, 244, .22)";
            if (vertical) {
                context.fillRect(channelX - 1.5, channelY, 1.5, channelH);
                context.fillRect(channelX + channelW, channelY, 1.5, channelH);
            } else {
                context.fillRect(channelX, channelY - 1.5, channelW, 1.5);
                context.fillRect(channelX, channelY + channelH, channelW, 1.5);
            }
            context.strokeStyle = "rgba(158, 192, 204, .30)";
            context.lineWidth = 1.5;
            context.beginPath();
            for (let bar = 5; bar < size; bar += 8) {
                if (vertical) {
                    context.moveTo(channelX + 1.5, channelY + bar);
                    context.lineTo(channelX + channelW - 1.5, channelY + bar);
                } else {
                    context.moveTo(channelX + bar, channelY + 1.5);
                    context.lineTo(channelX + bar, channelY + channelH - 1.5);
                }
            }
            context.stroke();
            if (terrainHash(x, y, 4133) > .4) {
                context.fillStyle = "rgba(46, 158, 166, .20)";
                if (vertical) context.fillRect(channelX + 3, channelY + 4, channelW - 6, size - 8);
                else context.fillRect(channelX + 4, channelY + 3, size - 8, channelH - 6);
            }
        }
    }
...
/**
 * What hangs off the Gateworks frontages: lamps, and the shop cloth beneath them.
 *
 * Drawn after the architecture rather than with the ground fabric. Placed with
 * the ground pass they landed on the wall tiles behind the halls and the
 * buildings painted straight over them, so the quarter stayed unlit -- a lamp
 * under a roof lights nothing.
 *
 * Both are hung on a fixed pitch off each continuous frontage, so they read as
 * a street that was fitted out rather than decorated. The cloth is the only
 * colour in the quarter and it is what makes the grey read as a city people
 * work in rather than a wall with doors in it.
 */
function drawGateworksFrontages(context: CanvasRenderingContext2D, camera: Camera): void {
    const size = FIRST_PACT_TILE_SIZE;
    const inDistrict = (x: number, y: number) => firstPactDistrictAt({ x, y }) === "gateworks";
    const isLane = (x: number, y: number) => firstPactTileAt(x, y) === FirstPactTile.Road && isFirstPactWalkable(x, y);
    // A frontage is a lane tile with something solid standing at its north edge.
    const fronted = (x: number, y: number) => inDistrict(x, y) && isLane(x, y) && !isFirstPactWalkable(x, y - 1);

    for (let y = 36; y <= 53; y += 1) {
        for (let x = 53; x <= 81; x += 1) {
            if (!fronted(x, y)) continue;
            let runLeft = x;
            while (fronted(runLeft - 1, y)) runLeft -= 1;
            let runRight = x;
            while (fronted(runRight + 1, y)) runRight += 1;
            if (runRight - runLeft + 1 < 2) continue;
            const step = (x - runLeft) % 4;
            if (step !== 1 && step !== 3) continue;

            const left = x * size - camera.x;
            const top = y * size - camera.y;
            if (left + size * 2 < 0 || top + size * 2 < 0 || left - size * 2 > camera.width || top - size * 2 > camera.height) continue;
            const midX = left + size / 2;

            if (step === 3) {
                // Split cloth at the frontage: the doorway marker every working
                // quarter in this city hangs, and the only colour in the works.
                const warm = terrainHash(x, y, 4201) > .5;
                const clothW = 30;
                const clothH = 26 + Math.round(terrainHash(x, y, 4217) * 8);
                const clothTop = top + 3;
                const clothLeft = midX - clothW / 2;
                context.save();
                // The rail, and its shadow on the wall behind.
                context.fillStyle = "rgba(0, 0, 0, .45)";
                context.fillRect(clothLeft - 3, clothTop - 1, clothW + 6, 4);
                context.fillStyle = "rgba(64, 52, 38, .96)";
                context.fillRect(clothLeft - 4, clothTop - 3, clothW + 8, 3.5);
                context.fillStyle = "rgba(126, 106, 78, .7)";
                context.fillRect(clothLeft - 4, clothTop - 3, clothW + 8, 1);
                // Two panels with the slit between them, the way noren hangs.
                const panel = (px: number, pw: number) => {
                    context.fillStyle = "rgba(0, 0, 0, .34)";
                    context.fillRect(px + 2, clothTop + 3, pw, clothH);
                    const dye = context.createLinearGradient(px, 0, px + pw, 0);
                    if (warm) {
                        dye.addColorStop(0, "rgba(62, 30, 27, .95)");
                        dye.addColorStop(.45, "rgba(96, 46, 38, .95)");
                        dye.addColorStop(1, "rgba(58, 28, 25, .95)");
                    } else {
                        dye.addColorStop(0, "rgba(24, 33, 56, .95)");
                        dye.addColorStop(.45, "rgba(40, 54, 86, .95)");
                        dye.addColorStop(1, "rgba(22, 31, 52, .95)");
                    }
                    context.fillStyle = dye;
                    context.fillRect(px, clothTop, pw, clothH);
                    // Folds, so the panel hangs instead of reading as a painted board.
                    context.strokeStyle = "rgba(0, 0, 0, .26)";
                    context.lineWidth = 1;
                    context.beginPath();
                    for (let fold = 4; fold < pw; fold += 4.5) {
                        context.moveTo(px + fold + .5, clothTop + 1);
                        context.lineTo(px + fold + .5, clothTop + clothH - 1);
                    }
                    context.stroke();
                    context.fillStyle = warm ? "rgba(150, 92, 66, .34)" : "rgba(84, 108, 154, .32)";
                    context.fillRect(px, clothTop, pw, 2);
                    context.fillStyle = "rgba(0, 0, 0, .34)";
                    context.fillRect(px, clothTop + clothH - 3, pw, 3);
                };
                panel(clothLeft, clothW / 2 - 2);
                panel(midX + 2, clothW / 2 - 2);
                // A pale mark across both panels, read as a house sign.
                context.fillStyle = "rgba(222, 212, 190, .40)";
                context.fillRect(clothLeft + 3, clothTop + 10, clothW / 2 - 8, 2);
                context.fillRect(midX + 5, clothTop + 10, clothW / 2 - 8, 2);
                context.fillStyle = "rgba(222, 212, 190, .24)";
                context.fillRect(clothLeft + 5, clothTop + 16, clothW / 2 - 12, 1.5);
                context.fillRect(midX + 7, clothTop + 16, clothW / 2 - 12, 1.5);
                context.restore();
                continue;
            }

            drawWorksLantern(context, midX, top + 15, top - 2, "north");
        }
    }

    // The alleys carry a strung lantern instead. There is nothing to bracket a
    // lamp to down here -- the halls' grate aprons are walkable, so the lanes
    // between them have no wall face -- and a line strung across a working alley
    // is what a quarter like this would actually hang.
    const span = (x: number, y: number, dx: number, dy: number) => {
        let low = 0;
        while (isLane(x - dx * (low + 1), y - dy * (low + 1))) low += 1;
        let high = 0;
    ...
    for (let y = 36; y <= 53; y += 1) {
        for (let x = 53; x <= 81; x += 1) {
            ...
            const across = span(x, y, 1, 0);
            const along = span(x, y, 0, 1);
            if (across.length > 4 || along.length < 5) continue;
            if (x !== across.start + Math.floor((across.length - 1) / 2)) continue;
            if ((y - along.start) % 4 !== 2) continue;
            ...
            const wireY = top + 6;
            context.save();
            context.strokeStyle = "rgba(28, 24, 18, .85)";
            context.lineWidth = 1.5;
            context.beginPath();
            context.moveTo(midX - across.length * size * .5, wireY - 4);
            context.quadraticCurveTo(midX, wireY + 3, midX + across.length * size * .5, wireY - 4);
            context.stroke();
            context.restore();
            drawWorksLantern(context, midX, top + 20, wireY, "strung");
        }
    }
}

/** One paper lantern, hung off a bracket on the wall face it is fixed to. */
function drawWorksLantern(
    context: CanvasRenderingContext2D,
    midX: number,
    lampY: number,
    bracketY: number,
    face: "north" | "west" | "strung",
): void {
    const size = FIRST_PACT_TILE_SIZE;
    {
            context.save();
            context.fillStyle = "rgba(46, 38, 28, .96)";
            if (face === "north") {
                context.fillRect(midX - 12, bracketY, 14, 3);
                context.fillRect(midX - 2, bracketY, 3, 11);
            } else if (face === "west") {
                context.fillRect(midX - 14, bracketY, 16, 3);
                context.fillRect(midX - 2, bracketY, 3, 12);
            } else {
                context.fillRect(midX - 1.5, bracketY, 3, 8);
            }
            context.fillStyle = "rgba(0, 0, 0, .34)";
            context.beginPath();
            context.ellipse(midX + 2, lampY + 3, 9, 11, 0, 0, Math.PI * 2);
            context.fill();
            const paper = context.createLinearGradient(midX - 9, 0, midX + 9, 0);
            paper.addColorStop(0, "rgba(186, 118, 52, .96)");
            paper.addColorStop(.42, "rgba(255, 226, 158, .98)");
            paper.addColorStop(1, "rgba(172, 104, 44, .96)");
            context.fillStyle = paper;
            context.beginPath();
            context.ellipse(midX, lampY, 9, 11, 0, 0, Math.PI * 2);
            context.fill();
            context.strokeStyle = "rgba(92, 52, 22, .5)";
            context.lineWidth = 1;
            for (const rib of [-5, 0, 5]) {
                context.beginPath();
                context.ellipse(midX, lampY + rib, 9 * Math.sqrt(Math.max(0, 1 - (rib / 11) ** 2)), .8, 0, 0, Math.PI * 2);
                context.stroke();
            }
            context.fillStyle = "rgba(52, 40, 26, .96)";
            context.fillRect(midX - 5, lampY - 11.5, 10, 3);
            context.fillRect(midX - 4, lampY + 9.5, 8, 2.5);
            context.fillStyle = "rgba(146, 46, 34, .9)";
            context.fillRect(midX - 1.5, lampY + 12, 3, 6);

            context.globalCompositeOperation = "lighter";
            const flame = context.createRadialGradient(midX, lampY, 1, midX, lampY, 20);
            flame.addColorStop(0, "rgba(255, 210, 132, .55)");
            flame.addColorStop(.4, "rgba(250, 164, 70, .26)");
            flame.addColorStop(1, "rgba(238, 138, 44, 0)");
            context.fillStyle = flame;
            context.beginPath();
            context.arc(midX, lampY, 20, 0, Math.PI * 2);
            context.fill();
            // The pool on the flagging, tight enough to stay lamplight.
            const pool = context.createRadialGradient(midX, lampY + size * .42, size * .1, midX, lampY + size * .46, size * 1.15);
            pool.addColorStop(0, "rgba(255, 186, 96, .30)");
            pool.addColorStop(.42, "rgba(238, 148, 58, .13)");
            pool.addColorStop(1, "rgba(230, 132, 44, 0)");
            context.fillStyle = pool;
            context.beginPath();
            context.ellipse(midX, lampY + size * .5, size * .95, size * .72, 0, 0, Math.PI * 2);
            context.fill();
            context.restore();
    }
}
...
function paintWorldRegion(context: CanvasRenderingContext2D, camera: Camera, art: FirstPactWorldArt): void {
    ...
    drawGateworksStreetFabric(context, camera);
...
export function FirstPact({
    ...
    const cisternHeadRef = useRef<HTMLImageElement | null>(null);
    ...
    /** The door the player just stepped out of, so leaving does not re-enter. */
    const reenterGuardRef = useRef<ReturnType<typeof firstPactPointKey> | null>(null);
    ...
    const [cisternHeadReady, setCisternHeadReady] = useState(false);
    ...
    /** What closing the crossing credited, exactly as the server reported it. */
    const [crossingGrant, setCrossingGrant] = useState<{ titles: string[]; auraStones: number } | null>(null);
    ...
    useEffect(() => {
        ...
        prepareImage(cisternHead, cisternHeadGw, (image) => { cisternHeadRef.current = image; setCisternHeadReady(true); });
    ...
    const interiorCamera = useMemo(
        () => (interior ? firstPactInteriorCamera(interior, viewport, interiorSpot) : null),
        [interior, interiorSpot, viewport],
    );
    const interiorFocusLit = interior != null
        && Math.max(Math.abs(interiorSpot.x - interior.focus.position.x), Math.abs(interiorSpot.y - interior.focus.position.y)) <= 1;
 ...
 cisternHeadReady, pumpHouseReady, pumpWorldCache, valveHouseReady, marketArcadeReady, marketRowhouseReady, marketStallReady, marketWorkshopReady, player.x, player.y, propsAtlasReady, qaArchitectureScope, qaCameraFocus, stableTackAnnexReady, targetCamera.height, targetCamera.width, targetCamera.x, targetCamera.y, tileAtlasReady, valeStableReady, visualQaPreview, worldArt]);
    ...
    const movePlayer = useCallback((next: FirstPactPoint) => {
        ...
        setPlayer(next);
        const nextKey = firstPactPointKey(next);
        if (reenterGuardRef.current && reenterGuardRef.current !== nextKey) reenterGuardRef.current = null;
        const room = firstPactInteriorAtDoor(next);
        if (room && reenterGuardRef.current !== nextKey) {
            const entry = firstPactInteriorEntry(room);
            setInterior(room);
    ...
    useEffect(() => {
        if (!interior) return;
        const timer = window.setInterval(() => {
            if (interiorSpeech) return;
            const keys = keyState.current;
            const here = interiorSpotRef.current;
            let next: FirstPactPoint | null = null;
            if (keys.has("arrowup") || keys.has("w")) next = { x: here.x, y: here.y - 1 };
    ...
    /*
     * The bed follows the player rather than the screen.
     *
     * Three rooms, three beds: the street, the inside of a building, and the
     * threshold before the crossing is made. A mounted battle takes its own
     * audio over, so the city steps out of its way instead of playing beneath
     * it. Stopping on unmount matters more than it looks: this screen is lazily
     * mounted and left often, and an ambience voice outlives its component.
     */
    useEffect(() => {
        if (!entered) {
            startGameAmbience("ambience-hollow", { gain: 0.045 });
            return;
        }
        if (battle) {
            stopGameAmbience(400);
            return;
        }
        startGameAmbience(interior ? "ambience-interior" : "ambience-village", {
            gain: interior ? 0.05 : 0.062,
        });
    }, [battle, entered, interior]);

    useEffect(() => () => stopGameAmbience(320), []);
    ...
    const crossThreshold = async () => {
        ...
        setProgress(result.progress);
        setEntered(true);
        // Crossing is the one guaranteed gesture in this screen, and a gesture
        // is what unblocks the audio context. Everything the crossing will ever
        // need is fetched here so no later cue arrives late to its own moment.
        primeGameAudio(FIRST_PACT_AUDIO_CUES);
        playGameSfx("chapter-seal", { gain: 0.7 });
    ...
    /** Buy one finding its place in the record. The screen sends a writ id and
     *  renders whatever standing the server says is left; it never subtracts. */
    const enterFinding = async (writId: string): Promise<boolean> => {
        if (storyActionPending) return false;
        setStoryActionPending(true);
        setError(null);
        try {
            const result = await enterFirstPactFinding(character.name, writId);
            if ("error" in result) {
                if (result.progress) setProgress(result.progress);
                setError(result.error);
                return false;
            ...
            setProgress(result.progress);
            playGameSfx("paper", { gain: 0.7 });
            setDialogLine(FIRST_PACT_DIALOGUE_LAST_LINE);
            return true;
        } finally {
            setStoryActionPending(false);
        }
    ...
    const advanceMain = async (beat: FirstPactMainBeat): Promise<boolean> => {
        ...
        try {
            ...
            else if (result.progress.chapter !== chapterBefore) playGameSfx("chapter-seal", { gain: 0.68 });
            else playGameSfx("paper", { gain: 0.6 });
            // Closing the crossing credits titles and Aura Stones server-side.
            // The screen only reports what came back; it decides none of it.
            if (result.grantedTitles?.length || result.grantedAuraStones) {
                setCrossingGrant({
                    titles: result.grantedTitles ?? [],
                    auraStones: result.grantedAuraStones ?? 0,
                });
            }
            setDialogNpc(null);
            setDialogLine(0);
    ...
    const openSquad = (encounterId?: FirstPactEncounterId) => {
        ...
        setPendingEncounterId(encounterId
            ?? expectedFirstPactStandingCourtRound(progress)?.id
            ?? expectedFirstPactMainEncounter(progress)?.id
            ?? expectedFirstPactTournamentEncounter(progress)?.id
            ?? null);
        setBattleError(null);
        setDialogNpc(null);
        setSquadOpen(true);
    ...
    /**
     * The Court answers a loss.
     *
     * Every fight in this campaign is meant to be hard, and a hard fight that
     * ends in a silent rematch button reads as a dead end instead of pressure.
     * The reply comes from the same table that authored the roster the player
     * just lost to, so the opponent that beat them is the thing that speaks.
     *
     * Only losses. A win is already answered by the city: the step moves, the
     * standing moves, and the people in the district have new lines.
     */
    const battleResultNote = useCallback((outcome: "win" | "loss") => {
        if (outcome !== "loss") return null;
        return firstPactEncounter(battle?.encounterId)?.defeat ?? null;
    }, [battle?.encounterId]);
    ...
    return (
        <main className="first-pact-screen" style={{ "--fp-key-art": `url(${sunkenCourtKeyArt})` } as CSSProperties}>
            <div className="fp-world" ref={viewportRef} onPointerDown={handleWorldPointer}>
                ...
                {entered && !interior && FIRST_PACT_NPCS.map((npc) => {
                ...
                {interior && interiorCamera && interior.npcs.map((npc: FirstPactInteriorNpc) => {
                    const near = Math.max(Math.abs(npc.position.x - interiorSpot.x), Math.abs(npc.position.y - interiorSpot.y)) <= 1;
                    return (
                        <button
                            type="button"
                            key={npc.id}
                ...
                {interior && interiorCamera && interiorFocusLit && !criticCapture && (
                    <button
                        type="button"
                    ...
                    >
                        <span className="fp-interact">E</span>
                    </button>
                )}
                ...
                {entered && !criticCapture && <div className="fp-hud fp-hud-left">
                    ...
                    <h1>The First Pact</h1>
                    <p>{interior ? `${interior.name} · ${interior.subtitle}` : DISTRICT_LABELS[district]}</p>
                ...
                {entered && !criticCapture && <div className="fp-dpad" aria-label="Movement controls">
                    <button type="button" aria-label="Move north" onPointerDown={(event) => event.stopPropagation()} onClick={() => stepBy(0, -1)}>▲</button>
                    <button type="button" aria-label="Move west" onPointerDown={(event) => event.stopPropagation()} onClick={() => stepBy(-1, 0)}>◀</button>
                    <button type="button" aria-label="Interact" onPointerDown={(event) => event.stopPropagation()} onClick={interact}>●</button>
                    <button type="button" aria-label="Move east" onPointerDown={(event) => event.stopPropagation()} onClick={() => stepBy(1, 0)}>▶</button>
                    <button type="button" aria-label="Move south" onPointerDown={(event) => event.stopPropagation()} onClick={() => stepBy(0, 1)}>▼</button>
                </div>}
            </div>

            {!entered && !loading && !error && (
                <FirstPactCrossingPanel onCross={() => void crossThreshold()} onExit={onExit} />
            )}
                            ...
                            <div className="fp-dialogue-actions">
                                {dialogLine < dialog.lines.length - 1
                                    ...
                                    : dialog.choices?.length
                                        ? <div className="fp-dialogue-choices" aria-label={dialog.choicesLabel ?? "Choose the words Vey will preserve"}>
                ...
                <div className="fp-overlay fp-journal-overlay" role="presentation" onPointerDown={(event) => event.stopPropagation()}>
                    <section className="fp-journal" role="dialog" aria-modal="true" aria-label="First Pact Chronicle">
                        ...
                        <footer>
                            ...
                            <div><span>Writs answered</span><strong>{progress.writs.length} / {FIRST_PACT_DISTRICT_WRITS.length}</strong></div>
                            {/* Standing is spent here and nowhere else, so the
                            ...
                            {firstPactStandingCourtOpen(progress) && (
                                <div><span>The Standing Court</span><strong>{progress.standingCourt.clears > 0
                                    ? `${progress.standingCourt.clears} answered`
                                    : `best ${progress.standingCourt.best} / ${FIRST_PACT_STANDING_COURT_LENGTH}`}</strong></div>
                            )}
            ...
            {crossingGrant && epiloguePage == null && (
                <div className="fp-epilogue" role="dialog" aria-modal="true" aria-label="What the crossing left you">
                    ...
                    <section>
                        ...
                        {crossingGrant.auraStones > 0 && (
                            <p>
                                <strong>{crossingGrant.auraStones} Aura Stone{crossingGrant.auraStones === 1 ? "" : "s"}</strong>
                                {" "}came back through the threshold with you, still warm. Enough of them buys a
                                Bloodline Awakening at home, which the Sunken Court never lived to see.
                            </p>
                        )}
                        {/* end first-pact:reward-copy */}
                        {crossingGrant.titles.length > 0 && (
                            <p>
                                Entered in your name, and wearable from your profile:{" "}
                                <strong>{crossingGrant.titles.join(", ")}</strong>.
                            </p>
                        )}
                        <div>
                            <button type="button" onClick={() => setCrossingGrant(null)}>Close the record</button>
                        </div>
                    </section>
                </div>
            )}
