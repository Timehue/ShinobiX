...
import { WarCacheTurnIn } from "../features/anbuInfiltration/WarCacheTurnIn";
...
import { deriveKageChallengeState, formatObligation, formatKageDuration, KAGE_CHALLENGE_SEAL_COST, KAGE_CHALLENGE_MIN_LEVEL, KAGE_CHALLENGE_MIN_MERIT, type ServerKageState, type ServerKageChallenge } from "../lib/kage-challenge-state";
...
export function TownHall({ character, updateCharacter, creatorItems, allServerPlayers, savedBloodlines, creatorJutsus, sharedImages, setScreen, onBack }: { character: Character; updateCharacter: React.Dispatch<React.SetStateAction<Character | null>>; creatorItems: GameItem[]; allServerPlayers: ServerPlayerSummary[]; savedBloodlines: SavedBloodline[]; creatorJutsus: Jutsu[]; sharedImages: Record<string, string>; setScreen: (s: Screen) => void; onBack: () => void }) {
    ...
    const [tab, setTab] = useState<"status" | "upgrades" | "treasury" | "guard" | "politics" | "notices" | "mercenaries">("status");
    ...
    // Inline server error for the Kage Challenge Board (surfaced instead of alert()).
    const [kageError, setKageError] = useState<string | null>(null);
    // "Now" refreshed alongside each authoritative Kage poll (below) so grace /
    // countdown displays stay current without an impure Date.now() in render.
    ...
    async function declareChallenge() {
        setKageError(null);
        if (!serverKage?.kageSystemUnlocked) return setKageError("The Kage system is still sealed for this village.");
        const seatedKage = serverKage.seatedKage;
        if (!seatedKage) return setKageError("No seated Kage is available to challenge yet.");
        if (seatedKage.toLowerCase() === character.name.toLowerCase()) return setKageError("You are already the seated Kage.");
        ...
        // Surface the exact server reason inline (e.g. cooldown / grace / merit)
        // — the server is the source of truth for eligibility.
        if (!res.ok || !data.ok) return setKageError(data.error || "Could not declare the challenge.");
    ...
    async function sendKageDuel() {
        ...
        if (!sent) return setKageError(`${targetName} is not reachable right now. Try again while they're online.`);
    ...
    // Single source of truth for the Kage Challenge Board: derive the board
    // state (SEALED / NO_CHALLENGE / PENDING_CHALLENGE / ACCEPTED_DUEL /
    // GRACE_PERIOD) from authoritative server state + this viewer.
    const kageBoard = deriveKageChallengeState(serverKage, character, nowMs);
    ...
    return <div className="card town-hall-screen">
        ...
        {tab === "upgrades" && <section className="summary-box town-upgrade-summary"><h3>Village Upgrades</h3><p className="hint">Village upgrades now spend <strong>Honor Seals</strong>. Only the seated Kage can upgrade village structures.</p><p className="hint">Current Kage: <strong>{state.seatedKage ?? "No player seated yet"}</strong>{isSeatedKage ? " — you can upgrade structures." : " — upgrades are locked for your account."}</p><p className="hint">Total Village Development: <strong>{totalUpgradeLevel}</strong> / {VILLAGE_UPGRADE_MAX_LEVEL * villageUpgradeDefinitions.length}</p>
            <div className="town-upgrade-grid">
                <div className="town-upgrade-card" style={{ borderColor: hollowGateOpen ? "var(--purple-500)" : "#7c3aed", boxShadow: hollowGateOpen ? "0 0 16px rgba(168,85,247,0.35)" : undefined }}>
                    ...
                    <p className="town-upgrade-bonus">{hollowGateOpen ? <span style={{ color: "var(--green-300)" }}>Open until {new Date(hollowGateUntil).toLocaleDateString()} · re-break to add {HOLLOW_GATE_UNLOCK_DAYS} days.</span> : <>Cost: <strong>{HOLLOW_GATE_UNLOCK_COST.toLocaleString()} Honor Seals</strong> · {HOLLOW_GATE_UNLOCK_DAYS} days</>}</p>
            ...
            {warStructures && (
                <div className="town-war-structures" style={{ marginTop: "1.2rem" }}>
                    ...
                    <p className="hint">Standing war infrastructure — bought with <b>treasury Honor Seals</b> and kept across wars. Every level adds a <b>daily War-Resource upkeep</b> the village pays automatically from its WR pool; if the treasury can't cover it the structure goes <b>dormant</b> (its bonus suspended) until WR recovers. <b>Ramparts</b> &amp; <b>Watchtower</b> are per-war (War Resources) and live in the Sector War Map.</p>
                    <div className="town-upgrade-grid">
        ...
        {tab === "mercenaries" && <section className="summary-box"><h3>⚔ War Mercenaries</h3>{!primaryVillageWar ? <p className="hint">Mercenary bands can only be hired during an active village war. {character.village} is not at war right now — open the War Hall to declare one, then come back to muster swords for hire.</p> : <><p className="hint">Spend Honor Seals to throw mercenary bands at <strong>{activeWarEnemyVillage}</strong>. Each band strikes <strong>once</strong> for sealed war damage and can be hired <strong>once per war</strong> — the roster resets when a new war begins. Mercenaries soften the enemy; they can't end a war on their own, so your village still has to land the finish.</p><p className="hint">Your Honor Seals: <strong>{(character.honorSeals ?? 0).toLocaleString()}</strong> · {hiredMercTiers.length}/{MERCENARY_TIERS.length} bands hired this war.</p><div className="town-upgrade-grid">{MERCENARY_TIERS.map(tier => { const hired = hiredMercTiers.includes(tier.id); const afford = (character.honorSeals ?? 0) >= tier.costSeals; const busy = mercBusy === tier.id; return <div key={tier.id} className="town-upgrade-card" style={hired ? { borderColor: "var(--green-300)", boxShadow: "0 0 12px rgba(134,239,172,0.25)" } : undefined}><div className="town-upgrade-topline"><span className="town-upgrade-icon town-merc-icon">{mercPortrait(tier.id) ? <img src={mercPortrait(tier.id)} alt={tier.name} /> : "🗡️"}</span><div><strong>{tier.name}</strong><p>Level {tier.level} mercenary</p></div></div><p className="town-upgrade-desc">{tier.blurb}</p><p className="town-upgrade-bonus">Strikes for <strong>{tier.warDamage.toLocaleString()}</strong> war damage · <strong>{tier.costSeals.toLocaleString()} Honor Seals</strong></p><button disabled={hired || !afford || busy} onClick={() => hireMercenary(tier.id)}>{hired ? "✅ Hired this war" : busy ? "Hiring…" : afford ? `Hire — ${tier.costSeals.toLocaleString()} Seals` : `Need ${tier.costSeals.toLocaleString()} Seals`}</button></div>; })}</div></>}</section>}
        {tab === "politics" && <><section className="summary-box"><h3>Kage & Elder Seats</h3><div className="town-leader-row town-kage-card"><LeaderPortrait image={getLeaderImage(state.seatedKage, leadershipImages.kage)} name={state.seatedKage ?? leadership.kage} fallback="?" /><p><strong>Current Kage:</strong> {state.seatedKage ?? leadership.kage}</p></div><div className="elder-seat-grid"><div className={`elder-card${character.elderFocus === "war" ? " elder-card-active" : ""}`}><LeaderPortrait image={leadershipImages.elders?.[0]} name={leadership.elders[0]} fallback="?" /><span>War Elder</span><strong>{leadership.elders[0]}</strong><small className="elder-focus-desc">+1% damage reduction during village wars (applied to incoming damage in combat)</small><button className={character.elderFocus === "war" ? "active" : ""} onClick={() => supportVillageFocus("War Elder", "war")}>{character.elderFocus === "war" ? "✅ War Focus Active" : "Select War Focus"}</button></div><div className={`elder-card${character.elderFocus === "trade" ? " elder-card-active" : ""}`}><LeaderPortrait image={leadershipImages.elders?.[1]} name={leadership.elders[1]} fallback="?" /><span>Trade Elder</span><strong>{leadership.elders[1]}</strong><small className="elder-focus-desc">5% discount on all items in the Shop and Grand Marketplace</small><button className={character.elderFocus === "trade" ? "active" : ""} onClick={() => supportVillageFocus("Trade Elder", "trade")}>{character.elderFocus === "trade" ? "✅ Trade Focus Active" : "Select Trade Focus"}</button></div><div className={`elder-card${character.elderFocus === "training" ? " elder-card-active" : ""}`}><LeaderPortrait image={leadershipImages.elders?.[2]} name={leadership.elders[2]} fallback="?" /><span>Training Elder</span><strong>{leadership.elders[2]}</strong><small className="elder-focus-desc">+10% XP from all sources and 10% faster jutsu training</small><button className={character.elderFocus === "training" ? "active" : ""} onClick={() => supportVillageFocus("Training Elder", "training")}>{character.elderFocus === "training" ? "✅ Training Focus Active" : "Select Training Focus"}</button></div></div></section><section className="summary-box"><h3>ANBU Black Ops</h3><p className="hint">Seats 1-3 are appointed by the seated Kage. Seats 4-10 are earned by PvP kills this month ({currentAnbuMonth}).</p><datalist id="anbu-player-options">{villagePlayers.map(name => <option key={name} value={name} />)}</datalist>{isSeatedKage && <div className="treasury-grid">{[0, 1, 2].map(index => <div key={`anbu-appoint-${index}`}><label>Appointed Seat {index + 1}</label><input list="anbu-player-options" value={anbuAppointmentInputs[index] ?? ""} onChange={(event) => updateAnbuAppointmentInput(index, event.target.value)} placeholder="Type or choose player" /><div className="menu"><button onClick={() => appointAnbu(index)}>Appoint</button><button className="danger-button" onClick={() => clearAnbuAppointment(index)}>Clear</button></div></div>)}</div>}<div className="contrib-rank-grid">{anbuSlots.map((slot, idx) => <div key={`anbu-${idx}-${slot?.name ?? "empty"}`} className="clan-guard-row"><span>#{idx + 1} <strong>{slot?.name ?? "Open ANBU Seat"}</strong>{slot ? ` — ${slot.rankTitle}` : idx < 3 ? " — Kage appointed seat" : " — No qualifying player yet"}</span><span>{slot ? `${idx < 3 ? "Appointed" : "Earned"} · ${slot.monthlyKills.toLocaleString()} monthly PvP kill${slot.monthlyKills === 1 ? "" : "s"}` : "0 kills"}</span></div>)}</div><h4>ANBU Missions</h4><div className="contrib-rank-grid"><div className="clan-guard-row"><span>Scout enemy-controlled sectors</span><span>Reveal owner, HP, guards</span></div><div className="clan-guard-row"><span>Queue as sector guard</span><span>ANBU may guard any village sector</span></div><div className="clan-guard-row"><span>Sabotage raids</span><span>Support clan raids and defense pressure</span></div></div></section><section className="summary-box"><h3>Kage Challenge Board</h3>
            <div className="town-leader-row town-kage-card"><LeaderPortrait image={getLeaderImage(serverKage?.seatedKage ?? state.seatedKage, leadershipImages.kage)} name={serverKage?.seatedKage ?? state.seatedKage ?? leadership.kage} fallback="?" /><div><p><strong>Seated Kage:</strong> {serverKage?.seatedKage ?? state.seatedKage ?? leadership.kage}</p><p className="hint">Successful defenses this reign: <strong>{serverKage?.defenseCount ?? 0}</strong>{serverKage?.seatedAt ? ` · seated ${new Date(serverKage.seatedAt).toLocaleDateString()}` : ""}</p></div></div>
            <p className="hint">Declaring costs {KAGE_CHALLENGE_SEAL_COST} Honor Seals and needs level {KAGE_CHALLENGE_MIN_LEVEL}+ and {KAGE_CHALLENGE_MIN_MERIT}+ personal Village Merit. The Kage loses the seat only by losing the official duel, or by letting their accept-obligation clock run out while you are both online — the timer pauses the moment either of you goes offline, so no one can AFK-steal the seat.</p>
            {kageError && <p className="hint" style={{ color: "var(--red-300)", fontWeight: 600 }} role="alert">{kageError}</p>}
            {kageBoard.kind === "SEALED" && <p className="hint">The Kage system is sealed for {character.village}. Clear your village's level-100 story boss to open the seat.</p>}
            {kageBoard.kind === "GRACE_PERIOD" && <p className="hint">{kageBoard.isSeatedKage ? "You just took the seat — " : "The seat just changed hands — "}new challenges reopen in <strong>{formatKageDuration(kageBoard.graceUntil - nowMs)}</strong> (around {new Date(kageBoard.graceUntil).toLocaleString()}).</p>}
            {kageBoard.kind === "NO_CHALLENGE" && (kageBoard.isSeatedKage
                ? <p className="hint">You hold the Kage seat. Stay ready — any eligible villager can declare a challenge at any time.</p>
                : <><div className="contrib-rank-grid">{kageBoard.eligibility.map(item => <div key={item.label} className="clan-guard-row"><span>{item.ok ? "✅" : "⬜"} {item.label}</span><span>{item.detail ?? (item.ok ? "Met" : "Not yet")}</span></div>)}</div>
                    <button onClick={() => void declareChallenge()} disabled={!kageBoard.canDeclare}>Declare Kage Challenge ({KAGE_CHALLENGE_SEAL_COST} Seals)</button>
                    <p className="hint">{kageBoard.canDeclare ? "You meet every requirement — declaring stakes 500 Honor Seals." : "Meet every requirement above to declare. A recent-loss cooldown (if any) is enforced by the server and will show here if you try."}</p></>)}
            {(kageBoard.kind === "PENDING_CHALLENGE" || kageBoard.kind === "ACCEPTED_DUEL") && <div className={`notice-post ${kageBoard.kind === "ACCEPTED_DUEL" ? "pinned" : ""}`}>
                <div className="notice-post-head"><span>{kageBoard.kind === "ACCEPTED_DUEL" ? "OFFICIAL DUEL LOCKED" : "PENDING CHALLENGE"}</span><small>{new Date(kageBoard.challenge.createdAt).toLocaleString()}</small></div>
                <strong>{kageBoard.challenge.challenger} vs {kageBoard.seatedKage}</strong>
                <p>Kage accept obligation remaining: <strong>{formatObligation(kageBoard.challenge.obligationRemainingMs)}</strong> of verified online overlap.</p>
                {kageBoard.kind === "PENDING_CHALLENGE" && kageBoard.role === "challenger" && <><div className="menu"><button onClick={() => void sendKageDuel()}>Send Official Duel</button></div><p className="hint">Stay online together to burn the Kage's accept obligation — if it hits 0:00 they forfeit the seat to you. Or just beat them in the official duel.</p></>}
                {kageBoard.kind === "PENDING_CHALLENGE" && kageBoard.role === "kage" && <p className="hint">{kageBoard.challenge.challenger} is challenging you. When they send the official duel, accept it from your challenge alerts to defend — winning keeps your seat. If your accept obligation hits 0:00 while you are both online, you forfeit.</p>}
                {kageBoard.kind === "PENDING_CHALLENGE" && kageBoard.role === "bystander" && <p className="hint">A challenge is underway. The seat is settled by the official duel or the forfeit clock.</p>}
                {kageBoard.kind === "ACCEPTED_DUEL" && <p className="hint">The official duel is locked in{kageBoard.challenge.battleId ? ` (battle ${kageBoard.challenge.battleId.slice(0, 12)}…)` : ""}. Fight it out — the winner is settled automatically against the real battle, so no one needs to press anything after. {kageBoard.role === "challenger" ? "Win to take the seat." : kageBoard.role === "kage" ? "Win to defend your seat." : "Awaiting the result."}</p>}
            </div>}
            </section></>}
    </div>;
}
