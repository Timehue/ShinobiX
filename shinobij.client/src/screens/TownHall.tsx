...
export function TownHall({ character, updateCharacter, creatorItems, allServerPlayers, savedBloodlines, creatorJutsus, sharedImages, setScreen, onBack }: { character: Character; updateCharacter: React.Dispatch<React.SetStateAction<Character | null>>; creatorItems: GameItem[]; allServerPlayers: ServerPlayerSummary[]; savedBloodlines: SavedBloodline[]; creatorJutsus: Jutsu[]; sharedImages: Record<string, string>; setScreen: (s: Screen) => void; onBack: () => void }) {
    ...
    return <div className="card town-hall-screen">
        ...
        <header className="town-hall-hero" style={{ "--town-art": `url(${townHallHero})` } as React.CSSProperties}>
            ...
            <div className="town-hall-ledger"><span><small>Village Level</small><strong>{villageLevel}</strong></span><span><small>Seated Kage</small><strong>{state.seatedKage ?? "Unclaimed"}</strong></span><span><small>Honor Seals</small><strong>{(character.honorSeals ?? 0).toLocaleString()}</strong></span></div>
        </header>
        ...
        {tab === "status" && <div className="town-command">
            ...
            <section className="summary-box"><h3>War records</h3><div className="war-record-grid">{state.warRecords.map((war, idx) => <div key={`${war.opponent}-${idx}`} className="war-record-card"><strong>{war.winner} vs {war.opponent}</strong><span>{war.finalScore}</span><small>{war.date} · MVP {war.mvpClan}</small><small>Attack {war.topAttacker} · Defense {war.topDefender}</small><small>{war.rewards}</small></div>)}</div></section>
        </div>}
        {tab === "upgrades" && <section className="summary-box town-upgrade-summary"><div className="town-section-heading"><p className="act-label">Development office</p><h3>Village Upgrades</h3></div><div className="town-upgrade-overview"><span><small>Authority</small><strong>{isSeatedKage ? "Kage access" : state.seatedKage ?? "Seat unclaimed"}</strong></span><span><small>Development</small><strong>{totalUpgradeLevel} / {VILLAGE_UPGRADE_MAX_LEVEL * villageUpgradeDefinitions.length}</strong></span><span><small>Available funds</small><strong>{(character.honorSeals ?? 0).toLocaleString()} seals</strong></span></div><p className="hint">Upgrades you can authorize now appear first. Future and completed projects follow.</p>
        ...
        {tab === "politics" && <>
            <section className="summary-box"><h3>Village Council</h3><div className="town-leader-row town-kage-card"><LeaderPortrait image={getLeaderImage(state.seatedKage, leadershipImages.kage)} name={state.seatedKage ?? leadership.kage} fallback="?" /><p><strong>Kage:</strong> {state.seatedKage ?? leadership.kage}</p></div><div className="elder-seat-grid">
