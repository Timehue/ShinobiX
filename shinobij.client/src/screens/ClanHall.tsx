...
export function ClanHall({ character, updateCharacter, creatorItems, setScreen }: { character: Character; updateCharacter: React.Dispatch<React.SetStateAction<Character | null>>; creatorItems: GameItem[]; setScreen: (s: Screen) => void }) {
    ...
    return <div className="card clan-hall-screen">
        ...
        {view === "hall" && canManageClan(myRole) && <div className="summary-box"><h3>📣 Recruitment Pitch</h3><p className="hint">Shown to players browsing for a clan. Make your case (max 300 characters).</p><textarea value={recruitmentDraft} maxLength={300} rows={3} onChange={e => setRecruitmentDraft(e.target.value)} placeholder="e.g. Active war clan — daily clan wars, friendly veterans, newcomers mentored. Join us!" /><div className="menu"><button onClick={() => void saveRecruitment()}>Save Pitch</button></div></div>}
