...
import type { ClanJoinRequest, ClanMemberEntry, ClanRole, ClanTreasury, ClanTreasuryCurrencyKey, ClanUpgradeKey, ClanWarRecord, EnhancedClanData, NoticePostType } from "../types/clan";
import { ClanSealPool } from "../screens/ClanSealPool";
import { ClanRankings } from "./ClanRankings";
import { ClanBoss } from "./ClanBoss";
import { ClanChat } from "./ClanChat";
import { clanBossEnabled } from "../lib/clan-boss-api";
...
import { TERRITORY_CAPTURE_MIN_MEMBERS, TERRITORY_CONTROL_MAX, TERRITORY_CONTROL_SCROLL_ID, TERRITORY_HP_MAX, TERRITORY_REBUILD_COOLDOWN_MS } from "../constants/game";
...
export function ClanHall({ character, updateCharacter, creatorItems, setScreen, towerHostLoadout, sharedImages, onRecordBattle }: { character: Character; updateCharacter: React.Dispatch<React.SetStateAction<Character | null>>; creatorItems: GameItem[]; setScreen: (s: Screen) => void; towerHostLoadout?: TowerHostLoadout; sharedImages?: Record<string, string>; onRecordBattle?: (entry: BattleHistoryEntry) => void }) {
    ...
    async function doClaimMentor(student: string) {
        ...
        else updateCharacter((prev) => prev ? ({ ...prev, honorSeals: (prev.honorSeals ?? 0) + r.seals, clanEventContrib: (prev.clanEventContrib ?? 0) + r.contrib }) : prev);
        fetchMentorView(character.name).then(setMentorView);
    ...
    async function donateRyo() {
        ...
        try {
            ...
            updateCharacter((prev) => prev ? ({ ...prev, ryo: prev.ryo - amount, clanEventContrib: (prev.clanEventContrib ?? 0) + contrib }) : prev);
        } finally {
            setDonateBusy(false);
        }
    }
    ...
    if (loading) return <div className="card"><p style={{ color: "var(--text-dim)" }}>Loading clan data…</p></div>;
    if (!clanData) {
    ...
    return <div className="card clan-hall-screen">
        ...
        <WarCacheTurnIn character={character} updateCharacter={updateCharacter} dest="clan" />
        ...
        <div className="clan-exchange-quicknav">
        ...
        {view === "roster" && <div className="clan-roster">
            ...
            {sortedMembers.map((member, idx) => {
                ...
                const roleColor = CLAN_ROLE_COLOR[role] ?? "var(--text-dim)";
        ...
        {view === "missions" && <><p className="hint">Clan missions refresh <strong>every week</strong> — claim them each week to keep leveling the clan hall. Clan-XP rewards scale with clan size, so a full 10–15 member clan earns the normal rate and very small clans earn less.</p><div className="clan-mission-grid">{clanMissionDefinitions.map(mission => { const progress = clanMissionProgress(clanData, mission.key); const complete = progress >= mission.target; const claimable = mission.key !== "territory"; const claimed = claimedClanMissions.includes(mission.key); return <div key={mission.key} className="summary-box clan-mission-card"><h3>{mission.icon} {mission.name}</h3><p>{mission.description}</p><div className="town-upgrade-bar"><span style={{ width: `${Math.min(100, (progress / mission.target) * 100)}%` }} /></div><p><strong>{Math.min(progress, mission.target).toLocaleString()}</strong> / {mission.target.toLocaleString()}</p><p className="hint">Reward: {mission.reward}</p>{claimable && (claimed ? <p className="hint" style={{ color: "var(--green-400)", fontWeight: 600 }}>✓ Claimed this week</p> : complete && canManageClan(myRole) ? <div className="menu"><button onClick={() => void claimClanMission(mission.key)} disabled={clanMissionClaimBusy === mission.key}>{clanMissionClaimBusy === mission.key ? "Claiming…" : "Claim Reward"}</button></div> : complete ? <p className="hint">Complete — clan leadership can claim.</p> : null)}</div>; })}</div></>}
