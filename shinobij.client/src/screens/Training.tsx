        ...
        <div className="card">
            ...
            {activeTraining && (
                <div className="summary-box">
                    ...
                    <p>{trainingReady
                        ? <strong style={{ color: "var(--green-400)" }}>Ready to collect!</strong>
...
function JutsuSealPanel({
    ...
    return (
        <div className="summary-box" style={{ background: "linear-gradient(180deg, rgba(250,204,21,0.10), rgba(8,10,22,0.4))", border: "1px solid rgba(250,204,21,0.45)", marginBottom: "0.75rem" }}>
            <strong style={{ color: "var(--gold)" }}><GiRibbonMedal style={{ verticalAlign: "-0.12em", marginRight: "0.3rem" }} />Honor Seal Training</strong>
            <span className="hint" style={{ marginLeft: 10 }}>
                Balance: <strong style={{ color: "var(--gold)" }}>{balance.toLocaleString()}</strong>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                ...
                {activeJutsuTraining && Date.now() < activeJutsuTraining.endsAt && (
                    <>
                        <button onClick={() => void speedUp(1)} disabled={busy || balance < (hasDiscount ? 1 : 1)} style={{ background: "linear-gradient(#422006,#1c1006)", borderColor: "var(--gold-300)" }}>
                            {busy ? "…" : "−10 min (1 Seal)"}
                        </button>
                        <button onClick={() => void speedUp(10)} disabled={busy || balance < (hasDiscount ? 9 : 10)} style={{ background: "linear-gradient(#422006,#1c1006)", borderColor: "var(--gold-300)" }}>
                            {busy ? "…" : `Finish now (${hasDiscount ? 9 : 10} Seals)`}
                        </button>
                    </>
                )}
            </div>
            {msg && <p className="hint" style={{ margin: "8px 0 0", color: msg.startsWith("✅") ? "var(--gold)" : "var(--red-400)" }}>{msg}</p>}
        </div>
    );
}
...
export function JutsuTrainingHall({
    ...
    const activeTrainingPanel = activeJutsuTraining ? (
        <div className="summary-box">
            ...
            {activeRemaining > 0 && <button onClick={finishWithRyo} disabled={character.ryo < jutsuRyoFinishCost(activeRemaining)} style={{ marginLeft: 8, background: "linear-gradient(#14532d,#052e16)", borderColor: "var(--green-400)" }}>💰 Finish now ({jutsuRyoFinishCost(activeRemaining).toLocaleString()} ryo)</button>}
            {queued ? (
                <div className="summary-box" style={{ marginTop: 8, borderColor: "rgba(96,165,250,0.5)" }}>
                    <strong style={{ color: "var(--blue-400)" }}><GiFastForwardButton style={{ verticalAlign: "-0.12em", marginRight: "0.3rem" }} />Up next:</strong> {queued.label} — Level {queued.fromLevel} → {queued.toLevel} <span className="hint">({queued.ryoCost} ryo paid · ~{Math.round(queued.durationMs / 60000)} min)</span>
