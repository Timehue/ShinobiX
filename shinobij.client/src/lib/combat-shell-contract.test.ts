...
test("the Arena screen stays a lobby and never hosts a fight again", () => {
    ...
    // What it DOES still own: the lobby surfaces.
    assert.match(arenaLobby, /className="card arena-lobby"/);
    assert.match(arenaLobby, /function joinRankedQueue/);
    assert.match(arenaLobby, /async function challengePlayer/);
    assert.match(arenaLobby, /async function acceptChallenge/);
});
