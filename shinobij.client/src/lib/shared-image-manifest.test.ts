...
describe("shared image manifest", () => {
    it("reads the versioned shape", () => {
        const manifest = parseImageManifest({ version: "7", ids: ["jutsu:fireball", "jutsu:lightning-lance"] });
        assert.deepEqual(manifest, { version: "7", ids: ["jutsu:fireball", "jutsu:lightning-lance"] });
    });
