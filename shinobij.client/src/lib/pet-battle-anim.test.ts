...
test("petCardImage: a starter falls back to its idle pose (no inline image)", () => {
    ...
    assert.equal(petCardImage(mkPet({ id: "starter-fire", rarity: "standard" })), "/pet-poses/starter-fire-idle.webp?v=3");
});
...
test("petCardImage: an evolved starter prefers its stage pose", () => {
    ...
    assert.equal(petCardImage(mkPet({ id: "starter-fire", rarity: "rare", evolutionStage: 1 })), "/pet-poses/starter-fire-r-idle.webp?v=3");
});

test("petCardImage: an encounter clone resolves the pose by variant-stripped base id", () => {
    assert.equal(petCardImage(mkPet({ id: "standard-1-1700000000000" })), "/pet-poses/standard-1-idle.webp?v=3");
});
