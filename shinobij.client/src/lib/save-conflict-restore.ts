 ...
 * it and either the exact acknowledged version is read back, or a later version
 * still contains every recoverable field from the selected draft. Server-owned
 * fields may legitimately differ because the generic-save sanitizer retains
 * authority over them.
 *
...
export async function restoreSaveConflictRevision<TPayload extends Record<string, unknown>>(params: {
    ...
    await params.runExclusive(async () => {
        ...
        // Everything recoverable that differs from the selected draft. At the
        // exact acknowledged version this is a sanitizer refusal and can be
        // reported as declined. At a HIGHER version it is ambiguous: an
        // interleaving writer may have replaced our restored client-owned state,
        // so keep the draft rather than falsely declaring it one-shot complete.
        ...
        applyAuthoritative(verified);
        assertCurrent(draft.accountKey);
        if (verifiedVersion > acknowledgedVersion && declined.length > 0) {
            throw new Error("A newer server save changed recoverable progress before the restore could be confirmed. The local draft remains protected.");
        }
