 ...
 *
 * Each clash is KAGE TACTICS: pets hold discrete cells, respect cover and line
...
export interface RiteClash {
    ...
    blue: RiteCombatant[];
    red: RiteCombatant[];
    /** Compatibility sentinels; Kage Tactics always stores -1 because all fight. */
...
export function aiRitePlan(band: readonly Pet[], seed: number): RitePlan {
    ...
    return { formation, deployment, reformAfterClash: null, reform: null, reformDeployment: null };
}
...
export function runWarfrontRite(
    ...
    const redDeployment = [...(redPlan.deployment ?? defaultDeployment(bandSize))];
    ...
    for (let index = 0; index < RITE_MAX_CLASHES; index++) {
        ...
        if (bluePlan.reformAfterClash === index && bluePlan.reformDeployment && isDeployment(bluePlan.reformDeployment, bandSize)) {
            blueDeployment = [...bluePlan.reformDeployment];
        }
        // A sealed AI re-forms once, and only if it LOST the opening clash: it
        // puts its healthiest pets into the forward marks and drops its most
        // wounded to the back. The same rule applies to either AI seat so
        // headless balance probes cannot acquire a best-of-three seat bias. An
        // explicitly supplied plan belongs to a player/test harness and is never
        // overwritten — the live player's placement remains authoritative.
        const reformAiAfterLoss = (
            usesAiPlan: boolean,
            lost: boolean,
            formation: number[],
            deployment: number[],
            ...
            if (!usesAiPlan || index !== 0 || !lost) return formation;
            const reformed = [...formation].sort((a, b) => (hp.get(b) ?? 0) - (hp.get(a) ?? 0));
            reformed.forEach((slot, fieldIndex) => { deployment[slot] = WARFRONT_DEFAULT_DEPLOYMENT[fieldIndex]; });
            return reformed;
        };
        blueFormation = reformAiAfterLoss(blueUsesAiPlan, winner === "red", blueFormation, blueDeployment, blueHp);
        redFormation = reformAiAfterLoss(redUsesAiPlan, winner === "blue", redFormation, redDeployment, redHp);
    }
