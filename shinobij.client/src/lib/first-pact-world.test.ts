...
const gwCisternHead = fileURLToPath(new URL("../assets/first-pact/gateworks-v2/cistern-head.png", import.meta.url));
...
test("buildings are not stacked against each other without a yard between them", () => {
    // Two buildings sharing an edge read as one lump dropped on the map rather
    // than two structures in a street. The Gateworks pump house sat flush on the
    // maintenance store and the keeper's rowhouse on the valve house, which is
    // exactly the "just placed" look the brief rejects; a one-tile yard now
    // separates each pair.
    //
    // Trees, lanterns and stelae are excluded: a canopy overlapping a gate or a
    // lantern is planting, not stacking. The Kennel Ward's three pairs ARE
    // deliberate -- its own accepted test calls it "a compact street-edge
    // cluster" -- so they are named rather than silently allowed.
    const deliberate = new Set([
        "stable-tack-annex|vale-stable",
        "handler-lodge|stable-tack-annex",
        "feed-storehouse|kennel-house",
    ]);
    const isBuilding = (id: string) => !/maple|lantern|stele/.test(id);
    const buildings = FIRST_PACT_ARCHITECTURE.filter((placement) => isBuilding(placement.id));

    const stacked: string[] = [];
    for (let i = 0; i < buildings.length; i += 1) {
        for (let j = i + 1; j < buildings.length; j += 1) {
            const a = buildings[i], b = buildings[j];
            const dx = Math.max(0, Math.max(a.bounds.x - (b.bounds.x + b.bounds.width), b.bounds.x - (a.bounds.x + a.bounds.width)));
            const dy = Math.max(0, Math.max(a.bounds.y - (b.bounds.y + b.bounds.height), b.bounds.y - (a.bounds.y + a.bounds.height)));
            if (Math.max(dx, dy) > 0) continue;
            const pair = [a.id, b.id].sort().join("|");
            if (deliberate.has(pair)) continue;
            stacked.push(`${a.id} is flush against ${b.id}`);
        }
    }
    assert.deepEqual(stacked, [], `buildings stacked with no yard between them:\n${stacked.join("\n")}`);
});
