...
import { PET_ELEMENT_VFX_PROFILES, petBattleElement, type PetBattleElement, type PetElementVfxShape } from "./pet-element-vfx";

export type WarfrontCoreElement = PetBattleElement;
export type WarfrontSpectacleShape = PetElementVfxShape;
...
const profile = (element: WarfrontCoreElement) => PET_ELEMENT_VFX_PROFILES[element];

export const WARFRONT_ELEMENT_SIGNATURES: Readonly<Record<WarfrontCoreElement, WarfrontElementSignature>> = Object.freeze({
    Water: { ...profile("Water"), audioTell: "shield", audioContact: "hit", tellRate: 1.08, contactRate: 1.12 },
    Fire: { ...profile("Fire"), audioTell: "debuff", audioContact: "crit", tellRate: 1.08, contactRate: 1.08 },
    Wind: { ...profile("Wind"), audioTell: "dodge", audioContact: "hit", tellRate: 1.14, contactRate: 1.16 },
    Earth: { ...profile("Earth"), audioTell: "command", audioContact: "crit", tellRate: 0.88, contactRate: 0.9 },
    Lightning: { ...profile("Lightning"), audioTell: "debuff", audioContact: "crit", tellRate: 1.24, contactRate: 1.26 },
    None: { ...profile("None"), audioTell: "command", audioContact: "hit", tellRate: 1, contactRate: 1 },
});

export function warfrontCoreElement(value: string | null | undefined): WarfrontCoreElement {
    return petBattleElement(value);
}
