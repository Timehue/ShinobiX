...
// provisions = the clan mirror of Village Stores rations (server-credited only; api/_clan-save-validate.ts).
export type ClanTreasury = { ryo: number; fateShards: number; boneCharms: number; auraStones: number; mythicSeals: number; warSupply: number; provisions?: number; items: TreasuryItemStack[]; };
export type ClanTreasuryCurrencyKey = Exclude<keyof ClanTreasury, "items" | "warSupply" | "provisions">;
