...
test("ambient screen hints collapse to an operable combat-safe help control", () => {
    assert.match(hint, /onboarding-coach-banner screen-hint-banner screen-hint-collapsed screen-hint-\$\{screen\}/);
...
test("Mission Hall help occupies header space, never the floating action column", () => {
    const missions = readFileSync(new URL("../screens/Missions.tsx", import.meta.url), "utf8");
    assert.match(hint, /screen === "missions" && placement === "floating"\) return null/);
    assert.match(hint, /placement === "inline" \? content : createPortal\(content, document.body\)/);
    assert.match(missions, /className="mh-header-copy"[\s\S]*?<ScreenHint screen="missions"[^>]*placement="inline"/);
});
