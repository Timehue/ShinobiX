...
function GuideReader({ guide, onBack, onExit, onOpen }: {
    guide: Guide;
    onBack: () => void;
    onExit: () => void;
...
export function GuidesLibrary({ onExit }: { onExit: () => void }) {
    ...
    const guide = openId ? GUIDES.find((candidate) => candidate.id === openId) ?? null : null;

    const filteredGuides = useMemo(() => {
        const normalizedQuery = query.trim().toLocaleLowerCase();
        return GUIDES.filter((candidate) => {
    ...
    const featured = showFeatured ? GUIDES.find((candidate) => candidate.featured) ?? null : null;
    ...
    return (
        <div ref={rootRef} className={`guides-root${guide ? " guides-root--reading" : ""}`}>
            {guide ? (
                <GuideReader guide={guide} onBack={returnToLibrary} onExit={onExit} onOpen={openGuide} />
            ) : (
                <>
                    ...
                    <Panel as="header" surface="steel" className="guides-masthead">
                        ...
                        <div className="guides-masthead-copy">
                            ...
                            <div>
                                <Badge tone="spirit" icon={<GiOpenBook />}>{GUIDES.length} field guides</Badge>
                        ...
                        <div className="guides-filters" aria-label="Guide categories">
                            {(["All guides", ...GUIDE_CATEGORIES] as CategoryFilter[]).map((name) => {
                                const count = name === "All guides"
                                    ? GUIDES.length
                                    : GUIDES.filter((candidate) => candidate.category === name).length;
                                return (
                                    <Button
                                        key={name}
