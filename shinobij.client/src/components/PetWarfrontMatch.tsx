    ...
    return (
        <group ref={group} visible={false}>
            ...
            <mesh ref={strikeRing} visible={false} rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.075, 0]} renderOrder={3}>
                ...
                <meshBasicMaterial
                    ref={strikeRingMat}
                    color={isLane ? (side === "blue" ? "#7dd3fc" : "#fda4af") : "#e879f9"}
                    transparent
                    opacity={0}
                    depthWrite={false}
                    toneMapped={false}
    ...
    useFrame((_state, delta) => {
        ...
        if (root.current) {
            ...
            if (riftHalo.current) {
                ...
                const haloScale = 1 + Math.sin(now * 2.6) * 0.045 + (w.winding ? 0.14 : 0);
                riftHalo.current.scale.setScalar(haloScale);
            }
...
function WfMini({ result, clock, idx, name, glow }: { result: WarfrontResult; clock: WfClockRef; idx: number; name: string; glow: string }) {
    ...
    useFrame((_state, delta) => {
        ...
        if (campSigil.current) {
            ...
            const sigilScale = 1 + Math.sin(seconds * 2.7 + idx) * 0.06 + strikePulse * 0.12;
            campSigil.current.scale.setScalar(sigilScale);
        }
        if (sigilMat.current) {
...
function WfCameraRig({ result, clock, shake, camViewRef, camCtlRef, storyRef, modeRef }: {
    ...
    useFrame((state, delta) => {
        ...
        if (ctl.mode === "free") {
            ...
            let focus = ((): { fx: number; fz: number; span: number } => {
                if (mode === "team") {
                    const snap = snapAt(result, clock);
                    let n = 0, mx = 0, mz = 0;
                    for (const a of snap.actors) if (a.team === "blue" && a.state !== "respawning") { mx += a.x; mz += a.y; n++; }
...
// The monitor is deliberately not mounted; keeping this legacy renderer local
// lets Rollup erase it while old replay branches finish aging out.
...
function warfrontImpactStyle(key: string | null, element: string | null | undefined): Pick<WfFxItem, "kind" | "color" | "accent"> {
...
/** A cheap procedural layer beneath the flipbook: two ground shockwaves, a
 * light core and eight shared-geometry shards. It gives every impact depth and
 * direction without adding another texture fetch or particle system. */
function WfImpact3D({ pos, scale, durationMs, kind, color, accent, seed }: {
    pos: Vec3;
    scale: number;
    durationMs: number;
    kind: WfImpactKind;
    ...
    useFrame((state) => {
        ...
        const p = Math.min(1, ((state.clock.elapsedTime - start.current) * 1000) / Math.max(1, durationMs));
        const burst = 1 - (1 - p) ** 3;
        const fade = (1 - p) ** 2;
        const lift = kind === "heal" ? p * 1.05 : p * 0.42;
        if (outer.current) outer.current.scale.setScalar(scale * (0.45 + burst * (kind === "power" ? 3.2 : 2.5)));
        if (inner.current) {
            inner.current.scale.setScalar(scale * (0.3 + burst * 1.75));
            inner.current.rotation.z = seed * 0.37 - p * (kind === "shadow" ? 3.4 : 1.7);
        }
        if (core.current) {
            const pulse = Math.sin(Math.PI * p);
            core.current.position.y = 0.16 + lift;
            core.current.scale.setScalar(scale * pulse * (kind === "power" ? 1.25 : 0.82));
            core.current.rotation.y = seed + p * 4;
        }
        if (shards.current) {
            shards.current.position.y = lift;
            shards.current.rotation.y = seed * 0.61 + p * (kind === "shadow" ? -3.8 : 2.4);
            shards.current.scale.setScalar(scale * (0.4 + burst * (kind === "power" ? 3.4 : 2.45)));
        }
        if (outerMat.current) outerMat.current.opacity = fade * (kind === "heal" ? 0.58 : 0.82);
        if (innerMat.current) innerMat.current.opacity = fade * 0.72;
        if (coreMat.current) coreMat.current.opacity = fade * 0.9;
        if (shardMat.current) shardMat.current.opacity = fade * (kind === "hit" ? 0.9 : 0.68);
    });
    return (
        <group position={[pos[0], 0.055, pos[2]]}>
            <mesh ref={outer} geometry={WF_IMPACT_RING_GEOMETRY} rotation={[-Math.PI / 2, 0, 0]} renderOrder={5}>
                <meshBasicMaterial ref={outerMat} color={color} transparent depthWrite={false} toneMapped={false} blending={THREE.AdditiveBlending} />
            </mesh>
            <mesh ref={inner} geometry={WF_IMPACT_RING_GEOMETRY} rotation={[-Math.PI / 2, 0, 0]} renderOrder={5}>
                <meshBasicMaterial ref={innerMat} color={accent} transparent depthWrite={false} toneMapped={false} blending={THREE.AdditiveBlending} />
            </mesh>
            <mesh ref={core} geometry={WF_IMPACT_CORE_GEOMETRY} renderOrder={6}>
                <meshBasicMaterial ref={coreMat} color={accent} transparent depthWrite={false} toneMapped={false} blending={THREE.AdditiveBlending} />
            </mesh>
            <instancedMesh ref={shards} args={[WF_IMPACT_SHARD_GEOMETRY, undefined, 8]}>
                <meshBasicMaterial ref={shardMat} color={color} transparent depthWrite={false} toneMapped={false} blending={THREE.AdditiveBlending} />
            </instancedMesh>
        </group>
    );
}

function WfShotWake3D({ from, to, visual, durationMs, arc }: {
    from: Vec3;
    to: Vec3;
    ...
    useFrame((state) => {
        ...
        const point = (q: number, out: THREE.Vector3) => {
            ...
            return out.set(
                ...
                lerp(from[1], to[1], clamped) + arc * Math.sin(Math.PI * clamped),
    ...
    return (
        <mesh ref={wake} visible={false}>
            ...
            <meshBasicMaterial ref={material} color={visual.glow} transparent opacity={0} depthWrite={false} toneMapped={false} blending={THREE.AdditiveBlending} />
        </mesh>
    );
}
...
export type PetWarfrontMatchProps = {
    ...
    /** Locks replay/restart while a rewarded result is still authoritative or
     * while its post-match ceremony is awaiting an explicit player choice. */
    resultActionsLocked?: boolean;
    /** Makes the blocked exit state visible while the one-use settlement is in
     * flight. The parent still owns the defensive onExit guard. */
    settlementPending?: boolean;
    /** Authoritative post-settlement presentation (for example, Living
     * Witness recognition). It is mounted only on the completed result view. */
    ...
    return createPortal((
        <div className="pet-combat-takeover pet-warfront-takeover" style={{ backgroundColor: "#05060a" }}>
            ...
            <div className="wf-top-controls" style={{ position: "absolute", top: 10, left: 12, display: "flex", gap: 8, alignItems: "center" }}>
                <button onClick={onExit} disabled={settlementPending} aria-busy={settlementPending} style={actionButtonStyle(settlementPending)}>✕ Exit</button>
                <button onClick={doReplay} disabled={resultActionsLocked} style={actionButtonStyle(resultActionsLocked)} title="Rewatch this match from the start">⟲ Replay</button>
                <button onClick={doRestart} disabled={resultActionsLocked} style={actionButtonStyle(resultActionsLocked)} title="Fresh match, same seed">↻ Restart</button>
            ...
            <div className="wf-camera-modes" style={{ position: "absolute", top: 42, left: 12, display: "flex", gap: 4 }}>
                {([
                    ...
                    <button
                        key={id}
                        onClick={() => { setCamMode(id); camCtl.current.mode = "follow"; setFreeCam(false); }}
                        title={tip}
                ...
                {freeCam && (
                    <button
                        ...
                        style={{ padding: "3px 10px", background: "rgba(109,40,217,0.94)", border: "1px solid #a78bfa", borderRadius: 999, color: "#fff", cursor: "pointer", font: "700 11px Inter, system-ui, sans-serif" }}
            ...
            {ended && (
                <div style={{ position: "absolute", inset: 0, display: "grid", alignItems: "start", justifyItems: "center", overflowY: "auto", padding: "clamp(14px, 5vh, 56px) 14px 22px", boxSizing: "border-box", background: "rgba(3,7,18,0.55)", zIndex: 70 }}>
                    <div style={{ width: "min(100%, 720px)", textAlign: "center", animation: "wfEndIn 0.5s ease-out" }}>
                        ...
                        {resultSupplement ? (
                            <div style={{ margin: "14px auto 0", maxWidth: 640, textAlign: "left" }}>
                                {resultSupplement}
                            </div>
                        ) : null}
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "center", marginTop: 14 }}>
                            <button onClick={doReplay} disabled={resultActionsLocked} style={{ ...actionButtonStyle(resultActionsLocked), padding: "8px 14px" }}>⟲ Replay</button>
                            <button onClick={doRestart} disabled={resultActionsLocked} style={{ ...actionButtonStyle(resultActionsLocked), padding: "8px 14px" }}>↻ Restart</button>
                            ...
                            <button onClick={onExit} disabled={settlementPending} aria-busy={settlementPending} style={{ ...actionButtonStyle(settlementPending), background: "#1e3a8a", border: "1px solid #3b82f6", padding: "8px 14px" }}>Exit</button>
                        </div>
                    </div>
                </div>
            )}
