...
function FighterReadabilityLayer({ result, fighters, cues, clockRef, reducedMotion }: {
    result: DuelResult;
...
/**
 * One draw per effect family, regardless of how many simultaneous hits land.
 * The former pool rendered nine independently-materialed meshes per live event
 * (and mounted an HTML projection observer for every slot). Instance colour
 * carries event ownership while scale-to-zero supplies per-instance lifetime.
 */
function BatchedCombatPulses({ slots, clockRef, sparkCount }: {
    slots: readonly MutableRefObject<ImpactSlot>[];
    clockRef: MutableRefObject<number>;
    sparkCount: number;
}) {
    ...
    const transform = useMemo(() => new THREE.Object3D(), []);
    const tint = useMemo(() => new THREE.Color(), []);
    const shardCapacity = slots.length * sparkCount;

    useLayoutEffect(() => {
        const meshes = [cores.current, shards.current];
        transform.position.set(0, -100, 0);
        transform.rotation.set(0, 0, 0);
        transform.scale.setScalar(0);
        transform.updateMatrix();
    ...
    useFrame(() => {
        ...
        const shardMesh = shards.current;
        if (!coreMesh || !shardMesh) return;

        for (let index = 0; index < slots.length; index++) {
            const slot = slots[index].current;
            const age = (clockRef.current - slot.t) / DUEL_TPS;
            const life = fxLife(slot.kind);
            const active = slot.active && age >= 0 && age <= life;
            const k = active ? age / life : 1;
            if (!active && slot.active && age > life) slot.active = false;
            const emphatic = slot.kind === "crit" || slot.kind === "ko" || slot.kind === "ultimate" || slot.kind === "objective";
            const restorative = slot.kind === "heal" || slot.kind === "guard";
            const cast = slot.kind === "cast";
            const fade = active ? Math.max(0.035, 1 - k) : 0;
            tint.set(slot.color).multiplyScalar(fade);

            transform.position.set(slot.x, 0.76 + (active ? Math.sin(k * Math.PI) * 0.22 : 0), slot.z);
            transform.rotation.set(k * 4.2, slot.seed + k * 5.4, k * 2.8);
            transform.scale.setScalar(active ? fade * (emphatic ? 0.42 : 0.28) : 0);
            transform.updateMatrix();
            coreMesh.setMatrixAt(index, transform.matrix);
            coreMesh.setColorAt(index, tint);

            for (let shard = 0; shard < sparkCount; shard++) {
                const shardIndex = index * sparkCount + shard;
                const show = active && !cast && slot.kind !== "dodge";
                const angle = (shard / Math.max(1, sparkCount)) * Math.PI * 2 + slot.seed * 0.71;
                const radius = restorative ? 0.22 + (shard % 2) * 0.12 : 0.16 + k * (emphatic ? 1.9 : 1.25);
                const rise = restorative ? 0.12 + k * (1.5 + (shard % 3) * 0.22) : 0.28 + Math.sin(k * Math.PI) * (0.7 + (shard % 2) * 0.25);
                transform.position.set(slot.x + Math.cos(angle) * radius, rise, slot.z + Math.sin(angle) * radius);
                transform.rotation.set(k * 5 + shard, angle, k * 3.2);
                transform.scale.setScalar(show ? Math.max(0.001, (1 - k) * (emphatic ? 0.16 : 0.11)) : 0);
                transform.updateMatrix();
                shardMesh.setMatrixAt(shardIndex, transform.matrix);
                shardMesh.setColorAt(shardIndex, tint);
            }
        }

        for (const mesh of [coreMesh, shardMesh]) {
            mesh.instanceMatrix.needsUpdate = true;
            if (mesh.instanceColor) mesh.instanceColor.needsUpdate = true;
        }
    });
    ...
    return (
        <group name="wfr-batched-impact-layer">
            <instancedMesh ref={cores} args={[undefined, undefined, slots.length]} frustumCulled={false}>
                <octahedronGeometry args={[1, 0]} />
                <meshBasicMaterial vertexColors wireframe transparent opacity={0.86} depthWrite={false} blending={THREE.AdditiveBlending} />
            </instancedMesh>
            <instancedMesh ref={shards} args={[undefined, undefined, shardCapacity]} frustumCulled={false}>
                <octahedronGeometry args={[1, 0]} />
...
function PaperLantern({ color = "#ffb45c", position = [0, 0, 0] }: {
    ...
    useLayoutEffect(() => {
        ...
        for (const [index, y] of [0.61, 0.08].entries()) {
    ...
    return (
        <group position={position}>
            ...
            <instancedMesh ref={caps} args={[undefined, undefined, 2]} frustumCulled={false}>
...
function ShinobiMazeWall({ wall }: {
    ...
    const horizontalLattice = useRef<THREE.InstancedMesh>(null);
    const verticalLattice = useRef<THREE.InstancedMesh>(null);
    const posts = useRef<THREE.InstancedMesh>(null);
    const rails = useRef<THREE.InstancedMesh>(null);
    const eaves = useRef<THREE.InstancedMesh>(null);
    useLayoutEffect(() => {
        const transform = new THREE.Object3D();
        const write = (mesh: THREE.InstancedMesh | null, index: number, position: [number, number, number], rotation: [number, number, number] = [0, 0, 0]) => {
            if (!mesh) return;
        ...
        for (let index = 0; index < panelCount; index++) {
            ...
            const panelPosition: [number, number, number] = runsAlongX ? [along, 0.56, 0] : [0, 0.56, along];
            write(panels.current, index, panelPosition);
            for (let rail = 0; rail < 2; rail++) {
                const offset = rail ? 0.23 : -0.23;
                write(horizontalLattice.current, index * 2 + rail, runsAlongX
                    ? [along, 0.56 + offset, thickness * 0.36]
                    : [thickness * 0.36, 0.56 + offset, along]);
            }
            write(verticalLattice.current, index, runsAlongX
                ? [along, 0.56, thickness * 0.37]
                : [thickness * 0.37, 0.56, along]);
        }
        for (let index = 0; index < panelCount + 1; index++) {
            const along = -length / 2 + 0.12 + index * panelLength;
            write(posts.current, index, runsAlongX ? [along, 0.66, 0] : [0, 0.66, along]);
        }
        write(rails.current, 0, [0, 0.07, 0]);
        write(rails.current, 1, [0, 1.08, 0]);
        write(eaves.current, 0, runsAlongX ? [0, 1.31, -thickness * 0.46] : [-thickness * 0.46, 1.31, 0], runsAlongX ? [0.34, 0, 0] : [0, 0, -0.34]);
        write(eaves.current, 1, runsAlongX ? [0, 1.31, thickness * 0.46] : [thickness * 0.46, 1.31, 0], runsAlongX ? [-0.34, 0, 0] : [0, 0, 0.34]);
        for (const mesh of [panels.current, horizontalLattice.current, verticalLattice.current, posts.current, rails.current, eaves.current]) {
            if (!mesh) continue;
            mesh.instanceMatrix.needsUpdate = true;
            mesh.computeBoundingSphere();
        }
    }, [length, panelLength, runsAlongX, thickness]);
    return (
        <group position={[wall.x * WORLD_SCALE, 0, wall.y * WORLD_SCALE]}>
            {/* Warm translucent washi and an unmistakable timber lattice keep
                the collision silhouette readable. The lower timber sill now
                touches the shared court instead of sitting on a plinth. */}
            <instancedMesh ref={panels} args={[undefined, undefined, panelCount]} frustumCulled={false}>
            ...
            <instancedMesh ref={horizontalLattice} args={[undefined, undefined, panelCount * 2]} frustumCulled={false}>
            ...
            <instancedMesh ref={verticalLattice} args={[undefined, undefined, panelCount]} frustumCulled={false}>
                <boxGeometry args={[0.045, 1.02, 0.045]} />
                <meshStandardMaterial color="#42251a" roughness={0.75} />
            </instancedMesh>
            <instancedMesh ref={posts} args={[undefined, undefined, panelCount + 1]} frustumCulled={false}>
                <boxGeometry args={[0.105, 1.3, 0.105]} />
                <meshStandardMaterial color="#2b1711" roughness={0.72} />
            </instancedMesh>
            <instancedMesh ref={rails} args={[undefined, undefined, 2]} frustumCulled={false}>
            ...
            <instancedMesh ref={eaves} args={[undefined, undefined, 2]} frustumCulled={false}>
    ...
    return (
        <group position={position} rotation={[0, rotation, 0]}>
            {([-1, 1] as const).map((side) => (
                <group key={side}>
                    <mesh position={[side * 0.95, 0.12, 0]} castShadow>
                        <cylinderGeometry args={[0.27, 0.33, 0.24, 10]} />
                        <meshStandardMaterial color="#414946" roughness={0.94} />
                    </mesh>
                    <mesh position={[side * 0.95, 1.05, 0]} castShadow>
                        <cylinderGeometry args={[0.12, 0.17, 2.1, 10]} />
                        <meshStandardMaterial color="#4b211b" roughness={0.68} />
                    </mesh>
                    <mesh position={[side * 1.34, 2.22, 0]} rotation={[0, 0, side * 0.12]} castShadow>
                        <boxGeometry args={[0.82, 0.14, 0.32]} />
                        <meshStandardMaterial color="#672c22" roughness={0.6} />
                    </mesh>
                </group>
            ))}
            <mesh position={[0, 1.86, 0]} castShadow><boxGeometry args={[2.5, 0.14, 0.2]} /><meshStandardMaterial color="#3d1b17" roughness={0.7} /></mesh>
            <mesh position={[0, 2.17, 0]} castShadow><boxGeometry args={[2.35, 0.16, 0.3]} /><meshStandardMaterial color="#713026" roughness={0.6} /></mesh>
...
function BambooGrove({ x, z, mirror = 1 }: { x: number; z: number; mirror?: number }) {
    ...
    useLayoutEffect(() => {
        const stalkMesh = stalks.current;
        const leafMesh = leaves.current;
        if (!stalkMesh || !leafMesh) return;
        const transform = new THREE.Object3D();
        for (let index = 0; index < 7; index++) {
            const px = ((index % 3) - 1) * 0.34 * mirror;
            const pz = (Math.floor(index / 3) - 0.8) * 0.38;
            const height = 1.25 + (index % 3) * 0.24;
            transform.position.set(px, height / 2, pz);
            transform.rotation.set(0, 0, 0);
            transform.scale.set(1, height, 1);
            transform.updateMatrix();
            stalkMesh.setMatrixAt(index, transform.matrix);
            transform.position.set(px + 0.16 * mirror, height * 0.82, pz);
            transform.rotation.set(0, 0, -0.65 * mirror);
            transform.scale.set(1, 1, 1);
            transform.updateMatrix();
        ...
        stalkMesh.computeBoundingSphere();
        leafMesh.computeBoundingSphere();
    }, [mirror]);
    return (
        <group position={[x, 0, z]}>
            <instancedMesh ref={stalks} args={[undefined, undefined, 7]} frustumCulled={false}>
                <cylinderGeometry args={[0.045, 0.065, 1, 7]} />
                <meshStandardMaterial color="#31543a" roughness={0.88} />
            </instancedMesh>
            <instancedMesh ref={leaves} args={[undefined, undefined, 7]} frustumCulled={false}>
                <coneGeometry args={[0.18, 0.48, 5]} />
                <meshStandardMaterial color="#456d46" roughness={0.96} />
            </instancedMesh>
        </group>
    );
}
...
function KageSmoke({ x, reducedMotion }: { x: number; reducedMotion: boolean }) {
    const root = useRef<THREE.Group>(null);
    useFrame(({ clock }) => {
        if (!root.current) return;
        root.current.rotation.y = reducedMotion ? 0 : clock.elapsedTime * (x < 0 ? 0.08 : -0.08);
        root.current.position.y = reducedMotion ? 0.2 : 0.2 + Math.sin(clock.elapsedTime * 0.7 + x) * 0.05;
    });
    return (
...
function arenaPiece(
    ...
    for (let index = 0; index < vertexCount; index++) color.toArray(colors, index * 3);
    geometry.setAttribute("color", new THREE.BufferAttribute(colors, 3));
    bucket.push(geometry);
}

function finishArenaBucket(geometries: THREE.BufferGeometry[]): THREE.BufferGeometry {
...
/**
 * Battle-specific arena mesh. It preserves every board boundary, blocker,
 * team gate and lantern landmark, but bakes their immutable transforms and
 * vertex colours into three shared buffers. The previous component tree used
 * roughly ninety draw calls for the same static silhouettes.
 */
function BatchedKageTacticsArena() {
    const halfX = WARFRONT_ARENA_X * WORLD_SCALE * 1.08;
    const halfZ = WARFRONT_ARENA_Y * WORLD_SCALE * 1.12;
    const geometries = useMemo(() => {
        const buckets: ArenaGeometryBuckets = { opaque: [], accent: [], glow: [] };
        const addBox = (
            bucket: THREE.BufferGeometry[],
            position: readonly [number, number, number],
            ...
            rotation: readonly [number, number, number] = [0, 0, 0],
            parent?: THREE.Matrix4,
        ) => arenaPiece(bucket, new THREE.BoxGeometry(1, 1, 1), color, arenaMatrix(position, rotation, scale, parent));
        const addLantern = (position: readonly [number, number, number], glow: THREE.ColorRepresentation, parent?: THREE.Matrix4) => {
            arenaPiece(buckets.glow, new THREE.CylinderGeometry(0.13, 0.16, 0.46, 8), glow,
                arenaMatrix([position[0], position[1] + 0.34, position[2]], [0, 0, 0], [1, 1, 1], parent));
            addBox(buckets.opaque, [position[0], position[1] + 0.61, position[2]], [0.18, 0.07, 0.18], "#24160f", [0, 0, 0], parent);
            addBox(buckets.opaque, [position[0], position[1] + 0.08, position[2]], [0.18, 0.07, 0.18], "#24160f", [0, 0, 0], parent);
        };

        // No live slab: KageBoardCells contributes only a transparent shadow
        // catcher and flush octagonal inlays over the authored courtyard.
        ...
        for (const wall of WARFRONT_MAZE_WALLS) {
            ...
            const length = Math.max(width, depth);
            const thickness = Math.min(width, depth);
            const x = wall.x * WORLD_SCALE;
            const z = wall.y * WORLD_SCALE;
            addBox(buckets.opaque, [x, 0.035, z], [width * 1.05, 0.07, depth * 1.18], "#171a19");
            addBox(buckets.accent, [x, 0.56, z], alongX ? [length * 0.94, 1.0, thickness * 0.55] : [thickness * 0.55, 1.0, length * 0.94], "#c8b999");
            for (let post = 0; post < 4; post++) {
                const offset = -length * 0.47 + length * 0.94 * post / 3;
                addBox(buckets.opaque, alongX ? [x + offset, 0.64, z] : [x, 0.64, z + offset], [0.09, 1.28, 0.09], "#352018");
            }
            for (const railY of [0.08, 0.82] as const) {
                addBox(buckets.opaque, [x, railY, z], alongX ? [length * 0.98, 0.065, thickness * 0.74] : [thickness * 0.74, 0.065, length * 0.98], "#45271b");
            }
            addBox(buckets.opaque, [x, 1.29, z], alongX ? [length * 1.08, 0.13, thickness * 1.14] : [thickness * 1.14, 0.13, length * 1.08], "#182022");
            addBox(buckets.accent, [x, 1.365, z], alongX ? [length * 0.92, 0.025, thickness * 0.82] : [thickness * 0.82, 0.025, length * 0.92], "#8e6338");
        }

        for (const [xCell, zCell, teamColor] of [
            [-3.2, -6, TEAM_COLOR.player],
            [3.2, 6, TEAM_COLOR.enemy],
        ] as const) {
            const x = xCell * WORLD_SCALE;
            const z = zCell * WORLD_SCALE;
            arenaPiece(buckets.accent, new THREE.RingGeometry(0.5, 0.78, 24), "#a17f4c", arenaMatrix([x, 0.039, z], [-Math.PI / 2, 0, 0]));
            addLantern([x, 0, z], teamColor);
        }

        for (const side of [-1, 1] as const) {
            const x = side * halfX * 0.995;
            for (const zScale of [-0.68, 0, 0.68] as const) {
                addBox(buckets.accent, [x, 0.016, zScale * halfZ], [0.045, 0.016, halfZ * 0.34], side < 0 ? "#4d9cac" : "#a64e43");
            }
        }

        const addGate = (position: readonly [number, number, number], yaw: number, team: "player" | "enemy") => {
            const parent = arenaMatrix(position, [0, yaw, 0]);
            const glow = team === "player" ? "#63cfe2" : "#ff654f";
            for (const side of [-1, 1] as const) {
                arenaPiece(buckets.opaque, new THREE.CylinderGeometry(0.27, 0.33, 0.24, 8), "#414946",
                    arenaMatrix([side * 0.95, 0.12, 0], [0, 0, 0], [1, 1, 1], parent));
                arenaPiece(buckets.opaque, new THREE.CylinderGeometry(0.12, 0.17, 2.1, 8), "#4b211b",
                    arenaMatrix([side * 0.95, 1.05, 0], [0, 0, 0], [1, 1, 1], parent));
                addBox(buckets.opaque, [side * 1.34, 2.22, 0], [0.82, 0.14, 0.32], "#672c22", [0, 0, side * 0.12], parent);
            }
            addBox(buckets.opaque, [0, 1.86, 0], [2.5, 0.14, 0.2], "#3d1b17", [0, 0, 0], parent);
            addBox(buckets.opaque, [0, 2.17, 0], [2.35, 0.16, 0.3], "#713026", [0, 0, 0], parent);
            addBox(buckets.accent, [0, 1.52, 0.03], [0.82, 0.38, 0.06], team === "player" ? "#174f68" : "#742d27", [0, 0, 0], parent);
            addLantern([-1.28, 1.45, 0], glow, parent);
            addLantern([1.28, 1.45, 0], glow, parent);
        };
        addGate([-halfX * 0.82, 0, -halfZ * 0.9], 0, "player");
        addGate([halfX * 0.82, 0, halfZ * 0.9], Math.PI, "enemy");

        arenaPiece(buckets.accent, new THREE.RingGeometry(0.68, 0.78, 24), "#c19a61",
            arenaMatrix([0, 0.034, 0], [-Math.PI / 2, 0, 0]));

        return {
            opaque: finishArenaBucket(buckets.opaque),
            accent: finishArenaBucket(buckets.accent),
            glow: finishArenaBucket(buckets.glow),
        };
    }, [halfX, halfZ]);

    useEffect(() => () => {
        geometries.opaque.dispose();
        geometries.accent.dispose();
        geometries.glow.dispose();
    }, [geometries]);

    return (
        <group name="wfr-batched-static-arena">
            <mesh geometry={geometries.opaque} receiveShadow={false} castShadow={false}>
                <meshStandardMaterial vertexColors roughness={0.82} metalness={0.08} />
            </mesh>
            <mesh geometry={geometries.accent}>
...
/** A continuous authored court with collision-faithful shoji and landmarks.
 * Legal cells remain simulation data, not raised presentation geometry. */
function KageTacticsArena({ reducedMotion }: { reducedMotion: boolean }) {
    const halfX = WARFRONT_ARENA_X * WORLD_SCALE * 1.08;
    const halfZ = WARFRONT_ARENA_Y * WORLD_SCALE * 1.12;
    return (
        <group>
            ...
            {([[-3.2, -6], [3.2, 6]] as const).map(([x, z], index) => (
                <group key={index} position={[x * WORLD_SCALE, 0, z * WORLD_SCALE]}>
                    <mesh position={[0, 0.039, 0]} rotation={[-Math.PI / 2, 0, 0]}><ringGeometry args={[0.5, 0.78, 24]} /><meshStandardMaterial color="#a17f4c" emissive="#4d3519" emissiveIntensity={0.12} roughness={0.72} metalness={0.18} /></mesh>
                    <PaperLantern position={[0, 0, 0]} color={index === 0 ? "#66d8ec" : "#ff7867"} />
                </group>
            ))}
            ...
            <group position={[0, 0.075, 0]}>
                ...
                <mesh rotation={[-Math.PI / 2, 0, Math.PI / 4]}><ringGeometry args={[0.3, 0.37, 4]} /><meshBasicMaterial color="#c7a368" transparent opacity={0.38} depthWrite={false} /></mesh>
                {([0, 1, 2, 3] as const).map((quarter) => (
                    <mesh key={quarter} rotation={[-Math.PI / 2, 0, quarter * Math.PI / 2]} position={[Math.cos(quarter * Math.PI / 2) * 0.52, 0.004, Math.sin(quarter * Math.PI / 2) * 0.52]}>
                        <planeGeometry args={[0.08, 0.32]} /><meshBasicMaterial color="#aa8551" transparent opacity={0.3} depthWrite={false} />
                    </mesh>
                ))}
            </group>
            <KageSmoke x={-3.2 * WORLD_SCALE} reducedMotion={reducedMotion} />
            <KageSmoke x={3.2 * WORLD_SCALE} reducedMotion={reducedMotion} />
            {([-1, 1] as const).map((side) => (
                <group key={side} position={[side * halfX * 0.995, 0, 0]}>
                    {([-0.68, 0, 0.68] as const).map((z) => (
                        <group key={z} position={[0, 0, z * halfZ]}>
                            <mesh position={[0, 0.008, 0]}><boxGeometry args={[0.045, 0.016, halfZ * 0.34]} /><meshBasicMaterial color={side < 0 ? "#4d9cac" : "#a64e43"} transparent opacity={0.48} /></mesh>
                        </group>
                    ))}
                </group>
            ))}
            <ToriiGate team="player" position={[-halfX * 0.82, 0, -halfZ * 0.9]} />
            <ToriiGate team="enemy" position={[halfX * 0.82, 0, halfZ * 0.9]} rotation={Math.PI} />
        </group>
    );
}
