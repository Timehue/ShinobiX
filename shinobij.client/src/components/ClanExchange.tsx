...
// api/clan/_exchange-parity.test.ts fails if this file drifts.
...
// source of truth). Names/descriptions/tiers/halls/levels/costs/limits/rewards must match.
...
// Hall shelves — order + unlock levels mirror clanHallTier() breakpoints on the
// server (camp 1 / dojo 7 / compound 15 / fortress 25 / citadel 40).
const EXCHANGE_TIERS = [
    { hall: "camp" as ExchangeHall, level: 1, label: "Clan Camp · Member issue", icon: "scroll" as GameIconName },
    { hall: "dojo" as ExchangeHall, level: 7, label: "Fortified Dojo · Proven contributors", icon: "shield" as GameIconName },
    { hall: "compound" as ExchangeHall, level: 15, label: "Hidden Compound · Officer stores", icon: "shield" as GameIconName },
    { hall: "fortress" as ExchangeHall, level: 25, label: "War Fortress · War stores", icon: "sword" as GameIconName },
    { hall: "citadel" as ExchangeHall, level: 40, label: "Legendary Citadel · Elite vault", icon: "sigil" as GameIconName },
] as const;
            ...
            {(futureGroups.length > 0 || comingSoonItems.length > 0) && <section className="clan-exchange-progression">
                ...
                <div className="clan-exchange-future-shelves">
                    {futureGroups.map((group, index) => (
                        <details className="clan-exchange-future-group" key={group.hall} open={index === 0}>
                            <summary>
                                <span><GameIcon name={group.icon} size={20} /> {group.label}</span>
