...
function encodeMaskRle(value) {
    const runs = [];
    let bit = value[0], run = 0;
    for (const next of value) {
        if (next === bit) run++;
        else { runs.push(run); bit = next; run = 1; }
    }
    runs.push(run);
    const bytes = [];
...
const maskRle = encodeMaskRle(bits);
const maskRleSource = Array.from({ length: Math.ceil(maskRle.length / 100) }, (_, i) =>
    `    "${maskRle.slice(i * 100, (i + 1) * 100)}"`,
).join(' +\n');
// connectivity report
