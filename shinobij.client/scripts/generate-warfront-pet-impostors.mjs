...
function tsUrlManifest(entries) {
    const revisions = Object.fromEntries(entries.map((entry) => [entry.sourceUrl, entry.atlasSha256.slice(0, 12)]));
    return `/** Generated by scripts/generate-warfront-pet-impostors.mjs. Do not edit. */\nexport const WARFRONT_IMPOSTOR_ATLAS_REVISIONS = ${JSON.stringify(revisions, null, 4)} as const;\n`;
}
...
if (checkOnly) {
    ...
    await writeStable(tsUrlManifestPath, tsUrlBytes);
}
