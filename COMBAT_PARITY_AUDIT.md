...
- Focused tests were added for Clan Boss boss resolution and tower weapon/item cooldown behavior.
...
| Battle Towers API/rewards | `api/towers/start.ts`, `api/towers/action.ts`, `api/towers/settle.ts`, `api/towers/_tower-store.ts`, `api/towers/_tower-rewards.ts`, `api/towers/join.ts`, `api/towers/state.ts` | Server-authoritative tower run lifecycle | Yes | Yes for combat; mode-specific for rewards | No | Uses tower engine and idempotent reward receipts; persistent item deduction is not yet unified with PvP `itemsUsed` | High |
...
| Consumables/throwables work | Partially verified | Tower engine spends sealed per-fight charges and now enforces cooldowns; persistent inventory deduction remains a parity gap |
...
#### Remaining: Persistent Item Deduction Is Not Unified For Towers/Clan Boss

- Severity: High for economy parity, Medium for immediate combat outcome
- What is broken: Tower/Clan Boss fights spend sealed per-fight charges in the session, but persistent inventory deduction is not wired to PvP's `itemsUsed` settlement model.
...
- Actual behavior: PvP tracks `itemsUsed` and deducts on claim; tower actors only mutate `itemCharges` inside the run.
- Files involved: `api/towers/_engine.ts`, `api/towers/_tower-session.ts`, `api/towers/_tower-store.ts`, `api/clan-boss/assault-settle.ts`, `api/pvp/claim-rewards.ts`
- Likely cause: Tower session schema does not currently persist an `itemsUsed` ledger comparable to PvP.
- Suggested fix: Add a shared item-spend helper, add an `itemsUsed` ledger to `TowerActor`, update engine spend points, and settle/deduct under player save locks for win/loss/timeout paths. This needs compatibility handling for pre-existing sessions.
- Confidence: High.
...
| Tower/Clan Boss item cleanup | `api/towers/_engine.ts`, `api/towers/_tower-store.ts`, `api/clan-boss/assault-settle.ts` | Per-fight charges are spent, but persistent inventory deduction is not tracked like PvP `itemsUsed` | Economy parity gap; players may keep consumed items after tower/Clan Boss fights | Add `itemsUsed` ledger to tower actors and a shared exactly-once deduction helper | High | High |
| Tower consumable enemy-target items | `api/towers/_engine.ts` | Tower item action has no target id and currently treats consumables as self-use; PvP can apply opponent/both-target support item effects | Items like Smoke Bomb may not match PvP semantics in N-actor fights | Design a target contract for tower items, then apply PvP item targeting rules to selected/all hostile actors as intended | Medium | Medium |
...
- `npx tsx --test api/clan-boss/_storage.test.ts api/towers/_engine.test.ts` - passed, 73 tests.
- `npm test` - passed, 2262 tests.
- `npm run build` - passed, including server compile, client type-check/bundle, and `verify:dist`.

`npm run lint` inside `shinobij.client/` was not run because this pass did not
touch frontend source. The client type-check and production bundle did run as
