...
test('beta report URL is bounded, aggregate population is explicit, and credentials stay in headers', async () => {
  ...
  const body = await fetchBetaReport({
    baseUrl: 'https://shinobijourney.com/game?secret=no',
  ...
  assert.equal(calls[0].url, 'https://shinobijourney.com/api/admin/beta-metrics?days=60&includePopulation=1&format=text');
...
test('beta report refuses plaintext remote transport and missing admin credentials', async () => {
  assert.throws(() => betaReportUrl('http://example.com'), /approved ShinobiX origin/);
  assert.throws(() => betaReportUrl('https://evil.example'), /approved ShinobiX origin/);
  assert.equal(betaReportUrl('http://127.0.0.1:3000').protocol, 'http:');
  await assert.rejects(() => fetchBetaReport({ baseUrl: 'https://shinobijourney.com', adminPassword: '' }), /ADMIN_PASSWORD/);
});
