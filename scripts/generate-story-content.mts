...
async function expectedFiles() {
    ...
    return { assets, epilogueAssets, fieldAsset, echoes, manifest };
}

async function main() {
    const { assets, epilogueAssets, fieldAsset, echoes, manifest } = await expectedFiles();
    ...
    console.log(`[story-content] ${checkOnly ? "verified" : "generated"} ${assets.length} village assets (${raw} B raw / ${gzip} B gzip), ${epilogueAssets.length} epilogue assets (${epilogueRaw} B raw / ${epilogueGzip} B gzip), field journeys (${fieldAsset.raw} B raw / ${fieldAsset.gzip} B gzip), and Echoes of War (${echoes.raw} B raw / ${echoes.gzip} B gzip)`);
}
