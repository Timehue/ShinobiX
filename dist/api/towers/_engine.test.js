...
(0, node_test_1.describe)('Battle Towers engine (P1.A2)', () => {
    ...
    (0, node_test_1.it)('AI seeks a contested shrine it can reach instead of only chasing the far target', () => {
        const shrineTile = 24; // (0,3), left; the combat target sits far right
        const mkMap = (withObjects) => ({
        ...
        const a = (0, _engine_js_1.pickAiAction)(seek, (0, _tower_session_js_1.getActor)(seek, 'en-1'), (0, _sim_js_1.makeRng)(1));
        node_assert_1.strict.equal(a.type, 'move');
        node_assert_1.strict.ok((0, _aoe_js_1.hexDistance)(a.tile, shrineTile, 8) < (0, _aoe_js_1.hexDistance)(27, shrineTile, 8), 'moves TOWARD the shrine');
        // Control: with no board objects the enemy heads for the target (far right) — old policy.
        const plain = build(false);
        const b = (0, _engine_js_1.pickAiAction)(plain, (0, _tower_session_js_1.getActor)(plain, 'en-1'), (0, _sim_js_1.makeRng)(1));
        node_assert_1.strict.equal(b.type, 'move');
        node_assert_1.strict.ok((0, _aoe_js_1.hexDistance)(b.tile, 31, 8) < (0, _aoe_js_1.hexDistance)(27, 31, 8), 'without objects, approaches the target');
    });
    (0, node_test_1.it)('AI standing in a hazard flees to a safe (non-hazard) tile', () => {
        ...
        s.activeAp = 100;
        s.actionsThisTurn = 0;
        const a = (0, _engine_js_1.pickAiAction)(s, (0, _tower_session_js_1.getActor)(s, 'en-1'), (0, _sim_js_1.makeRng)(1));
        node_assert_1.strict.equal(a.type, 'move');
        node_assert_1.strict.ok(!hazard.includes(a.tile), 'flees onto a safe tile off the hazard');
    });
...
(0, node_test_1.describe)('Battle Towers basic actions', () => {
    ...
    (0, node_test_1.it)('Push shoves the target AWAY, Pull drags it TOWARD, Debuff Prevent blocks it (PvP parity)', () => {
        const shove = (name) => ({ id: name.toLowerCase(), name, type: 'Taijutsu', ap: 40, range: 3, effectPower: 10, tags: [{ name }] });
        ...
        const distNow = (s) => (0, _aoe_js_1.hexDistance)((0, _tower_session_js_1.getActor)(s, 'en-1').pos, (0, _tower_session_js_1.getActor)(s, 'sq-1').pos, 8);
        const push = mk();
        const beforeP = distNow(push);
        node_assert_1.strict.ok((0, _engine_js_1.applyAction)(push, floor, { actorId: 'sq-1', type: 'jutsu', jutsuId: 'push', targetId: 'en-1' }, (0, _sim_js_1.makeRng)(1)).applied);
        node_assert_1.strict.ok(distNow(push) > beforeP, `push moved the target away (${distNow(push)} > ${beforeP})`);
        const pull = mk();
        const beforeL = distNow(pull);
        node_assert_1.strict.ok((0, _engine_js_1.applyAction)(pull, floor, { actorId: 'sq-1', type: 'jutsu', jutsuId: 'pull', targetId: 'en-1' }, (0, _sim_js_1.makeRng)(1)).applied);
        node_assert_1.strict.ok(distNow(pull) < beforeL, `pull moved the target closer (${distNow(pull)} < ${beforeL})`);
        const warded = mk({ statuses: [{ name: 'Debuff Prevent', rounds: 2, kind: 'positive' }] });
        const posBefore = (0, _tower_session_js_1.getActor)(warded, 'en-1').pos;
        (0, _engine_js_1.applyAction)(warded, floor, { actorId: 'sq-1', type: 'jutsu', jutsuId: 'push', targetId: 'en-1' }, (0, _sim_js_1.makeRng)(1));
        node_assert_1.strict.equal((0, _tower_session_js_1.getActor)(warded, 'en-1').pos, posBefore, 'Debuff Prevent blocks displacement (PvP parity)');
    });
});
