...
exports.buildMercCharacter = buildMercCharacter;
exports.resolveMercBattle = resolveMercBattle;
/*
...
function mercJutsuKit() {
    const strike = (id, name, element, tag) => ({
...
function playerActorFromSealed(name, ownerSlug, sealedChar) {
...
function resolveMercBattle(args) {
    ...
    const out = (0, _engine_js_1.runTowerFloor)(session, MERC_FLOOR, (0, _sim_js_1.makeRng)(args.seed));
    const playerDead = (out.actors.find((a) => a.id === 'sq-0')?.hp ?? 1) <= 0;
    const mercDead = (out.actors.find((a) => a.id === 'merc-0')?.hp ?? 1) <= 0;
    const winner = mercDead ? 'player' : playerDead ? 'merc' : 'stall';
    return {
        winner,
        mercWon: winner === 'merc',
