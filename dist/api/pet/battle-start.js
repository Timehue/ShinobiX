...
const clampLevel = (n) => Math.max(1, Math.min(100, Math.floor(Number.isFinite(n) ? n : 1)));
async function handler(req, res) {
    ...
    try {
        ...
        if (opponentName) {
            ...
            if (opponentPets.length && oppChar)
                realOpponentLevel = clampLevel(Number(oppChar.level ?? 1));
        }
        ...
        const sealedOpponentLevel = realOpponentLevel != null
            ? realOpponentLevel
            : clampLevel(Math.max(1, ...opponentPets.map((p) => Number(p.level ?? 1))));
