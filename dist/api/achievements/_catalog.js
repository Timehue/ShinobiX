...
exports.ACHIEVEMENT_RULES = exports.ACHIEVEMENT_TITLES = void 0;
exports.achievementTitlesForIds = achievementTitlesForIds;
exports.eligibleAchievementIds = eligibleAchievementIds;
exports.achievementRewardForIds = achievementRewardForIds;
const n = (character, field) => Math.max(0, Number(character[field]) || 0);
const countItems = (character) => {
    ...
    const stacks = Array.isArray(character.itemStacks)
        ? character.itemStacks.reduce((sum, stack) => sum + Math.max(0, Math.floor(Number(stack?.count) || 0)), 0)
        : 0;
...
const value = (character, field) => field.endsWith('.length')
    ? (Array.isArray(character[field.slice(0, -7)]) ? character[field.slice(0, -7)].length : 0)
    : n(character, field);
...
function achievementTitlesForIds(ids) {
    return [...new Set([...ids].map((id) => exports.ACHIEVEMENT_TITLES[id]).filter((title) => Boolean(title)))];
}
exports.ACHIEVEMENT_RULES = [
    ...numeric.map(([id, field, threshold]) => ({ id, hidden: HIDDEN_IDS.has(id), check: (character) => value(character, field) >= threshold })),
    ...
    { id: 'secret-weekly-bosses-5', hidden: true, check: (c) => c.weeklyBossKills != null && typeof c.weeklyBossKills === 'object' && Object.keys(c.weeklyBossKills).length >= 5 },
    { id: 'secret-minmaxer', hidden: true, check: (c) => n(c, 'level') >= 50 && n(c, 'unspentStats') === 0 },
];
function eligibleAchievementIds(character) {
    return exports.ACHIEVEMENT_RULES.filter((rule) => rule.check(character)).map((rule) => rule.id);
}
function achievementRewardForIds(ids) {
    let ryo = 0;
    let fateShards = 0;
    for (const id of new Set(ids)) {
        if (!exports.ACHIEVEMENT_RULES.some((rule) => rule.id === id))
            continue;
