...
(0, node_test_1.describe)('player auth hardening', () => {
    (0, node_test_1.it)('honors the emergency new-registration switch without blocking login', async () => {
        process.env.DISABLE_NEW_REGISTRATIONS = '1';
        const blocked = await register('closedbeta', 'StrongPass1');
        strict_1.default.equal(blocked.statusCode, 503);
        strict_1.default.equal(blocked.body?.code, 'registrations_disabled');
        strict_1.default.equal(store.has('auth:closedbeta'), false);
        delete process.env.DISABLE_NEW_REGISTRATIONS;
        strict_1.default.equal((await register('existingbeta', 'StrongPass1')).statusCode, 200);
        process.env.DISABLE_NEW_REGISTRATIONS = '1';
    ...
    (0, node_test_1.it)('rejects blocked usernames server-side, including a leetspeak variant', async () => {
        const blocked = await register('n1gger-ninja', 'StrongPass1');
        strict_1.default.equal(blocked.statusCode, 400);
        strict_1.default.match(String(blocked.body?.error), /not allowed/i);
        strict_1.default.equal(store.has('auth:n1gger-ninja'), false);
    });
    ...
    (0, node_test_1.it)('does not reveal whether an ordinary account name is unused during verify', async () => {
        const unused = await post({ action: 'verify', name: 'nobody', password: 'AnyPass1' });
        strict_1.default.equal(unused.statusCode, 200);
        strict_1.default.deepEqual(unused.body, { ok: false });
