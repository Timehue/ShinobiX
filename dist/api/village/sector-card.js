...
function sessionKey(sectorWarId) {
    return `sector-card:${sectorWarId}`;
}
async function saveSession(s) {
    await _storage_js_1.kv.set(sessionKey(s.sectorWarId), s, { ex: SESSION_TTL_SEC });
}
...
async function handler(req, res) {
    ...
    if (!identity.admin && !(await (0, _ratelimit_js_1.enforceRateLimitKv)(req, res, 'sector-card', 90, 60_000, identity.name)))
        return;
    try {
        ...
        function viewerKey(session) {
            if (session.p1.name.toLowerCase() === meLower)
                return 'p1';
            if (session.p2 && session.p2.name.toLowerCase() === meLower)
                return 'p2';
            return null;
        }
        ...
        if (action === 'state') {
            ...
            if (session.status === 'picking' && session.pickingDeadline && now > session.pickingDeadline) {
                await (0, _lock_js_1.withKvLock)(sessionKey(sectorWarId), async () => {
                    const fresh = await _storage_js_1.kv.get(sessionKey(sectorWarId));
                ...
                await (0, _lock_js_1.withKvLock)(sessionKey(sectorWarId), async () => {
                    const fresh = await _storage_js_1.kv.get(sessionKey(sectorWarId));
                    if (!fresh || fresh.status !== 'active' || !fresh.turnDeadline || now <= fresh.turnDeadline || !fresh.p2)
                        return;
            ...
            const latest = (await _storage_js_1.kv.get(sessionKey(sectorWarId))) ?? session;
        ...
        const result = await (0, _lock_js_1.withKvLock)(sessionKey(sectorWarId), async () => {
            ...
            if (action === 'join') {
                ...
                if (contest.winCondition !== 'card')
                    return { status: 409, body: { error: 'That sector is not a Card contest.' } };
                ...
                if (!existing || existing.status === 'done') {
                    if (!isAttackerSide)
                        return { status: 409, body: { error: 'Waiting for an attacker to open a card battle.' } };
                    const session = {
                    ...
                    await saveSession(session);
                    return { status: 200, body: { session: projectFor(session, 'p1') } };
                }
                // Idempotent re-join.
                if (existing.p1.name.toLowerCase() === meLower || (existing.p2 && existing.p2.name.toLowerCase() === meLower)) {
                    return { status: 200, body: { session: projectFor(existing, viewerKey(existing)) } };
                }
                if (existing.status !== 'awaiting-defender') {
                    return { status: 409, body: { error: 'Card battle is no longer accepting a joiner.' } };
                }
                if (!isDefenderSide) {
                    return { status: 409, body: { error: 'A defender of this sector must join the battle.' } };
                }
                const session = {
                ...
                await saveSession(session);
                return { status: 200, body: { session: projectFor(session, 'p2') } };
            }
            if (!existing)
        ...
        // failClosed: the sector-card action RMW (join/pick/commit) mutates the shared
        // session — under lock contention we 500 (client retries) rather than fall through
        // to an unlocked write that could clobber the other player's committed move. The
        // 'state' poll path returns above and never reaches this lock, and the picking/turn
        // auto-resolve locks stay open so routine polls can't 500. The contest→territory
        // settlement (applyCardOutcomeToContest) already runs failClosed with its own guard.
