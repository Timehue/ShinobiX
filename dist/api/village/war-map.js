...
async function handler(req, res) {
    ...
    try {
        const [warRaws, stateRaws, contests, territoryKeys] = await Promise.all([
            Promise.all(_war_map_sectors_js_1.WAR_VILLAGES.map((v) => _storage_js_1.kv.get((0, _war_state_js_1.villageWarKey)(v)))),
            Promise.all(_war_map_sectors_js_1.WAR_VILLAGES.map((v) => _storage_js_1.kv.get(`${VILLAGE_STATE_PREFIX}${(0, _war_state_js_1.villageWarSlug)(v)}`))),
            (0, _sector_war_store_js_1.listActiveSectorWars)(),
            _storage_js_1.kv.keys(`${TERRITORY_KEY_PREFIX}*`),
        ]);
        ...
        const villages = _war_map_sectors_js_1.WAR_VILLAGES.map((v, i) => {
            ...
            const treasury = (stateRaws[i]?.treasury ?? {});
            const treasurySeals = Number(treasury.honorSeals) || 0;
            return (0, _war_map_view_js_1.villageWarMapView)({ village: v, record, treasurySeals, sectorsHeld: heldCount[v] ?? 0 });
        });
