...
function applyDefense(state, challengerName, now) {
    ...
    (0, _utils_js_1.setSafeRecordValue)(cooldowns, lower(challengerName), now + exports.KAGE_LOSS_COOLDOWN_MS);
...
function applyExpiry(state, now) {
    ...
    const cooldowns = { ...(state.challengerCooldowns ?? {}) };
    if (challenge)
        (0, _utils_js_1.setSafeRecordValue)(cooldowns, lower(challenge.challenger), now + exports.KAGE_LOSS_COOLDOWN_MS);
...
function pruneCooldowns(cooldowns, now) {
    ...
    for (const [k, v] of Object.entries(cooldowns))
        if (v > now)
            (0, _utils_js_1.setSafeRecordValue)(out, k, v);
    return out;
}
