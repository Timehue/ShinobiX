...
async function handler(req, res) {
    ...
    try {
        ...
        const raidTokenRaw = typeof body.raidToken === 'string' && body.raidToken.trim()
            ? body.raidToken.trim()
            : undefined;
        const raidToken = raidTokenRaw && /^[A-Za-z0-9]+$/.test(raidTokenRaw)
            ? raidTokenRaw
            : undefined;
        ...
        if (raidId && !identity.admin) {
            ...
            const placed = await _storage_js_1.kv.set(idemKey, '1', { nx: true, ex: RAID_REPORT_DEDUP_TTL_SECONDS }).catch(() => 'error');
            if (placed === null) {
        ...
        const capCheck = await (0, _lock_js_1.withKvLock)(dailyKey, async () => {
            const reportedToday = Number((await _storage_js_1.kv.get(dailyKey)) ?? 0);
