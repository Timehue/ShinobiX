 ...
 */
function validateClanSaveWrite(existing, incoming, ctx) {
    const suppressed = [];
    const prev = existing ?? {};
    ...
    if (prev.name && lower(incoming.name) !== lower(prev.name)) {
        if (!ctx.isAdmin) {
            next.name = prev.name;
            suppressed.push('name change (admin only)');
        }
    }
    ...
    if (!prev.name && incoming.name && !ctx.isAdmin) {
        if (!(0, _text_moderation_js_1.isCleanText)(String(incoming.name))) {
    ...
    if (prev.createdAt && incoming.createdAt && Number(incoming.createdAt) !== Number(prev.createdAt)) {
        if (!ctx.isAdmin) {
            ...
            suppressed.push('createdAt change (admin only)');
        }
    }
    if (prev.village && lower(incoming.village) !== lower(prev.village)) {
        if (!ctx.isAdmin) {
            next.village = prev.village;
    ...
    if (Array.isArray(incoming.members)) {
        ...
        const prevByName = new Map(prevMembers.map((m) => [lower(m.name), m]));
        const incomingMembers = incoming.members.slice(0, MAX_MEMBERS);
        const incomingByName = new Map(incomingMembers.map((m) => [lower(m.name), m]));
        const added = [];
        const removed = [];
        for (const n of incomingByName.keys())
            if (n && !prevByName.has(n))
                added.push(n);
        ...
        if (!callerIsAdminRole) {
            ...
            if (illegalAdds.length > 0 || illegalRemoves.length > 0) {
                ...
                if (illegalAdds.length > 0)
                    suppressed.push(`members illegal add: ${illegalAdds.join(',')}`);
                if (illegalRemoves.length > 0)
                    suppressed.push(`members illegal remove: ${illegalRemoves.join(',')}`);
            }
            else {
    ...
    if (incoming.roleOverrides && typeof incoming.roleOverrides === 'object') {
        const prevOverrides = (prev.roleOverrides ?? {});
        const incomingOverrides = incoming.roleOverrides;
        if (callerIsFounder) {
            ...
            for (const k of allKeys) {
                if (lower(k) === ctx.callerName) {
                    if (incomingOverrides[k] !== undefined)
                        merged[k] = incomingOverrides[k];
                    else
                        delete merged[k];
                }
            }
            ...
            if (changedKeys.length > 0)
                suppressed.push(`roleOverrides change for [${changedKeys.join(',')}] (Founder only)`);
            if (removedKeys.length > 0)
                suppressed.push(`roleOverrides remove [${removedKeys.join(',')}] (Founder only)`);
    ...
    if (incoming.activeWar !== undefined) {
        const prevWar = (prev.activeWar ?? null);
        const inWar = incoming.activeWar;
        if (inWar === null) {
            ...
            if (incOur < prevOur && !ctx.isAdmin)
                suppressed.push('activeWar.ourScore decrease (admin only)');
            if (incEnemy < prevEnemy && !ctx.isAdmin)
                suppressed.push('activeWar.enemyScore decrease (admin only)');
            next.activeWar = {
                ...prevWar,
    ...
    if (incoming.treasury && typeof incoming.treasury === 'object') {
        const prevTreasury = (prev.treasury ?? {});
        const inTreasury = incoming.treasury;
        const outTreasury = { ...prevTreasury };
    ...
    if (Array.isArray(incoming.joinRequests)) {
        ...
        if (badAdds.length > 0 || badRemoves.length > 0) {
            ...
            if (badAdds.length > 0)
                suppressed.push(`joinRequest illegal add: ${badAdds.join(',')}`);
            if (badRemoves.length > 0)
                suppressed.push(`joinRequest illegal remove: ${badRemoves.join(',')}`);
        }
        else {
    ...
    if (Array.isArray(incoming.notices)) {
        ...
        const prevIds = new Set(prevNotices.map((n) => String(n.id ?? '')).filter(Boolean));
        const incomingNotices = incoming.notices.slice(0, MAX_NOTICES);
        const incomingIds = new Set(incomingNotices.map((n) => String(n.id ?? '')).filter(Boolean));
    ...
    if (finalWar && typeof finalWar === 'object') {
        ...
        if (endsAt > 0 && now - endsAt > ACTIVE_WAR_GRACE_MS) {
            ...
            const result = ourScore > enemyScore ? 'Won' :
