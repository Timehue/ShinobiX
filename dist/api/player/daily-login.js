...
async function handler(req, res) {
    ...
    try {
        ...
        if (!identity.admin && !(await (0, _ratelimit_js_1.enforceRateLimitKv)(req, res, 'daily-login', 30, 60_000, identity.name)))
            return;
        const today = utcDateOffset(0);
        const yesterday = utcDateOffset(-1);
        let out;
        try {
            out = await (0, _lock_js_1.withKvLock)(`save:${playerName}`, async () => {
                const rec = await _storage_js_1.kv.get(`save:${playerName}`);
                const char = (rec?.character ?? null);
                if (!rec || !char)
                    return { error: 'no-save' };
                const reward = (0, _daily_login_js_1.computeLoginReward)({
