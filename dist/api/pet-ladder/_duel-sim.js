...
function buildFighter(pet, team, slot, x, y, atkMult = 1, hpMult = 1, reviveOnce = false, applyItems = false) {
    ...
    if (applyItems)
        statuses.shieldHp = (0, _pet_gear_js_1.petGearStartShield)(gp);
    return {
...
function applyDamage(att, tgt, ab, rng, t, events, viaProjectile) {
    ...
    if (att.itemsOn)
        mult *= (0, _pet_gear_js_1.petGearExecuteMult)(att.pet, tgt.hp, tgt.maxHp);
    ...
    if (tgt.itemsOn) {
        dmg = Math.max(1, Math.round(dmg * (0, _pet_gear_js_1.petGearLastStandMult)(tgt.pet, tgt.hp, tgt.maxHp)));
    ...
    if (dmg > 0) {
        ...
        if (att.itemsOn && !ab) {
            const dot = (0, _pet_gear_js_1.petGearDotOnHit)(att.pet);
            if (dot) {
            ...
            const heal = (0, _pet_gear_js_1.petGearLifestealHeal)(att.pet, dmg);
            if (heal > 0)
...
const GCOLS = _walkmask_js_1.WALK_COLS, GROWS = _walkmask_js_1.WALK_ROWS;
...
const maskAt = (c, r) => _walkmask_js_1.WALK_MASK.charCodeAt(r * GCOLS + c) === 49; // '1'
...
function beginCast(f, idx, targetId, t, events) {
    ...
    if (idx >= 0 && f.abilities[idx].signature)
        events.push({ t, type: "ultimate", side: f.team, actorId: f.id });
    else if (idx >= 0 && f.abilities[idx].cls === "support")
        events.push({ t, type: "cast", side: f.team, actorId: f.id, kind: f.abilities[idx].kind });
    else
