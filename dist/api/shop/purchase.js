...
async function handler(req, res) {
    ...
    try {
        ...
        if (!identity.admin && !(await (0, _ratelimit_js_1.enforceRateLimitKv)(req, res, 'shop-purchase', 60, 60_000, identity.name)))
            return;
        const result = await (0, _mutate_player_save_js_1.mutatePlayerSave)(playerName, ({ character }) => {
            const receipts = Array.isArray(character.redeemedShopPurchases)
                ? character.redeemedShopPurchases.filter((entry) => entry && typeof entry.id === 'string') : [];
            const prior = receipts.find((entry) => entry.id === requestId);
            if (prior)
                return { ok: true, character, value: { purchase: prior.purchase, replayed: true } };
            const bought = (0, _purchase_js_1.purchaseCatalogItem)(character, body.itemId, body.qty);
            if (!bought.ok)
                return { ok: false, status: 409, error: bought.reason };
            const receipt = { id: requestId, purchase: bought.item, at: Date.now() };
            return { ok: true, character: { ...bought.character, redeemedShopPurchases: [...receipts.slice(-99), receipt] }, value: { purchase: bought.item, replayed: false } };
        });
        if (!result.ok)
