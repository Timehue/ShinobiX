...
async function handler(req, res) {
    ...
    try {
        ...
        if (!identity.admin && !(await (0, _ratelimit_js_1.enforceRateLimitKv)(req, res, 'hollow-gate-forge-key', 20, 60_000, identity.name)))
            return;
        const result = await (0, _mutate_player_save_js_1.mutatePlayerSave)(playerName, ({ character }) => {
            const forged = (0, _forge_key_js_1.forgeHollowGateKey)(character, source);
            if (!forged.ok)
                return { ok: false, status: 409, error: forged.reason };
            return { ok: true, character: forged.character, value: {} };
        });
        if (!result.ok)
