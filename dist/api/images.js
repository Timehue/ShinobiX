...
async function handler(req, res) {
        ...
        try {
            ...
            if (cat) {
                ...
                const [hashImages, catImages, legacyLeader, avatarMap] = await Promise.all([
                    ...
                    cat === 'avatar' ? withTimeout(_storage_js_1.kv.hgetall(AVATAR_MAP_KEY)) : Promise.resolve({}),
                ]);
            ...
            const [categoryEntries, legacyLeader, avatarMap] = await Promise.all([
                ...
                withTimeout(_storage_js_1.kv.hgetall(AVATAR_MAP_KEY)),
            ]);
    ...
    if (req.method === 'POST') {
        ...
        try {
            ...
            await _storage_js_1.kv.hset(cat === 'avatar' ? AVATAR_MAP_KEY : catHashKey(cat), { [id]: image });
            return res.status(200).end();
        }
        catch (err) {
