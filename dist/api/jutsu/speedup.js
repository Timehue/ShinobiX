...
function effectiveCost(seals, profession, professionRank) {
...
async function handler(req, res) {
    ...
    try {
        ...
        if (!identity.admin && !(await (0, _ratelimit_js_1.enforceRateLimitKv)(req, res, 'jutsu-speedup', 10, 60_000, identity.name)))
            return;
        ...
        const lockResult = await (0, _lock_js_1.withKvLock)(key, async () => {
            ...
            const activeJutsuTraining = record.activeJutsuTraining;
            if (!activeJutsuTraining || !activeJutsuTraining.endsAt) {
                return { status: 400, body: { error: 'No active jutsu training to speed up.' } };
            }
            const remainingMs = Number(activeJutsuTraining.endsAt) - Date.now();
            if (remainingMs <= 0) {
                return { status: 400, body: { error: 'Your training is already complete — collect it instead.' } };
            }
