...
function snapshotKeyFor(playerName, ts) {
...
async function handler(req, res) {
    ...
    try {
        ...
        if (action === 'snapshot') {
            ...
            if (!live)
                return res.status(404).json({ error: 'No live save for that player.' });
            const ts = Date.now();
            const key = snapshotKeyFor(playerName, ts);
            await _storage_js_1.kv.set(key, live, { ex: SNAPSHOT_TTL_SECONDS });
            return res.status(200).json({ ok: true, snapshotKey: key, ts });
        }
        if (action === 'list') {
            const keys = await _storage_js_1.kv.keys(`${snapshotPrefixFor(playerName)}*`);
            if (keys.length === 0)
        ...
        if (action === 'restore') {
            ...
            const snap = await _storage_js_1.kv.get(snapshotKey);
            if (!snap)
