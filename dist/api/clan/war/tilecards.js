...
function freshSide(name, clan, defaultDeck) {
    return {
...
async function handler(req, res) {
    ...
    if (!identity.admin && !(await (0, _ratelimit_js_1.enforceRateLimitKv)(req, res, 'cw-tilecards', 90, 60_000, identity.name)))
        return;
    try {
        ...
        if (action === 'state') {
            ...
            if (!session)
                return res.status(404).json({ error: 'No duel session yet.' });
            const now = Date.now();
            ...
            if (session.status === 'picking' && session.pickingDeadline && now > session.pickingDeadline) {
                await (0, _lock_js_1.withKvLock)(sessionKey(challengeId), async () => {
                    ...
                    if (!fresh || fresh.status !== 'picking' || !fresh.pickingDeadline || now <= fresh.pickingDeadline)
                        return;
                    await _storage_js_1.kv.set(sessionKey(challengeId), startMatch(fresh, now));
                });
            }
            ...
            const latest = (await _storage_js_1.kv.get(sessionKey(challengeId))) ?? session;
        ...
        const result = await (0, _lock_js_1.withKvLock)(sessionKey(challengeId), async () => {
            ...
            if (action === 'join') {
                ...
                if (ch.mode !== 'tilecards')
                    return { status: 400, body: { error: 'Challenge is not a card duel.' } };
            ...
            if (!existing)
                return { status: 404, body: { error: 'No duel session yet — call join first.' } };
            const meLower = me.toLowerCase();
