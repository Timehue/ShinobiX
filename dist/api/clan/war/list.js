...
async function handler(req, res) {
    ...
    try {
        ...
        // everything.
        const identity = await (0, _auth_js_1.authedPlayerOrAdmin)(req).catch(() => null);
        let viewerClan = '';
        let isAdmin = false;
        if (identity && identity.admin) {
            ...
            const ctx = await (0, _storage_js_1.loadClanContext)(identity.name).catch(() => null);
            viewerClan = ctx?.clan ?? '';
        }
        const wars = await (0, _storage_js_1.loadAllClanWars)();
        const now = Date.now();
        const projected = wars.map(w => (0, _storage_js_1.applyLazyClanWarExpiry)(w, now).war);
        const shaped = isAdmin
            ? projected
            : projected.map(w => (0, _storage_js_1.redactClanWarForViewer)(w, viewerClan));
