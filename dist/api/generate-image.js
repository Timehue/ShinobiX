...
function flagged(text) {
    for (const p of INJECTION_PATTERNS) {
        if (p.test(text))
            return { reason: 'prompt structure not allowed' };
    }
...
async function handler(req, res) {
    ...
    if (authedName) {
        ...
        const used = Number((await _storage_js_1.kv.get(dk)) ?? 0);
        ...
        await _storage_js_1.kv.set(dk, used + 1, { ex: 26 * 60 * 60 }).catch(() => undefined);
    }
    ...
    try {
        ...
        const finalPrompt = `Create a polished 2D anime shinobi RPG game asset for a family-friendly browser game.\n\n` +
            `<user_request>\n${promptStr}\n</user_request>\n\n` +
            `<asset_label>\n${labelStr}\n</asset_label>\n\n` +
