...
async function handler(req, res) {
    ...
    if (!(await (0, _ratelimit_js_1.enforceRateLimitKv)(req, res, 'pvp-combat-log', 60, 60_000)))
        return;
    ...
    let p1Name = '';
    let p2Name = '';
    const live = await _storage_js_1.kv.get(`pvp:${battleId}`);
    if (live) {
        p1Name = String(live.p1?.name ?? '');
        p2Name = String(live.p2?.name ?? '');
    }
    else {
        const battle = await (0, _receipts_js_1.readBattleReceipt)(battleId);
        if (battle) {
    ...
    if (!identity.admin) {
        ...
        const isParticipant = !!me && (me === (0, _utils_js_1.safeName)(p1Name) || me === (0, _utils_js_1.safeName)(p2Name));
        if (!isParticipant) {
            return res.status(403).json({ error: 'Only the battle participants or an admin can view this combat log.' });
        }
    }
    const entries = await (0, _receipts_js_1.readActionReceipts)(battleId);
