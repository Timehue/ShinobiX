...
const constants_js_1 = require("../combat-core/constants.js");
const grid_js_1 = require("../combat-core/grid.js");
const cooldowns_js_1 = require("../combat-core/cooldowns.js");
const resources_js_1 = require("../combat-core/resources.js");
const resolveJutsu_js_1 = require("../combat-core/resolveJutsu.js");
const formulas_js_1 = require("../combat-core/formulas.js");
const statuses_js_1 = require("../combat-core/statuses.js");
...
const _kage_settle_js_1 = require("../village/_kage-settle.js");
...
async function saveSession(key, session, opts = {}) {
    ...
    await _storage_js_1.kv.set(key, next, { ex: opts.ex ?? constants_js_1.SESSION_TTL });
}
...
const VFX_ELEMENTAL_60_KEYS = new Set(['fire60', 'water60', 'wind60', 'lightning60', 'earth60']);
...
function elemental60VfxKey(jutsu) {
    if (Number(jutsu.ap) !== 60 || jutsu.target === 'SELF')
        return null;
    const selected = (0, _jutsu_visuals_js_1.sanitizeJutsuVisualEffect)(jutsu.visualEffect, jutsu.ap, jutsu.target);
    if (selected)
        return selected;
...
function jutsuVisualAnchor(jutsu, key, opts = {}) {
    ...
    if ((0, _jutsu_visuals_js_1.sanitizeJutsuVisualEffect)(jutsu.visualEffect, jutsu.ap, jutsu.target))
        return 'target';
...
function generalsBonus(f, round) {
    return (0, formulas_js_1.generalsBonusFromStatuses)(activeStatuses(f, round), nameMatches);
}
function disciplineBonuses(f, round) {
    return (0, formulas_js_1.disciplineBonusesFromStatuses)(activeStatuses(f, round), nameMatches);
}
// ─── Fighter helpers ──────────────────────────────────────────────────────────
function isStatusActive(status, round) {
    return (0, statuses_js_1.isCombatStatusActive)(status, round);
}
function activeStatuses(f, round) {
    return (0, statuses_js_1.activeCombatStatuses)(f.statuses, round);
}
...
function hasStatus(f, name, round) {
    return (0, statuses_js_1.hasCombatStatus)(f.statuses, name, round, nameMatches);
}
function addStatus(f, s) {
    // v4.3: apply duration override (IDG/IDT/DDG/DDT → 2 rounds), then either stack or replace.
    return {
        ...f,
        statuses: (0, statuses_js_1.addCombatStatus)(f.statuses, s, {
            durationFor: formulas_js_1.statusDurationFor,
            isStackable: name => _tags_js_1.STACKABLE_STATUS.has(name),
            nameMatches,
        }),
    };
}
function countActive(f, name, round) {
    return (0, statuses_js_1.countActiveCombatStatuses)(f.statuses, name, round, nameMatches);
}
...
function sumActivePct(f, name, round, fallback = 30) {
    return (0, statuses_js_1.sumActiveCombatStatusPercent)(f.statuses, name, round, fallback);
}
...
function capWoundStacks(f) {
    const statuses = (0, statuses_js_1.capCombatStatusStacks)(f.statuses, 'Wound', formulas_js_1.MAX_WOUND_STACKS);
    return statuses === f.statuses ? f : { ...f, statuses: statuses };
}
function groundEffectTiles(center) {
    return [center, ...(0, grid_js_1.hexNeighbors)(center)];
}
...
function tickStatuses(f, round) {
    return { ...f, statuses: (0, statuses_js_1.tickCombatStatuses)(f.statuses, round) };
}
function tickCooldowns(cds) {
    return (0, cooldowns_js_1.tickCombatCooldowns)(cds);
}
...
function drContributionFor(attacker, defender, round) {
    return (0, formulas_js_1.drContributionFromStatuses)(activeStatuses(attacker, round), activeStatuses(defender, round));
}
...
function ampMultiplierFor(attacker, defender, round) {
    return (0, formulas_js_1.ampMultiplierFromStatuses)(activeStatuses(attacker, round), activeStatuses(defender, round), nameMatches);
}
...
function scaledTagPercent(rawPct, masteryLevel, tagName, bloodlineRank) {
    return (0, formulas_js_1.scaledTagPercent)(rawPct, masteryLevel, tagName, bloodlineRank, _tags_js_1.CAPPED_AMP_TAGS);
}
...
function resolveBaseDamage(self, opponent, jutsu, wMult, biome, round, masteryLevel) {
    ...
    return (0, formulas_js_1.directDamageBaseFormula)({
        jutsu,
        attackerStats: offStats,
        defenderStats: defStats,
        attackerCharacter: self.character,
        defenderCharacter: opponent.character,
        masteryLevel,
        wMult,
        biome,
...
function increaseHealMult(fighter, round) {
    return (0, formulas_js_1.healMultiplierFromStatuses)(activeStatuses(fighter, round));
}
...
function resolveTagStatuses(self, opponent, jutsu, round, masteryLevel, baseDmg, healBoost) {
    ...
    for (const tag of tags) {
        ...
        if (tagName === 'Heal') {
            const healAmt = (0, formulas_js_1.healAmountForMastery)(masteryLevel, healBoost);
            healing += healAmt;
            damage = 0;
        ...
        if (tagName === 'Shield') {
            const shieldAmt = (0, formulas_js_1.shieldAmountForMastery)(masteryLevel);
        ...
        if (tagName === 'Barrier') {
            const tile = (0, grid_js_1.nextStepToward)(s.pos, o.pos);
        ...
        if (tagName === 'Drain') {
            ...
            if (!hasStatus(o, 'Debuff Prevent', round)) {
                const drainTickAmount = (0, formulas_js_1.drainTick)(masteryLevel);
        ...
        if (tagName === 'Increase Discipline') {
            const disc = formulas_js_1.DISCIPLINE_OFFENSE_FIELD[String(jutsu.type ?? '')] ? jutsu.type : undefined;
        ...
        if (tagName === 'Push') {
            if (!hasStatus(o, 'Debuff Prevent', round)) {
                ...
                for (let step = 0; step < dist; step++) {
                    const away = (0, grid_js_1.hexNeighbors)(nextPos).filter(t => (0, grid_js_1.hexDistance)(t, s.pos) > (0, grid_js_1.hexDistance)(nextPos, s.pos) && t !== s.pos && !tileBlocked(t, s, o));
                    if (!away.length)
                        break;
                    nextPos = away[0];
                }
        ...
        if (tagName === 'Pull') {
            if (!hasStatus(o, 'Debuff Prevent', round)) {
                ...
                let nextPos = o.pos;
                for (let step = 0; step < dist; step++) {
                    const toward = (0, grid_js_1.hexNeighbors)(nextPos).filter(t => (0, grid_js_1.hexDistance)(t, s.pos) < (0, grid_js_1.hexDistance)(nextPos, s.pos) && t !== s.pos && !tileBlocked(t, s, o));
                    if (!toward.length)
                        break;
                    nextPos = toward[0];
                }
...
function resolveDamageNumber(self, opponent, jutsu, round, masteryLevel, offStats, damageIn, pierce, effectiveDR) {
    return (0, formulas_js_1.directDamageNumberFormula)({
        damageIn,
        pierce,
        offenseComposite: (0, formulas_js_1.getOffense)(offStats, jutsu.type),
        jutsuAp: jutsu.ap ?? 40,
        masteryLevel,
        effectiveDR,
...
function resolvePostDamage(sIn, oIn, jutsu, round, damage, pierce, healBoost) {
    ...
    const { blocked, finalDmg, reflectedDmg, absorbHeal, itemAbsorbHeal, itemReflectedDmg, itemLifeStealHeal, } = (0, formulas_js_1.postDamageFormula)({
        damage,
        shield: o.shield,
        pierce,
        reflectPct,
    ...
    for (const tag of tags) {
        ...
        if (tagName === 'Wound' && !hasStatus(o, 'Debuff Prevent', round)) {
            ...
            const amt = (0, formulas_js_1.woundAmountForFinalDamage)(finalDmg, pct, jutsu);
        ...
        // gated on finalDmg.)
        if (tagName === 'Siphon') {
            const h = (0, formulas_js_1.postDamagePercentAmount)(finalDmg, pct, healBoost);
    ...
    if (recoilStatus && finalDmg > 0) {
        const rc = (0, formulas_js_1.postDamagePercentAmount)(finalDmg, recoilStatus.percent ?? 30);
    ...
    if (lsPct > 0 && finalDmg > 0) {
        const h = (0, formulas_js_1.postDamagePercentAmount)(finalDmg, lsPct, healBoost);
        s = { ...s, hp: Math.min(s.maxHp, s.hp + h) };
...
const pvpResolveJutsuPhases = {
    ...
    makeHitFx: (who, amount, kind) => (amount > 0 ? { who, amount: Math.round(amount), kind } : undefined),
};
...
function applyJutsu(self, opponent, jutsu, wMult = 1, biome = 'central', round = 1) {
    ...
    const masteryLevel = Math.min(storedMastery, (0, formulas_js_1.jutsuLevelCapForLevel)(Number(self.character.level) || 1));
    ...
    const cappedSelf = { ...self, character: { ...self.character, stats: (0, formulas_js_1.withDisciplineBonuses)((0, formulas_js_1.withGeneralsBonus)((0, formulas_js_1.perRankStatCap)(self.character.stats ?? {}, Number(self.character.level) || 1), generalsBonus(self, round)), disciplineBonuses(self, round)) } };
    const cappedOpp = { ...opponent, character: { ...opponent.character, stats: (0, formulas_js_1.withDisciplineBonuses)((0, formulas_js_1.withGeneralsBonus)((0, formulas_js_1.perRankStatCap)(opponent.character.stats ?? {}, Number(opponent.character.level) || 1), generalsBonus(opponent, round)), disciplineBonuses(opponent, round)) } };
    const resolved = (0, resolveJutsu_js_1.resolveJutsu)({
        self,
        opponent,
...
function applyDoTs(fighter, round) {
    ...
    const dotMitigation = (0, formulas_js_1.dotMitigationFromRawDr)(ownArmor, ownStatusDR);
...
function applyQueuedMovement(target, source, round) {
    ...
    for (const status of movementStatuses) {
        ...
        for (let step = 0; step < dist; step++) {
            const candidates = (0, grid_js_1.hexNeighbors)(nextPos).filter(tile => {
                if (tile === source.pos || tileBlocked(tile, fighter, source))
                    return false;
                return status.name === 'Push'
                    ? (0, grid_js_1.hexDistance)(tile, source.pos) > (0, grid_js_1.hexDistance)(nextPos, source.pos)
                    : (0, grid_js_1.hexDistance)(tile, source.pos) < (0, grid_js_1.hexDistance)(nextPos, source.pos);
            });
...
function checkWinner(s) {
    ...
    else if (s.round > constants_js_1.MAX_ROUNDS) {
        status = 'done';
        if (p1.hp > p2.hp) {
            winner = 'p1';
...
function endTurn(session) {
    ...
    const baseAp = stunStatus ? Math.max(0, 100 - formulas_js_1.STUN_AP_PENALTY) : 100;
    if (stunStatus) {
...
async function handler(req, res) {
    ...
    try {
        ...
        function adjustedCost(base) {
            ...
            return (0, resources_js_1.adjustedApCost)(base, {
        ...
        switch (action) {
            ...
            case 'move': {
                ...
                if (!(0, grid_js_1.hexNeighbors)(me.pos).includes(tile) || tile === opp.pos || tileBlocked(tile, me, opp))
            ...
            case 'basicAttack': {
                ...
                if ((0, grid_js_1.hexDistance)(me.pos, opp.pos) > 1) {
            ...
            case 'jutsu': {
                ...
                if (!selfTarget && !groundTarget && !moveTag && affectsOpponent) {
                    ...
                    if (range > 0 && (0, grid_js_1.hexDistance)(me.pos, opp.pos) > range) {
                        return finish(await rejectWithLog(`${jutsu.name} is out of range (need ≤${range}, distance ${Math.round((0, grid_js_1.hexDistance)(me.pos, opp.pos))}).`));
                    }
                }
                ...
                const jWMult = (0, formulas_js_1.weatherMultiplier)(jutsu.weatherElement ?? jutsu.element, weatherPositiveElement, weatherNegativeElement);
                ...
                if (moveTag && tile !== undefined) {
                    ...
                    if (destTile < 0 || destTile >= constants_js_1.GRID_W * constants_js_1.GRID_H || (0, grid_js_1.hexDistance)(me.pos, destTile) > range || destTile === opp.pos || destTile === me.pos || tileBlocked(destTile, me, opp)) {
                    ...
                    if (jutsuMethod === 'AOE_SPIRAL') {
                        ...
                        const groundEffect = {
                            ...
                            owner: role,
                            name: jutsu.name,
                            tiles: (0, _aoe_js_1.filledDiskTiles)(destTile, constants_js_1.SPIRAL_RADIUS, constants_js_1.GRID_W, constants_js_1.GRID_H),
                            rounds: 2,
                            tags: zoneTags,
                        };
                ...
                if (groundTarget && tile !== undefined) {
                    ...
                    const range = Math.max(1, Number(jutsu.range) || 4);
                    if (targetTile < 0 || targetTile >= constants_js_1.GRID_W * constants_js_1.GRID_H || (0, grid_js_1.hexDistance)(me.pos, targetTile) > range || targetTile === opp.pos || targetTile === me.pos || tileBlocked(targetTile, me, opp)) {
                    ...
                    if (jutsuMethod === 'INSTANT_EFFECT' || jutsuMethod === 'AOE_SPIRAL') {
                        ...
                        const zoneTiles = jutsuMethod === 'AOE_SPIRAL'
                            ? (0, _aoe_js_1.filledDiskTiles)(targetTile, constants_js_1.SPIRAL_RADIUS, constants_js_1.GRID_W, constants_js_1.GRID_H)
            ...
            case 'weapon': {
                ...
                if ((0, grid_js_1.hexDistance)(me.pos, opp.pos) > weapRange) {
                ...
                const wWMult = (0, formulas_js_1.weatherMultiplier)(serverItem.weaponElement, weatherPositiveElement, weatherNegativeElement);
        ...
        if (result.status === 'done') {
            ...
            if (result.winner && result.winner !== 'draw') {
                try {
                    const kagePtr = await _storage_js_1.kv.get((0, _kage_settle_js_1.kageDuelKey)(result.battleId));
                    if (kagePtr?.village) {
                        ...
                        await (0, _kage_settle_js_1.recordPendingKageSettle)(kagePtr.village, result, kagePtr.challengeId);
                        await (0, _kage_settle_js_1.settleKageDuel)(kagePtr.village, result.battleId, Date.now(), { expectChallengeId: kagePtr.challengeId });
                    }
                }
                catch (err) {
