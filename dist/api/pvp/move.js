...
function jutsuLevelCapForLevel(level) {
    ...
    if (lvl >= 50)
        return JUTSU_LEVEL_CAP_JONIN;
    if (lvl >= 30)
        return JUTSU_LEVEL_CAP_CHUNIN;
    if (lvl >= 15)
        return JUTSU_LEVEL_CAP_GENIN;
...
function statCapForLevel(level) {
    const lvl = Math.max(1, Math.floor(Number(level) || 1));
    if (lvl >= 80)
        return STAT_CAP_SPECIAL_JONIN;
    if (lvl >= 50)
        return STAT_CAP_JONIN;
    if (lvl >= 30)
        return STAT_CAP_CHUNIN;
    if (lvl >= 15)
        return STAT_CAP_GENIN;
...
function homeTerrainMultiplier(self, jutsu) {
    const homeType = self.character.homeTerrainType;
...
function resolveBaseDamage(self, opponent, jutsu, wMult, biome, round, masteryLevel) {
    ...
    const baseDmg = Math.max(0, Math.floor(scaledEp * EP_MULTIPLIER * statFactor * wMult * tMult * hMult * bloodlineMult * itemDamageMult));
