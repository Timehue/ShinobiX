 ...
 */
function spiralTiles(center, radius, width, height) {
 ...
 */
function ringTiles(center, radius, width, height) {
    const r = clampRadius(radius);
    const out = [];
    if (r === 0)
        return [center];
    const n = width * height;
    for (let p = 0; p < n; p++) {
        if (hexDistance(center, p, width) === r)
            out.push(p);
    }
    return out;
}
 ...
 */
function filledDiskTiles(center, radius, width, height) {
    const r = clampRadius(radius);
    const n = width * height;
    const out = [];
    for (let p = 0; p < n; p++) {
