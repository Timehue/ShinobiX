...
exports.sealBaseRewardStamp = sealBaseRewardStamp;
...
function sealV2JutsuCosts(list, level, specialty) {
    if (!_combat_resources_js_1.COMBAT_RESOURCES_V2 || !Array.isArray(list))
        return list;
    return list.map((j) => {
        const jj = (j ?? {});
        return { ...jj, ...(0, _combat_resources_js_1.v2JutsuCosts)(jj, level, specialty) };
    });
}
...
function hydrateCharacterFromSave(saveCharacter, clientCharacter, save = null) {
    ...
    merged.maxChakra = pickClamped(saveCharacter.maxChakra, clientCharacter.maxChakra, 0, _combat_resources_js_1.COMBAT_RESOURCES_V2 ? _xp_engine_js_1.CHAKRA_CAP_V2 : 5000, 50);
    merged.maxStamina = pickClamped(saveCharacter.maxStamina, clientCharacter.maxStamina, 0, _combat_resources_js_1.COMBAT_RESOURCES_V2 ? _xp_engine_js_1.STAMINA_CAP_V2 : 5000, 50);
...
function terrainBuffStatToJutsuType(stat) {
    switch (stat) {
...
function sealBaseRewardStamp(opts) {
    if (!opts.baseRewards)
        return { stamp: {}, denied: false };
    const bothRealPlayers = opts.p1HasSave && opts.p2HasSave;
    if (!opts.isAdmin && !bothRealPlayers)
        return { stamp: {}, denied: true };
...
async function handler(req, res) {
    ...
    if (req.method === 'POST') {
        ...
        try {
            ...
            const deathsGateVerified = online_store_js_1.onlineStore.get(p1Norm)?.sector === DEATHS_GATE_SECTOR
                && online_store_js_1.onlineStore.get(p2Norm)?.sector === DEATHS_GATE_SECTOR;
            ...
            if (baseRewardDenied) {
                ...
                console.warn(`[pvp/session] base-reward request denied — opponent has no authoritative save `
            ...
            if (!isRankedSession) {
                ...
                if (Number.isFinite(secNum) && secNum > 0) {
                    try {
                        const territory = await _storage_js_1.kv.get(`world:territory:${secNum}`);
                        ...
                        if (ownerClan && buffType) {
                            const clanOf = (save) => String((save?.character ?? null)?.clan ?? '').trim();
                            if (clanOf(p1Save) === ownerClan)
                                p1HomeTerrain = buffType;
                            if (clanOf(p2Save) === ownerClan)
                                p2HomeTerrain = buffType;
                        }
                    }
