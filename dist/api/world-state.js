...
exports.activeVillageWarEnemiesOf = activeVillageWarEnemiesOf;
exports.listActiveVillageWars = listActiveVillageWars;
exports.captureSectorForVillage = captureSectorForVillage;
exports.seedHomeSectorOwnership = seedHomeSectorOwnership;
exports.applyMercVillageWarDamage = applyMercVillageWarDamage;
...
async function isSeatedKageOf(playerName, village) {
    ...
    try {
        ...
        const state = await _storage_js_1.kv.get((0, _kage_settle_js_1.kageKey)(village));
        const seated = (0, _utils_js_1.safeName)(String(state?.seatedKage ?? ''));
        return !!seated && seated === (0, _utils_js_1.safeName)(playerName);
    }
    catch {
        return false;
    }
...
async function activeVillageWarEnemiesOf(village) {
    const wars = await getByPrefix(VILLAGE_WAR_KEY_PREFIX);
    const out = [];
    for (const w of wars) {
        if (w.endedAt || !w.villages.includes(village))
            continue;
...
async function listActiveVillageWars() {
    const wars = await getByPrefix(VILLAGE_WAR_KEY_PREFIX);
    return wars
...
function applyWarDecay(war, now = Date.now()) {
    ...
    for (const v of war.villages) {
        ...
        (0, _utils_js_1.setSafeRecordValue)(newHp, v, Math.max(0, before - totalDamage));
    }
 ...
 *  a client outcome. */
async function applyMercVillageWarDamage(attackerVillage, enemyVillage, damage, now = Date.now()) {
    const warKey = `${VILLAGE_WAR_KEY_PREFIX}${villageWarId(attackerVillage, enemyVillage)}`;
    return (0, _lock_js_1.withKvLock)(warKey, async () => {
        let war = await _storage_js_1.kv.get(warKey);
        if (!war || war.endedAt)
            return null;
        ...
        if (war.endedAt || warIsPending(war))
            return null; // ended by decay, or pre-war window (HP frozen)
        if (!war.villages.includes(enemyVillage))
            return null;
        ...
        if (after !== before) {
            await _storage_js_1.kv.set(warKey, { ...war, hp: { ...war.hp, [enemyVillage]: after }, updatedAt: now });
        }
...
function warStandingKey(village) {
    return `${WAR_STANDING_PREFIX}${village.toLowerCase().replace(/[^a-z0-9]/g, '')}`;
}
function vnum(v) { const x = Number(v); return Number.isFinite(x) ? x : 0; }
async function bumpVillageStanding(village, result, now) {
...
async function settleVillageWar(war, now) {
    if (!war.endedAt || !war.winnerVillage)
        return;
    ...
    try {
        const didSettle = await (0, _lock_js_1.withKvLock)(k1, async () => (0, _lock_js_1.withKvLock)(k2, async () => {
            const placed = await _storage_js_1.kv.set(`war:settled:${war.id}`, { ts: now, winner, loser }, { nx: true, ex: 90 * 24 * 60 * 60 });
            if (!placed)
                return false; // already settled by a concurrent/earlier call
            const loserState = (await _storage_js_1.kv.get(loserKey)) ?? {};
            const winnerState = (await _storage_js_1.kv.get(winnerKey)) ?? {};
        ...
        await (0, _lock_js_1.withKvLock)(`${VILLAGE_WAR_KEY_PREFIX}${war.id}`, async () => {
            const fresh = await _storage_js_1.kv.get(`${VILLAGE_WAR_KEY_PREFIX}${war.id}`);
            if (!fresh || fresh.settled)
                return;
            await _storage_js_1.kv.set(`${VILLAGE_WAR_KEY_PREFIX}${war.id}`, { ...fresh, settled: true });
        }).catch(() => undefined);
    }
...
async function handler(req, res) {
    ...
    if (req.method === 'GET') {
        ...
        try {
            ...
            [territories, warsRaw, standingRows] = await (0, _proc_cache_js_1.cachedFor)('world-state:frame', 3_000, () => Promise.all([
        ...
        for (const w of wars) {
            if (w.endedAt && w.winnerVillage && !w.settled)
                void settleVillageWar(w, now);
        }
                ...
                if (!identity.admin) {
                    try {
                        ...
                        if (matchesClan || matchesVillage) {
                            const owned = (0, _territory_supply_js_1.resolveClaimedWarSupply)(prev, incomingTerritory, Date.now());
                    ...
                    if (!identity.admin) {
                        try {
                            if (isCreating) {
                                ...
                                await _storage_js_1.kv.set(`audit:village-war-declare:${normalizeVillageKey(actorVillage)}:${Date.now()}`, {
