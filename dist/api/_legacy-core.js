...
exports.TRIAL_VARIANT_COUNT = exports.legacyAcceptedKey = exports.legacyTrialKey = void 0;
...
exports.trialIntroFor = trialIntroFor;
exports.trialCompletionFor = trialCompletionFor;
...
const SECONDARY_KIND_FACTOR = { awaken: 0, bind: 1, prove: 1.25, mythic: 1.75 };
function trialObjectivesFor(def, kind, variant = 0) {
    ...
    const out = primary.map((t) => ({
        stat: t.stat,
        delta: Math.max(1, Math.round(t.delta * factor)),
    }));
    const secondaryFactor = RARITY_FACTOR[def.rarity] * SECONDARY_KIND_FACTOR[kind];
    const addSecondary = (o) => {
        if (out.some((existing) => existing.stat === o.stat))
            return;
        out.push({ stat: o.stat, delta: Math.max(1, Math.round(o.delta * secondaryFactor)) });
    };
    if (kind === 'bind' || kind === 'mythic')
        addSecondary(BIND_SECONDARIES[def.category]);
    if (kind === 'prove' || kind === 'mythic')
        addSecondary(PROVE_EXTRAS[def.category]);
    return out;
}
