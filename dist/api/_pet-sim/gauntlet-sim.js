...
exports.replayGauntlet = replayGauntlet;
/*
 ...
 */
const _gauntlet_pool_js_1 = require("./_gauntlet-pool.js");
const pet_synergies_js_1 = require("./pet-synergies.js");
const pet_board_sim_js_1 = require("./pet-board-sim.js");
...
const RARITY_COST = { standard: 3, rare: 5, legendary: 7, mythic: 9 };
...
function shopRaritiesForRound(round) {
    if (round <= 2)
        return ['standard', 'standard', 'standard', 'rare'];
    if (round <= 4)
        return ['standard', 'rare', 'rare', 'legendary'];
    if (round <= 6)
        return ['rare', 'rare', 'legendary', 'legendary'];
    if (round <= 8)
        return ['rare', 'legendary', 'legendary', 'mythic'];
    return ['legendary', 'legendary', 'mythic', 'mythic'];
}
function enemyRarityForRound(round) {
    if (round <= 2)
        return 'standard';
    if (round <= 5)
        return 'rare';
...
function enemySizeForRound(round) {
    if (round <= 2)
        return 2;
    if (round <= 4)
        return 3;
...
const GAUNTLET_ITEM_BY_ID = Object.fromEntries(GAUNTLET_ITEMS.map((d) => [d.id, d]));
function itemCost(def, owned) { return def.baseCost + def.step * Math.max(0, owned); }
const GAUNTLET_RELICS = [
...
const RELIC_BY_ID = Object.fromEntries(GAUNTLET_RELICS.map((d) => [d.id, d]));
function relicValorPerRound(relics) { return relics.reduce((s, id) => s + (RELIC_BY_ID[id]?.valorPerRound ?? 0), 0); }
function hasFreeReroll(relics) { return relics.some((id) => RELIC_BY_ID[id]?.freeReroll); }
function mergeDiscountFromRelics(relics) { return relics.reduce((s, id) => s + (RELIC_BY_ID[id]?.mergeDiscount ?? 0), 0); }
function boardModsFromRelics(relics) {
    const m = { shieldStartFrac: 0, reflectPct: 0, chainPct: 0, lifestealPct: 0, reviveCharges: 0, reviveHpFrac: 0 };
...
function mergeRelicStat(buffs, stat) {
    if (!stat)
        return buffs;
    return { atk: buffs.atk + (stat.atk ?? 0), def: buffs.def + (stat.def ?? 0), hp: buffs.hp + (stat.hp ?? 0), spd: buffs.spd + (stat.spd ?? 0) };
}
function instantiate(template, instanceN, statMult = 1) {
    const scale = (v, min) => Math.max(min, Math.round(v * statMult));
    return {
        ...template,
...
function rollShop(seed, round, rerolls) {
    ...
    return shopRaritiesForRound(round).slice(0, GAUNTLET_SHOP_SIZE).map((rarity) => {
        const tier = _gauntlet_pool_js_1.GAUNTLET_POOL_BY_RARITY[rarity] ?? [];
        const pet = tier.length ? tier[Math.floor(rng() * tier.length)] : _gauntlet_pool_js_1.GAUNTLET_POOL[0];
        return { pet, cost: RARITY_COST[pet.rarity] };
    });
}
function rollRelicShop(seed, round, rerolls, owned) {
...
function startGauntletRun(seed) {
    return {
...
function buyOffer(run, offerIndex) {
    ...
    if (run.valor < offer.cost)
        return run;
    if (run.roster.length >= GAUNTLET_ROSTER_CAP)
        return run;
...
function buyItem(run, itemId) {
    ...
    if (owned >= def.max)
        return run;
    if (itemId === 'mend' && run.hearts >= GAUNTLET_START_HEARTS)
        return run;
    const cost = itemCost(def, owned);
    if (run.valor < cost)
        return run;
    const next = { ...run, valor: run.valor - cost, itemsBought: { ...run.itemsBought, [itemId]: owned + 1 } };
    if (itemId === 'mend')
        next.hearts = Math.min(GAUNTLET_START_HEARTS, run.hearts + 1);
    else if (itemId === 'whetstone')
        next.buffs = { ...run.buffs, atk: run.buffs.atk + 0.08 };
    else if (itemId === 'bulwark')
        next.buffs = { ...run.buffs, def: run.buffs.def + 0.08 };
    else if (itemId === 'vigor')
        next.buffs = { ...run.buffs, hp: run.buffs.hp + 0.08 };
    return next;
}
function buyRelic(run, relicId) {
    if (run.status !== 'drafting')
        return run;
    if (!run.relicShop.includes(relicId) || run.relics.includes(relicId))
        return run;
    const def = RELIC_BY_ID[relicId];
    if (!def || run.valor < def.cost)
        return run;
    return {
        ...run,
...
function premiumUnlocked(run) { return run.roundsCleared >= GAUNTLET_PREMIUM_ROUND; }
function buyPremium(run, kind) {
    if (run.status !== 'drafting' || !premiumUnlocked(run))
        return run;
    if (kind === 'fateShard') {
        if (run.boughtFateShard || run.valor < GAUNTLET_SHARD_COST)
            return run;
        return { ...run, valor: run.valor - GAUNTLET_SHARD_COST, boughtFateShard: true };
    }
    if (run.boughtBoneCharm || run.valor < GAUNTLET_CHARM_COST)
        return run;
...
function applyGauntletBuffs(pets, buffs) {
    if (buffs.atk === 0 && buffs.def === 0 && buffs.hp === 0 && buffs.spd === 0)
        return pets;
    return pets.map((p) => ({
        ...p,
...
function setField(run, fieldIds) {
    if (run.status !== 'drafting')
        return run;
    const valid = fieldIds.filter((id) => run.roster.some((p) => p.id === id)).slice(0, GAUNTLET_FIELD_CAP);
    return { ...run, fieldIds: valid };
}
function fieldedPets(run) {
    return run.fieldIds.map((id) => run.roster.find((p) => p.id === id)).filter((p) => !!p);
}
function enemySquadForRound(run) {
    ...
    const tier = _gauntlet_pool_js_1.GAUNTLET_POOL_BY_RARITY[rarity] ?? _gauntlet_pool_js_1.GAUNTLET_POOL_BY_RARITY.standard;
    return pickN(tier, size, rng).map((tpl, i) => instantiate(tpl, 100000 + run.round * 10 + i, mult));
}
function beginFight(run) {
    if (run.status !== 'drafting' || run.fieldIds.length === 0)
        return run;
    return { ...run, status: 'fighting' };
}
function applyRoundResult(run, won) {
    ...
    if (hearts <= 0)
        return { ...run, status: 'lost', hearts: 0 };
    if (run.round >= run.maxRounds)
        return { ...run, status: 'won', hearts };
...
function fightSeed(run) { return (run.seed * 7919 + run.round * 104729) >>> 0; }
function enemyUnits(pets) {
    const rows = [[], [], []];
    ...
    rows.forEach((rowPets, row) => {
        const start = Math.max(0, Math.round((pet_board_sim_js_1.BOARD_COLS - rowPets.length) / 2));
        rowPets.forEach((pet, i) => units.push({ pet, row, col: Math.min(pet_board_sim_js_1.BOARD_COLS - 1, start + i) }));
    });
    return units;
}
function cell(c) {
    const row = Number.isInteger(c?.row) ? Math.max(0, Math.min(pet_board_sim_js_1.BOARD_ROWS_PER_SIDE - 1, c.row)) : 2;
    const col = Number.isInteger(c?.col) ? Math.max(0, Math.min(pet_board_sim_js_1.BOARD_COLS - 1, c.col)) : 0;
    return { row, col };
}
/**
 ...
 */
function replayGauntlet(seed, transcript) {
    let run = startGauntletRun(seed >>> 0);
    const actions = Array.isArray(transcript) ? transcript : [];
    for (const a of actions) {
        ...
        switch (a.k) {
            ...
            case 'fight': {
                ...
                if (started.status !== 'fighting') {
                    run = started;
                    break;
                }
                const squad = (0, pet_synergies_js_1.applySynergiesToSquad)(applyGauntletBuffs(fieldedPets(started), started.buffs));
                const place = Array.isArray(a.place) ? a.place : [];
                const playerUnits = squad.map((pet, i) => ({ pet, ...cell(place[i]) }));
                const enemy = enemySquadForRound(started);
                const result = (0, pet_board_sim_js_1.runPetGridBattle)(playerUnits, enemyUnits(enemy), fightSeed(started), { playerMods: boardModsFromRelics(started.relics) });
