...
exports.CINEMATIC_ELEMENT_PROFILES = exports.DUEL_COVER_NODES = void 0;
exports.runPetDuelCinematic = runPetDuelCinematic;
exports.runPetPartyDuelCinematic = runPetPartyDuelCinematic;
exports.petCinematicArchetype = petCinematicArchetype;
const pet_arena_walkmask_js_1 = require("./pet-arena-walkmask.js");
const petAccuracyEnabled = () => false; // server: accuracy is passed in explicitly
const pet_config_js_1 = require("./pet-config.js");
const pet_duel_sim_js_1 = require("./pet-duel-sim.js");
...
const CAP_TICKS = pet_duel_sim_js_1.DUEL_TPS * 75; // one complete cinematic fight, decided by one KO
...
const TTK_HP = 3.0;
const LATE_T = pet_duel_sim_js_1.DUEL_TPS * 34; // final act: preserve the full bar, but prevent defensive score decisions
const LATE_RAMP = pet_duel_sim_js_1.DUEL_TPS * 7;
...
const STAM_MAX = 100;
const STAM_REGEN = 22 / pet_duel_sim_js_1.DUEL_TPS;
const COST_BASIC = 12;
const COST_DODGE = 20;
const CRIT_CHANCE = 0.12;
const DMG_SCALE = 1.5;
const TARGET_LOCK_TICKS = Math.round(pet_duel_sim_js_1.DUEL_TPS * 1.4);
...
const GCOLS = pet_arena_walkmask_js_1.WALK_COLS, GROWS = pet_arena_walkmask_js_1.WALK_ROWS;
const CELL_X = (pet_duel_sim_js_1.ARENA_X * 2) / GCOLS;
const CELL_Y = (pet_duel_sim_js_1.ARENA_Y * 2) / GROWS;
const cellCol = (x) => clamp(Math.floor((x + pet_duel_sim_js_1.ARENA_X) / CELL_X), 0, GCOLS - 1);
const cellRow = (y) => clamp(Math.floor((y + pet_duel_sim_js_1.ARENA_Y) / CELL_Y), 0, GROWS - 1);
const cellCenter = (c, r) => [(c + 0.5) * CELL_X - pet_duel_sim_js_1.ARENA_X, (r + 0.5) * CELL_Y - pet_duel_sim_js_1.ARENA_Y];
...
const arenaFloorCell = (c, r) => {
    if (c < 0 || r < 0 || c >= GCOLS || r >= GROWS)
        return false;
    const [x, y] = cellCenter(c, r), ax = Math.abs(x), ay = Math.abs(y);
    return ax <= pet_duel_sim_js_1.ARENA_X - 0.35 && ay <= pet_duel_sim_js_1.ARENA_Y - 0.35 && !(ax > pet_duel_sim_js_1.ARENA_X - 2.1 && ay > pet_duel_sim_js_1.ARENA_Y - 1.65);
};
...
exports.DUEL_COVER_NODES = Object.freeze([]);
const DUEL_COVER_MASK = (() => {
    const blocked = new Uint8Array(GCOLS * GROWS);
    for (let r = 0; r < GROWS; r++)
        for (let c = 0; c < GCOLS; c++) {
            const cx = (c + 0.5) * CELL_X - pet_duel_sim_js_1.ARENA_X, cy = (r + 0.5) * CELL_Y - pet_duel_sim_js_1.ARENA_Y;
            for (const cover of exports.DUEL_COVER_NODES) {
                const dx = cx - cover.x, dy = cy - cover.y;
                if (dx * dx + dy * dy < cover.radius * cover.radius) {
                    blocked[r * GCOLS + c] = 1;
                    break;
                }
            }
...
let _cinematicInitiativeTeam = "player";
const WALL_TICKS = Math.round(pet_duel_sim_js_1.DUEL_TPS * 0.85); // how long a wall BLOCKS (short → a beat, not a big defensive advantage)
const WALL_PENALTY_TICKS = Math.round(pet_duel_sim_js_1.DUEL_TPS * 3.0); // caster's damage halved this long after raising a wall (the wall's cost)
const cellBlockedByWall = (c, r) => {
    if (SIM_WALLS.length === 0)
        return false;
    const cx = (c + 0.5) * CELL_X - pet_duel_sim_js_1.ARENA_X, cy = (r + 0.5) * CELL_Y - pet_duel_sim_js_1.ARENA_Y;
    for (const w of SIM_WALLS) {
...
const cellBlockedByArenaCover = (c, r) => DUEL_COVER_MASK[r * GCOLS + c] === 1;
const cellWalkable = (c, r) => arenaFloorCell(c, r)
    && !cellBlockedByArenaCover(c, r) && !cellBlockedByWall(c, r);
function walkableAt(x, y) {
    if (x < -pet_duel_sim_js_1.ARENA_X || x > pet_duel_sim_js_1.ARENA_X || y < -pet_duel_sim_js_1.ARENA_Y || y > pet_duel_sim_js_1.ARENA_Y)
        return false;
    return cellWalkable(cellCol(x), cellRow(y));
}
function sightWalkableAt(x, y) {
    if (x < -pet_duel_sim_js_1.ARENA_X || x > pet_duel_sim_js_1.ARENA_X || y < -pet_duel_sim_js_1.ARENA_Y || y > pet_duel_sim_js_1.ARENA_Y)
        return false;
...
function arenaCoverAt(x, y, padding = 0) {
    for (const cover of exports.DUEL_COVER_NODES) {
...
function hasLineOfSight(ax, ay, bx, by) {
    ...
    for (let i = 1; i < steps; i++) {
        const tt = i / steps;
        if (!sightWalkableAt(ax + dx * tt, ay + dy * tt))
            return false;
    }
    return true;
}
...
function bfsNextStep(fc, fr, tc, tr) {
    ...
    while (head < queue.length) {
        ...
        if (cc === fc && cr === fr) {
            const nxt = came.get(cur);
            return nxt === undefined || nxt < 0 ? null : [nxt % GCOLS, (nxt - (nxt % GCOLS)) / GCOLS];
        }
...
function abilityClass(kind) {
    if (kind === "heal" || kind === "buff" || kind === "shield" || kind === "barrier" || kind === "absorb" || kind === "haste")
        return "support";
    if (kind === "damage" || kind === "crush" || kind === "lifesteal")
        return "melee";
    return "ranged";
}
function buildAbility(j) {
    const cls = abilityClass(j.kind);
    const base = Math.max(Math.round(pet_duel_sim_js_1.DUEL_TPS * 0.8), Math.round((j.cooldown || 0) * 1.2 * pet_duel_sim_js_1.DUEL_TPS));
    return {
        name: j.name, kind: j.kind, accuracy: pet_duel_sim_js_1.KIND_ACCURACY[j.kind] ?? 100, cls, isMove: j.kind === "move",
        power: Math.max(1, j.power || 1), signature: !!j.signature, aoe: !!j.aoe,
        range: cls === "support" ? 999 : cls === "ranged" ? RANGED_RANGE : MELEE_RANGE,
        castTicks: Math.max(4, Math.round(pet_duel_sim_js_1.DUEL_TPS * (j.signature ? 0.34 : cls === "support" ? 0.21 : 0.2))),
        cdTicks: base + (j.signature ? Math.round(pet_duel_sim_js_1.DUEL_TPS * 1.5) : 0),
        cdLeft: j.signature
            ? Math.round(pet_duel_sim_js_1.DUEL_TPS * 2.5)
            : (j.kind === "buff" || j.kind === "haste")
                ...
                ? Math.round(pet_duel_sim_js_1.DUEL_TPS * 0.36)
                : Math.round(pet_duel_sim_js_1.DUEL_TPS * 0.5),
...
function petCinematicAbilities(pet) {
...
function statusTicks(ab, rounds) {
    if (rounds && rounds > 0)
        return Math.round(pet_duel_sim_js_1.DUEL_TPS * rounds * 0.8);
    ...
    return Math.round(pet_duel_sim_js_1.DUEL_TPS * (setup ? 2.6 : 1.0) * (ab.signature ? 1.4 : 1));
}
...
function buffTicks(ab) {
    return Math.round(pet_duel_sim_js_1.DUEL_TPS * BUFF_STANCE_SECS * (ab.signature ? 1.4 : 1.0));
}
const emptyStatuses = () => ({ burnLeft: 0, burnDmg: 0, halfHeal: false, stunLeft: 0, slowLeft: 0, hasteLeft: 0, rootLeft: 0, shieldHp: 0, buffLeft: 0, buffMag: 0, marked: false, tauntById: null, wallPenaltyLeft: 0 });
...
exports.CINEMATIC_ELEMENT_PROFILES = Object.freeze({
...
function elementProfile(element) {
    const key = String(element ?? "None").toLowerCase();
    if (key === "fire")
        return exports.CINEMATIC_ELEMENT_PROFILES.Fire;
    if (key === "water")
        return exports.CINEMATIC_ELEMENT_PROFILES.Water;
    if (key === "lightning")
        return exports.CINEMATIC_ELEMENT_PROFILES.Lightning;
    if (key === "earth")
        return exports.CINEMATIC_ELEMENT_PROFILES.Earth;
    if (key === "wind")
        return exports.CINEMATIC_ELEMENT_PROFILES.Wind;
    return exports.CINEMATIC_ELEMENT_PROFILES.None;
}
...
function classifyArchetype(pet, abilities) {
    ...
    if (sub === "support" || role === "sage" || (hasSupport && !glass))
        return "support";
    if (sub === "kite" || (hasRanged && !hasMelee) || (hasRanged && fast && glass))
        return "kiter";
    if (sub === "tank" || role === "defender" || tanky)
        return "defender";
    if (sub === "assassin" || role === "assassin" || (fast && glass && hasMelee))
        return "rusher";
    if (sub === "striker" || sub === "bruiser" || role === "tracker")
        return "brawler";
    return "balanced";
}
...
function deriveStyle(pet, abilities, oppElement, itemsOn) {
    ...
    let aggression, retreatHp, dodgeBias, orbitStrong, rangedPref;
    switch (arche) {
        case "rusher":
            aggression = 0.85;
            retreatHp = 0.18;
        ...
        default:
            ...
            retreatHp = 0.28;
            dodgeBias = 0.08;
            orbitStrong = hasRanged;
            rangedPref = hasRanged && !hasMelee;
            break;
    }
    ...
    const adv = (0, pet_duel_sim_js_1.elementMult)(pet.element, oppElement);
    if (adv > 1) {
        aggression = clamp(aggression + 0.1, 0, 1);
        retreatHp = Math.max(0.1, retreatHp - 0.04);
    }
    else if (adv < 1) {
        aggression = clamp(aggression - 0.1, 0, 1);
        dodgeBias += 0.04;
    ...
    // TRAIT flavor.
    if (trait === "Aggressive")
        aggression = clamp(aggression + 0.12, 0, 1);
    if (trait === "Battleborn")
        aggression = clamp(aggression + 0.08, 0, 1);
    if (trait === "Guardian") {
        retreatHp = Math.max(0.1, retreatHp - 0.05);
    }
    if (trait === "Swift") {
        dodgeBias += 0.08;
        orbitStrong = true;
    ...
    if (itemsOn) {
        const ch = (0, pet_config_js_1.petConsumableCharges)(pet);
        if (ch.endure > 0)
            aggression = clamp(aggression + 0.1, 0, 1);
        if (ch.thorns > 0)
            aggression = clamp(aggression + 0.05, 0, 1);
        if (ch.dodge > 0)
            dodgeBias += 0.05;
    }
    ...
    dodgeBias += ep.dodge;
    if (String(pet.element ?? "").toLowerCase() === "wind")
        orbitStrong = true;
    return {
...
function buildFighter(pet, team, slot, x, y, oppElement, atkMult, hpMult, reviveOnce, applyItems) {
    ...
    const baseMoveSpeed = clamp(4.35 + speed * 0.035, 4.35, 9.4) / pet_duel_sim_js_1.DUEL_TPS;
    const abilities = petCinematicAbilities(gp);
    const sigAb = abilities.find((a) => a.signature);
    if (sigAb) {
        sigAb.cdTicks = Math.max(sigAb.cdTicks, Math.round(pet_duel_sim_js_1.DUEL_TPS * 9));
        sigAb.cdLeft = Math.round(pet_duel_sim_js_1.DUEL_TPS * 4.5);
    }
    ...
    const ch = applyItems ? (0, pet_config_js_1.petConsumableCharges)(gp) : null;
    if (applyItems)
        statuses.shieldHp = (0, pet_config_js_1.petGearStartShield)(gp);
    return {
        ...
        basicCdLeft: 0, basicCdT: Math.round(pet_duel_sim_js_1.DUEL_TPS * 0.5),
        windT: Math.max(2, Math.round(pet_duel_sim_js_1.DUEL_TPS * clamp(0.42 - speed * 0.0012, 0.16, 0.42) * style.windMult)), // snappy vs telegraphed
        recovT: Math.max(2, Math.round(pet_duel_sim_js_1.DUEL_TPS * clamp(0.46 - speed * 0.0010, 0.20, 0.46) * style.recovMult)), // relentless vs long punish window
        staggerT: Math.round(pet_duel_sim_js_1.DUEL_TPS * 0.35), dashT: 7, dodgeT: 6,
...
// ── Targeting ────────────────────────────────────────────────────────────────────
function pickTarget(f, fighters) {
    ...
    if (locked && f.targetLockLeft > 0)
        return locked;
    let best = null, bestKey = Infinity;
    for (const g of fighters) {
        if (g.team === f.team || g.hp <= 0)
            continue;
        ...
        if (f.style.arche === "rusher")
            key = distance * 0.045 + hpFrac * 1.2 - (open ? 0.65 : 0);
        else if (f.style.arche === "defender")
            key = distance * 0.13 + hpFrac * 0.25 - (attackingAlly ? 0.85 : 0);
        else if (f.style.arche === "support")
            key = distance * 0.1 + hpFrac * 0.35 - (attackingAlly ? 0.65 : 0);
        else if (f.style.arche === "kiter")
            key = distance * 0.06 + hpFrac * 0.75 - (open ? 0.25 : 0);
        if (g.slot === f.slot)
            key -= 0.16;
        if (g.id === f.targetId)
            key -= 0.22;
        const matchup = (0, pet_duel_sim_js_1.elementMult)(f.element, g.element);
        if (matchup > 1)
            key -= 0.12;
        else if (matchup < 1)
            key += 0.08;
        if (key < bestKey || (key === bestKey && best && g.id < best.id)) {
            bestKey = key;
            best = g;
        }
    }
...
function pickAlly(f, fighters) {
    let best = f, bestFrac = f.hp / f.maxHp;
    for (const g of fighters) {
        if (g.team !== f.team || g.hp <= 0)
            continue;
        const frac = g.hp / g.maxHp;
        if (frac < bestFrac || (frac === bestFrac && g.id < best.id)) {
            bestFrac = frac;
            best = g;
        }
    }
...
const teamAlive = (fighters, team) => fighters.some((f) => f.team === team && f.hp > 0);
function setIntent(f, state, desiredRange, plan, reason) {
    f.aiState = state;
...
function elementalPayoff(att, tgt) {
    const el = String(att.element ?? "").toLowerCase();
    if (el === "fire") {
        if (tgt.statuses.burnLeft > 0)
            return { mult: 1.12, combo: "Inferno Pressure" };
        if (tgt.statuses.rootLeft > 0 || tgt.statuses.slowLeft > 0)
            return { mult: 1.08, combo: "Ignition Trap" };
    }
    if (el === "water" && (tgt.statuses.slowLeft > 0 || tgt.statuses.stunLeft > 0))
        return { mult: 1.08, combo: "Undertow Punish" };
    if (el === "lightning") {
        if (tgt.statuses.marked)
            return { mult: 1, combo: "Charged Burst" };
        if (tgt.statuses.slowLeft > 0 || tgt.statuses.stunLeft > 0)
            return { mult: 1.18, combo: "Conductive Surge" };
    }
    if (el === "earth" && (att.statuses.shieldHp > 0 || att.statuses.wallPenaltyLeft > 0))
        return { mult: 1.08, combo: "Fortified Counter" };
    if (el === "wind") {
        if (tgt.statuses.burnLeft > 0)
            return { mult: 1.10, combo: "Firestorm Chain" };
        if (tgt.statuses.rootLeft > 0 || tgt.statuses.slowLeft > 0)
            return { mult: 1.10, combo: "Gale Extension" };
    }
    return { mult: 1 };
}
function applyDamage(att, tgt, ab, rng, t, events, viaProjectile) {
    if (tgt.hp <= 0)
        return;
    ...
    const matchup = (0, pet_duel_sim_js_1.elementMult)(att.element, tgt.element);
    ...
    mult *= elemental.mult;
    if (att.statuses.wallPenaltyLeft > 0)
        mult *= 0.5; // barrier caster's OFFENSE halved while its wall stands (the visible cost)
    if (tgt.statuses.wallPenaltyLeft > 0)
        mult *= 1.7; // …and it's EXPOSED — takes extra damage — which is what actually offsets a tanky pet's outlast-via-wall advantage
    if (tgt.statuses.marked) {
        mult *= 1.4;
        tgt.statuses.marked = false;
    }
    if (att.itemsOn)
        mult *= (0, pet_config_js_1.petGearExecuteMult)(att.pet, tgt.hp, tgt.maxHp);
    const mitigation = clamp(1 - tgt.def * 0.0012, 0.35, 1);
    if (t > LATE_T)
        mult *= 1 + (t - LATE_T) / LATE_RAMP;
    const base = att.atk * DMG_SCALE * powerScale;
    let dmg = Math.max(1, Math.round(base * mult * mitigation));
    if (tgt.itemsOn) {
        dmg = Math.max(1, Math.round(dmg * (0, pet_config_js_1.petGearLastStandMult)(tgt.pet, tgt.hp, tgt.maxHp)));
    ...
    if (ab)
        applyOnHit(att, tgt, ab);
    if (ab && ab.kind === "lifesteal" && att.hp > 0)
        att.hp = Math.min(att.maxHp, att.hp + Math.round(dmg * 0.5));
    if (dmg > 0) {
        ...
        if (att.itemsOn && !ab) {
            const dot = (0, pet_config_js_1.petGearDotOnHit)(att.pet);
            if (dot) {
                tgt.statuses.burnLeft = Math.max(tgt.statuses.burnLeft, Math.round(pet_duel_sim_js_1.DUEL_TPS * dot.rounds * 0.8));
                tgt.statuses.burnDmg = Math.max(tgt.statuses.burnDmg, dot.damage);
            }
            const heal = (0, pet_config_js_1.petGearLifestealHeal)(att.pet, dmg);
            if (heal > 0 && att.hp > 0)
                att.hp = Math.min(att.maxHp, att.hp + heal); // never heal a corpse alive (dead owner's in-flight poke)
        }
        if (tgt.itemsOn && tgt.cLifelinePct > 0 && tgt.hp > 0 && (tgt.hp / tgt.maxHp) * 100 < pet_config_js_1.PET_CONSUMABLE_LIFELINE_THRESHOLD_PCT) {
    ...
    if (d > 1e-6) {
        const [nx, ny] = snapPos(tgt.x + (dx / d) * kb, tgt.y + (dy / d) * kb);
        tgt.x = nx;
        tgt.y = ny;
    }
    if (ab && ab.kind === "pull" && d > 1e-6) {
        const [nx, ny] = snapPos(tgt.x - (dx / d) * 1.2, tgt.y - (dy / d) * 1.2);
        tgt.x = nx;
        tgt.y = ny;
    }
    if ((tgt.state === "idle" || tgt.state === "dash" || tgt.state === "windup") && tgt.statuses.stunLeft <= 0) {
        if (tgt.lungeAbIdx > -2)
            clearLunge(tgt); // staggered mid-pounce → drop the dive so it can't re-resolve for free on the next dash
        tgt.state = "stagger";
        tgt.stateLeft = att.staggerT;
        events.push({ t, type: "stagger", side: tgt.team, actorId: tgt.id });
    }
}
function applyOnHit(att, tgt, ab) {
    const s = tgt.statuses;
    const dur = statusTicks(ab);
    switch (ab.kind) {
        case "burn":
        case "dot":
            s.burnLeft = Math.max(s.burnLeft, dur);
            s.burnDmg = Math.max(s.burnDmg, Math.max(1, Math.round(att.atk * 0.12 * (ab.power / 100))));
            break;
        case "wound":
            s.burnLeft = Math.max(s.burnLeft, dur);
            s.burnDmg = Math.max(s.burnDmg, Math.max(1, Math.round(att.atk * 0.1)));
            s.halfHeal = true;
            break;
        case "freeze":
        case "stun":
        case "confuse":
            s.stunLeft = Math.max(s.stunLeft, ab.kind === "stun" ? dur : Math.round(dur * 0.7));
            break;
        case "slow":
        ...
        case "crush":
        case "debuff":
            s.buffLeft = Math.max(s.buffLeft, buffTicks(ab));
            s.buffMag = Math.min(s.buffMag, -0.25);
            break;
        case "movelock":
            s.rootLeft = Math.max(s.rootLeft, dur);
            break;
        case "taunt":
            s.tauntById = att.id;
            break;
...
function castSupport(f, ab, fighters, t, events) {
    ...
    if (ab.kind === "heal") {
        ...
        if (ab.kind === "barrier") {
            ...
            for (const g of fighters) {
                if (g.team === f.team || g.hp <= 0)
                    continue;
                const dd = (g.x - f.x) * (g.x - f.x) + (g.y - f.y) * (g.y - f.y);
                if (dd < bd) {
                    bd = dd;
                    foe = g;
                }
        ...
        f.statuses.buffLeft = buffTicks(ab);
        f.statuses.buffMag = Math.max(f.statuses.buffMag, 0.25);
...
// ── Casting ──────────────────────────────────────────────────────────────────────
function beginCast(f, idx, targetId, t, events) {
    f.pendingIdx = idx;
    ...
    if (idx >= 0 && f.abilities[idx].signature)
        events.push({ t, type: "ultimate", side: f.team, actorId: f.id, move: f.abilities[idx].name, targetId });
    else if (idx >= 0 && f.abilities[idx].cls === "support")
        events.push({ t, type: "cast", side: f.team, actorId: f.id, kind: f.abilities[idx].kind, move: f.abilities[idx].name });
    else
        events.push({ t, type: "windup", side: f.team, actorId: f.id, kind: idx >= 0 ? f.abilities[idx].kind : "damage", move: idx >= 0 ? f.abilities[idx].name : undefined, targetId });
}
function payAbilityCost(f, ab) {
    if (ab) {
...
function clearLunge(f) { f.lungeAbIdx = -2; f.lungeTgtId = null; f.lungeStuck = 0; }
function routeTravelSq(f, goal) {
    const dx = goal[0] - f.x, dy = goal[1] - f.y;
    return dx * dx + dy * dy;
}
function outerArenaDestination(f, e, retreat) {
    ...
    for (let attempt = 0; attempt < ROUTE_ANCHORS.length; attempt++) {
        ...
        if (!retreat && travelSq >= ROUTE_MIN_TRAVEL * ROUTE_MIN_TRAVEL)
            return candidate;
    }
    return fallback;
}
function coverArenaDestination(f, e, flank) {
    const lead = f.team === "player" ? 0 : 1;
    let best = outerArenaDestination(f, e, false);
    let bestScore = -1e9;
    for (let attempt = 0; attempt < exports.DUEL_COVER_NODES.length; attempt++) {
        const cover = exports.DUEL_COVER_NODES[(f.spacingBeat + lead + attempt) % exports.DUEL_COVER_NODES.length];
...
function assignArenaDestination(f, e) {
    const spacingPhrase = [-0.6, 1.25, 0.15, 1.85];
    const nextBeat = f.spacingBeat + 1;
    const wounded = f.hp / f.maxHp < f.style.retreatHp;
    let intent;
    if (wounded && f.style.arche !== "rusher" && f.style.arche !== "brawler")
        intent = "retreat";
    else if (f.style.rangedPref || f.style.arche === "support")
        intent = nextBeat % 4 === 1 ? "cross" : nextBeat % 4 === 3 ? "flank" : "cover";
    else
        intent = nextBeat % 3 === 0 ? "cross" : "flank";
    let chosen;
    if (!e)
        chosen = outerArenaDestination(f, null, false);
    else if (intent === "cover")
        chosen = coverArenaDestination(f, e, false);
    else if (intent === "flank")
        chosen = coverArenaDestination(f, e, true);
    else
        chosen = outerArenaDestination(f, e, intent === "retreat");
    if (routeTravelSq(f, chosen) < ROUTE_MIN_TRAVEL * ROUTE_MIN_TRAVEL)
        chosen = outerArenaDestination(f, e, intent === "retreat");
...
function waypointToward(f, goalX, goalY) {
...
function routeWaypoint(f) {
    return waypointToward(f, f.routeX, f.routeY);
}
function beginReposition(f, ticks, e = null) {
    ...
    if (f.reposLeft <= 0) {
        ...
        const travelTicks = clamp(Math.round(travel / Math.max(1e-4, f.maxSpeed * 0.86)), Math.round(pet_duel_sim_js_1.DUEL_TPS * 1.15), Math.round(pet_duel_sim_js_1.DUEL_TPS * 3.0));
        ticks = Math.max(ticks, travelTicks);
    }
    f.reposLeft = Math.max(f.reposLeft, Math.max(1, ticks));
}
function beginDodgeRetreat(f, e) {
    if (!e) {
        beginReposition(f, Math.round(pet_duel_sim_js_1.DUEL_TPS * 0.8), null);
        return;
    }
    ...
    if (!goal)
        goal = outerArenaDestination(f, e, true);
    f.spacingBeat++;
    f.spacingOffset = 1.85;
    f.routeX = goal[0];
    f.routeY = goal[1];
    f.routeActive = true;
    f.routeIntent = "retreat";
    ...
    f.reposLeft = Math.max(f.reposLeft, clamp(Math.round(travel / Math.max(1e-4, f.maxSpeed)), Math.round(pet_duel_sim_js_1.DUEL_TPS * 0.72), Math.round(pet_duel_sim_js_1.DUEL_TPS * 1.45)));
}
...
function resolveMeleeContact(f, ab, tgtId, fighters, rng, t, events, accuracyEnabled) {
    ...
    if (accuracyEnabled && ab && accRoll >= ab.accuracy / 100) {
    ...
    for (const tgt of hitList) {
        ...
        if (tgt.hp > 0 && d <= range + 0.5 && facingOK) {
            applyDamage(f, tgt, ab, rng, t, events, false);
            landed = true;
        }
    }
    if (!landed)
...
function resolveCast(f, fighters, projectiles, nextProjId, rng, t, events, accuracyEnabled) {
    ...
    if (ab && ab.cls === "ranged") {
        const targets = ab.aoe ? fighters.filter((g) => g.team !== f.team && g.hp > 0) : [fighters.find((g) => g.id === f.pendingTargetId && g.hp > 0)].filter(Boolean);
        for (const tgt of targets)
            projectiles.push({ id: nextProjId.n++, ownerId: f.id, team: f.team, targetId: tgt.id, abilityIdx: idx, x: f.x, y: f.y, speed: 0.56, ttl: Math.round(pet_duel_sim_js_1.DUEL_TPS * 3), element: f.element, kind: ab.kind });
    ...
    if (!ab && f.basicRanged) {
        ...
        if (tgt) {
            projectiles.push({ id: nextProjId.n++, ownerId: f.id, team: f.team, targetId: tgt.id, abilityIdx: -1, x: f.x, y: f.y, speed: 0.56, ttl: Math.round(pet_duel_sim_js_1.DUEL_TPS * 3), element: f.element, kind: "damage" });
...
function stepProjectiles(fighters, projectiles, rng, t, events) {
...
function effMoveSpeed(f) {
    let s = f.maxSpeed;
    if (f.statuses.slowLeft > 0)
        s *= 0.6;
    if (f.statuses.hasteLeft > 0)
        s *= 1.35;
    return s;
}
function followRouteWaypoint(f, e, goal, sprint = false) {
...
function writeMap(map, dx, dy, value, sharp) {
    for (let i = 0; i < N; i++) {
...
function steer(f, e, fighters, rStar, routeGoal, repositioning = false) {
    for (let i = 0; i < N; i++) {
        _interest[i] = 0;
        _danger[i] = 0;
    }
    ...
    if (routeGoal) {
        ...
        if (ev > 0.025) {
            ...
            writeMap(_interest, tx, ty, INT_CLOSE, false);
    }
    else if (d < rStar - BAND_H) {
        writeMap(_interest, -tx, -ty, INT_BACK, false);
    }
    else
        writeMap(_interest, tx, ty, 0.28, false);
    ...
    const edge = Math.max(Math.abs(f.x) / pet_duel_sim_js_1.ARENA_X, Math.abs(f.y) / pet_duel_sim_js_1.ARENA_Y);
    if (cl > 1e-3 && edge > 0.84)
        writeMap(_interest, -f.x / cl, -f.y / cl, EDGE_RETURN_BIAS * clamp((edge - 0.84) / 0.16, 0, 1), false);
    ...
    if (f.style.rangedPref && d < rStar - BAND_H)
        writeMap(_danger, tx, ty, DANGER_BUBBLE, false);
    ...
    if (!walkableAt(f.x + probe, f.y))
        writeMap(_danger, 1, 0, DANGER_WALL, false);
    if (!walkableAt(f.x - probe, f.y))
        writeMap(_danger, -1, 0, DANGER_WALL, false);
    if (!walkableAt(f.x, f.y + probe))
        writeMap(_danger, 0, 1, DANGER_WALL, false);
    if (!walkableAt(f.x, f.y - probe))
        writeMap(_danger, 0, -1, DANGER_WALL, false);
    // Ally separation (2v2) — don't stack on a teammate.
    for (const g of fighters) {
        if (g === f || g.team !== f.team || g.hp <= 0)
            continue;
        const gx = g.x - f.x, gy = g.y - f.y, gd = Math.sqrt(gx * gx + gy * gy);
        if (gd < 2.0 && gd > 1e-3)
            writeMap(_danger, gx / gd, gy / gd, 0.5 * (1 - gd / 2.0), false);
    }
    blur(_interest);
    blur(_danger);
    ...
    if (vlen > 1e-4) {
        let bestDot = -2;
        for (let i = 0; i < N; i++) {
            const dot = SLOT_X[i] * (f.vx / vlen) + SLOT_Y[i] * (f.vy / vlen);
            if (dot > bestDot) {
                bestDot = dot;
                curSlot = i;
            }
    ...
    if (best < 0) {
        best = 0;
        for (let i = 1; i < N; i++)
            if (_danger[i] < _danger[best])
                best = i;
    } // all dangerous → least-bad
    ...
    if (repositioning)
        spd = Math.max(spd, effMoveSpeed(f) * 0.86);
    ...
    if (!routeGoal && !repositioning) {
        const bandErr = Math.abs(d - rStar);
        if (bandErr < SLOW_RADIUS)
            spd *= Math.max(0.15, bandErr / SLOW_RADIUS);
    }
    ...
    if (slen > f.maxForce && slen > 1e-6) {
        sx = (sx / slen) * f.maxForce;
        sy = (sy / slen) * f.maxForce;
    }
    f.vx += sx;
    f.vy += sy;
...
function bestOffensive(f, e, forceSig) {
    ...
    for (let i = 0; i < f.abilities.length; i++) {
        ...
        if (a.cls === "support" || a.isMove || a.cdLeft > 0 || f.stamina < a.cost)
            continue; // a "move" ability is a reposition, never an attack
        if (a.signature) {
            sigIdx = i;
            continue;
        }
    ...
    if (sigIdx >= 0) {
        ...
        if (forceSig || foeFrac < 0.5 || foeOpen || !nonSigReady)
            return sigIdx;
    }
    return best;
}
function readyMobility(f) {
    for (let i = 0; i < f.abilities.length; i++) {
        const a = f.abilities[i];
        if (a.isMove && a.cdLeft <= 0 && f.stamina >= a.cost)
            return i;
    }
    return -1;
}
function maneuverName(f) {
...
function beginManeuver(f, e, idx, t, events) {
    const ab = f.abilities[idx];
    if (!f.routeActive || Math.hypot(f.routeX - f.x, f.routeY - f.y) < 3.2)
        assignArenaDestination(f, e);
    ...
    f.moveDx = mx;
    f.moveDy = my;
    f.maneuverGoalX = f.routeX;
    f.maneuverGoalY = f.routeY;
    f.faceX = mx;
    f.faceY = my;
    f.vx = 0;
    f.vy = 0;
    ...
    f.maneuverTotal = clamp(Math.round(d / Math.max(1e-4, f.maxSpeed * 2.55)), Math.round(pet_duel_sim_js_1.DUEL_TPS * 0.34), Math.round(pet_duel_sim_js_1.DUEL_TPS * 0.6));
...
function beginEngageManeuver(f, e, idx, desiredRange, t, events) {
    ...
    const pocket = snapPos(futureX - tx * desiredRange - ty * lateral * side, futureY - ty * desiredRange + tx * lateral * side);
    if (routeTravelSq(f, pocket) < 2.4 * 2.4)
        return false;
    f.routeX = pocket[0];
    f.routeY = pocket[1];
    f.routeActive = true;
...
function readySupport(f, fighters) {
    if (f.supportCastLocked)
        return -1;
    const ally = pickAlly(f, fighters);
    for (let i = 0; i < f.abilities.length; i++) {
        const a = f.abilities[i];
        if (a.cls !== "support" || a.cdLeft > 0 || f.stamina < a.cost)
            continue;
        const hasAlly = fighters.some((g) => g.team === f.team && g.id !== f.id && g.hp > 0);
        const healThresh = hasAlly ? 0.55 : 0.42;
        if (a.kind === "heal" && ally.hp / ally.maxHp < healThresh)
            return i;
        if ((a.kind === "buff" || a.kind === "haste") && f.statuses.buffLeft <= 0 && f.statuses.hasteLeft <= 0)
            return i;
        if ((a.kind === "shield" || a.kind === "barrier" || a.kind === "absorb") && ally.statuses.shieldHp <= 0 && ally.hp / ally.maxHp < 0.72)
            return i;
    }
    return -1;
}
function decide(f, fighters, projectiles, rng, t, events) {
    ...
    if (!desperate && f.dodgeCd <= 0 && f.stamina >= COST_DODGE && f.statuses.rootLeft <= 0
        ...
        if (d <= threatRange) {
            ...
            if (rng() < chance) { // rng() always drawn (order preserved); stall just lowers the threshold → no dodge
                ...
                let px = -dy / d, py = dx / d;
                if (!walkableAt(f.x + px * 1.6, f.y + py * 1.6)) {
                    px = -px;
                    py = -py;
                }
                f.moveDx = px;
                ...
                if (incomingWindup && rangedTell)
                    e.pendingTargetId = null;
                f.stamina -= COST_DODGE;
                f.dodgeCd = Math.round(pet_duel_sim_js_1.DUEL_TPS * 1.8);
                f.vx = 0;
                f.vy = 0;
            ...
            f.dodgeCd = Math.max(f.dodgeCd, Math.round(pet_duel_sim_js_1.DUEL_TPS * 0.4));
        }
    }
    ...
    const holdOpeningPower = t < Math.round(pet_duel_sim_js_1.DUEL_TPS * 2.2)
    ...
    if (sup >= 0 && !holdOpeningPower) {
        if (!desperate && d < SUPPORT_CAST_CLEARANCE) {
            ...
            beginReposition(f, Math.round(pet_duel_sim_js_1.DUEL_TPS * 0.9), e);
    ...
    if (useRanged) {
        ...
        const rangePhrase = [0.58, 0.94, 0.72, 0.86];
    ...
    if (canFire) {
        ...
        f.commit = 0;
        beginCast(f, offIdx, e.id, t, events);
        return;
    }
    ...
    const pressureTeam = _cinematicInitiativeTeam;
    if (!holdRepos && mobilityIdx >= 0 && f.team === pressureTeam && f.commit >= Math.round(pet_duel_sim_js_1.DUEL_TPS * 0.55)
        ...
        if (beginEngageManeuver(f, e, mobilityIdx, rStar, t, events))
            return;
    }
    ...
    if (offAb || f.basicCdLeft <= 0)
        f.commit++;
    else
        f.commit = Math.max(0, f.commit - 1);
    if (f.commit > Math.round(pet_duel_sim_js_1.DUEL_TPS * 1.25)) {
        if (!useRanged) {
            rStar = Math.min(rStar, MELEE_RANGE * 0.78);
            f.commit = Math.round(pet_duel_sim_js_1.DUEL_TPS * 1.2);
        }
        else {
    ...
    if ((enemyOpen || killShot) && !useRanged && d > moveRange)
        rStar = Math.min(rStar, moveRange * 0.9); // press the opening / go for the kill
    if (holdRepos)
        rStar = Math.max(rStar + f.style.reposBack + f.spacingOffset, MIN_REPOSITION_RANGE);
    else
        rStar = Math.max(MELEE_RANGE * 0.82, rStar + f.spacingOffset * (useRanged ? 0.34 : 0.16));
    ...
    {
        const p = _forcedEngage ? 1 : _stallPressure;
        if (p > 0)
            rStar *= 1 - 0.9 * p;
    }
    if (holdRepos)
        setIntent(f, hpFrac < f.style.retreatHp ? "retreat" : "reposition", rStar, "complete the exit lane and re-evaluate", hpFrac < f.style.retreatHp ? "health is below retreat threshold" : "attack recovery requires a spacing reset");
    else if (killShot || desperate)
        setIntent(f, "burst", rStar, "close and finish", killShot ? "enemy is vulnerable" : "last-stand pressure");
    else if (useRanged && d < rStar - BAND_H)
        setIntent(f, "kite", rStar, "open ranged spacing", "enemy is inside preferred range");
    else if (d > rStar + BAND_H)
        setIntent(f, "engage", rStar, "claim preferred attack range", "target is outside effective range");
    else
        setIntent(f, "hold position", rStar, "read cooldowns and preserve the firing pocket", "already inside preferred range band");
...
function tickStatuses(f) {
    ...
    if (s.burnLeft > 0) {
        if (s.burnLeft % Math.round(pet_duel_sim_js_1.DUEL_TPS * 0.4) === 0)
            f.hp -= s.burnDmg;
        if (--s.burnLeft <= 0) {
            s.burnDmg = 0;
            s.halfHeal = false;
    ...
    if (s.rootLeft > 0)
        s.rootLeft--;
    if (s.wallPenaltyLeft > 0)
        s.wallPenaltyLeft--;
    if (s.buffLeft > 0 && --s.buffLeft <= 0)
        s.buffMag = 0;
}
function stepManeuver(f, fighters) {
    const e = pickTarget(f, fighters);
    if (!e) {
        f.maneuverLeft = 0;
        return;
    ...
    const ml = Math.max(1e-4, Math.hypot(mx, my));
    mx /= ml;
    my /= ml;
    ...
    if (--f.maneuverLeft <= 0) {
        ...
        f.guardLeft = Math.round(pet_duel_sim_js_1.DUEL_TPS * (f.routeIntent === "flank" ? 0.08 : 0.27));
    }
}
function stepFighter(f, fighters, projectiles, nextProjId, rng, t, events, accuracyEnabled) {
    if (f.state === "dead" || f.hp <= 0)
        return;
    if (f.targetLockLeft > 0)
        f.targetLockLeft--;
    if (f.basicCdLeft > 0)
        f.basicCdLeft--;
    ...
    if (f.dodgeCd > 0)
        f.dodgeCd--;
    if (f.stamina < STAM_MAX)
        f.stamina = Math.min(STAM_MAX, f.stamina + STAM_REGEN);
    if (f.statuses.stunLeft > 0) {
        ...
        if (f.state === "dash" && f.lungeAbIdx > -2) {
            f.state = "stagger";
            f.stateLeft = f.staggerT;
            clearLunge(f);
        }
        f.maneuverLeft = 0;
        f.guardLeft = 0;
        f.vx = 0;
        f.vy = 0;
        f.x = clamp(f.x, -pet_duel_sim_js_1.ARENA_X, pet_duel_sim_js_1.ARENA_X);
        f.y = clamp(f.y, -pet_duel_sim_js_1.ARENA_Y, pet_duel_sim_js_1.ARENA_Y);
        return;
    }
    if (f.maneuverLeft > 0) {
    ...
    if (f.guardLeft > 0 && f.state === "idle") {
        ...
        if (!e || e.state !== "windup") {
            ...
            f.vy = 0;
            if (e) {
                const dx = e.x - f.x, dy = e.y - f.y, d = Math.max(1e-4, Math.hypot(dx, dy));
                f.faceX = dx / d;
                f.faceY = dy / d;
            }
            return;
    ...
    switch (f.state) {
        ...
        case "dash": {
            if (f.lungeAbIdx > -2) {
                ...
                if (f.lungeStuck >= 3) {
                    ...
                    if (bt && bd <= (ab ? ab.range : f.reach) + 0.45) {
                        resolveMeleeContact(f, ab, f.lungeTgtId, fighters, rng, t, events, accuracyEnabled);
                        clearLunge(f);
                        f.state = "strike";
                        f.stateLeft = 2;
                        ...
                        clearLunge(f);
                        f.state = "recover";
                        f.stateLeft = Math.max(1, f.recovT);
                    } // short recover — no dodge happened
                    break;
                }
                if (--f.stateLeft <= 0) {
                    ...
                    if (ab && !ab.signature)
                        ab.cdLeft = Math.min(ab.cdLeft, Math.round(ab.cdTicks * 0.35));
                    else if (!ab)
                        f.basicCdLeft = Math.min(f.basicCdLeft, Math.round(f.basicCdT * 0.35));
                    f.stamina = Math.min(STAM_MAX, f.stamina + (ab ? ab.cost : COST_BASIC) * 0.5);
                    clearLunge(f);
                    f.state = "recover";
                    f.stateLeft = Math.max(1, f.recovT + 4);
                }
                break;
            }
            const [nx, ny] = tryStep(f.x + f.moveDx * f.maxSpeed * 3.0, f.y + f.moveDy * f.maxSpeed * 3.0, f.x, f.y);
            f.x = nx;
            f.y = ny;
            f.faceX = f.moveDx;
            f.faceY = f.moveDy;
        ...
        case "dodge": {
            ...
            if (--f.stateLeft <= 0) {
                ...
                if (f.postDodgeSupport)
                    beginDodgeRetreat(f, target);
                else
                    beginReposition(f, Math.round(pet_duel_sim_js_1.DUEL_TPS * 0.65), target);
            }
            break;
        }
        case "windup":
            if (--f.stateLeft <= 0) {
                ...
                if (isMelee) {
                    ...
                    if (target && distance <= contact) {
                        ...
                        f.faceX = f.moveDx;
                        f.faceY = f.moveDy;
                        f.lungeAbIdx = f.pendingIdx;
                        f.lungeTgtId = f.pendingTargetId;
                        f.lungeStuck = 0;
                        f.state = "dash";
                        f.stateLeft = f.style.lungeTicks;
                    }
                    else {
        ...
        case "recover":
            if (--f.stateLeft <= 0) {
                ...
                const target = pickTarget(f, fighters);
                if (target)
                    _cinematicInitiativeTeam = target.team;
                ...
                if (target?.routeActive && target.reposLeft > 0) {
                    ...
                    f.vx = 0;
                    f.vy = 0;
                    f.guardLeft = Math.round(pet_duel_sim_js_1.DUEL_TPS * 0.18);
                }
                else
                    beginReposition(f, Math.round(pet_duel_sim_js_1.DUEL_TPS * f.style.reposDur), target);
            }
            break;
        case "stagger":
            if (--f.stateLeft <= 0) {
                ...
                f.vx = 0;
                f.vy = 0;
                f.guardLeft = Math.round(pet_duel_sim_js_1.DUEL_TPS * 0.2);
            }
            break;
    }
    f.x = clamp(f.x, -pet_duel_sim_js_1.ARENA_X, pet_duel_sim_js_1.ARENA_X);
    ...
    for (let i = 0; i < fighters.length; i++)
        for (let j = i + 1; j < fighters.length; j++) {
            ...
            if (d > 1e-6) {
                ...
                const [ax, ay] = snapPos(a.x - ux * push, a.y - uy * push);
                a.x = ax;
                a.y = ay;
                const [bx, by] = snapPos(b.x + ux * push, b.y + uy * push);
                b.x = bx;
                b.y = by;
            }
            else {
...
function quantizeFighter(f) {
    f.x = quant(f.x);
    f.y = quant(f.y);
    f.vx = quant(f.vx);
    f.vy = quant(f.vy);
    f.stamina = quant(f.stamina);
    f.faceX = quant(f.faceX);
    f.faceY = quant(f.faceY);
    f.statuses.shieldHp = quant(f.statuses.shieldHp);
}
function snap(t, fighters, projectiles, debugTrace) {
    return {
        t,
        actors: fighters.map((f) => ({
            ...
            ...(debugTrace ? { ai: {
                    ...
                    cooldownPriorities: f.abilities
                        ...
                        .slice(0, 4)
                        .map(({ ab }) => `${ab.name}: ${ab.cdLeft <= 0 ? "ready" : `${(ab.cdLeft / pet_duel_sim_js_1.DUEL_TPS).toFixed(1)}s`}`),
                    elementalSetup: (() => {
                        const target = f.targetId ? fighters.find((g) => g.id === f.targetId) : null;
                        const el = String(f.element ?? "None").toLowerCase();
                        if (el === "fire")
                            return target?.statuses.burnLeft ? "Burn active — force movement / finish" : "Looking to apply Burn pressure";
                        if (el === "water")
                            return target && (target.statuses.slowLeft || target.statuses.stunLeft) ? "Control active — punish or sustain" : "Looking for slow / redirection setup";
                        if (el === "lightning")
                            return target?.statuses.marked ? "Mark armed — burst will consume it" : "Looking for mark / interrupt window";
                        if (el === "earth")
                            return f.statuses.shieldHp > 0 || f.statuses.wallPenaltyLeft > 0 ? "Fortified zone active" : "Looking to establish barrier space";
                        if (el === "wind")
                            return target && (target.statuses.rootLeft || target.statuses.slowLeft) ? "Spacing control active — extend pressure" : "Looking to displace and change angle";
...
// ── Core loop ──────────────────────────────────────────────────────────────────
function simulate(fighters, seed, accuracyEnabled, debugTrace) {
    ...
    for (const f of fighters)
        prevTotalHp += Math.max(0, f.hp);
    for (const f of fighters) {
        const [sx, sy] = snapPos(f.x, f.y);
        f.x = sx;
        f.y = sy;
    }
    for (let t = 0; t < CAP_TICKS; t++) {
        ticks = t + 1;
        if (SIM_WALLS.length)
            SIM_WALLS = SIM_WALLS.filter((w) => w.expiry > t); // expire finished walls
        // Stall pressure: how long since ANY damage landed → ramps to force a decisive exchange.
        _stallPressure = clamp(((t - lastDmgTick) / pet_duel_sim_js_1.DUEL_TPS - STALL_START_SECS) / STALL_RAMP_SECS, 0, 1);
        if (_stallPressure >= 1)
            _forcedEngage = true; // latch a confirmed stand-off → brawl to a result
        ...
        if ((t & 1) === 1) {
            for (let i = fighters.length - 1; i >= 0; i--)
                stepFighter(fighters[i], fighters, projectiles, nextProjId, rng, t, events, accuracyEnabled);
        }
        else {
            for (const f of fighters)
                stepFighter(f, fighters, projectiles, nextProjId, rng, t, events, accuracyEnabled);
        }
        stepProjectiles(fighters, projectiles, rng, t, events);
        for (const f of fighters)
            tickStatuses(f);
        separateAll(fighters);
        const newlyDefeated = new Set();
        for (const f of fighters) {
        ...
        if (newlyDefeated.size > 0) {
            ...
            for (let i = projectiles.length - 1; i >= 0; i--) {
                const p = projectiles[i];
                if (newlyDefeated.has(p.ownerId) || newlyDefeated.has(p.targetId))
                    projectiles.splice(i, 1);
            }
        ...
        // builds in a genuine no-damage stand-off.
        {
            let totalHp = 0;
            for (const f of fighters)
                totalHp += Math.max(0, f.hp);
            if (totalHp < prevTotalHp - 0.5)
                lastDmgTick = t;
            prevTotalHp = totalHp;
        }
    ...
    const result = winner === "player" ? "win" : winner === "enemy" ? "loss" : "draw";
...
function runPetDuelCinematic(playerPet, enemyPet, seed, playerDamageMult = 1, playerHpMult = 1, playerReviveOnce = false, applyItems = true, accuracyEnabled = petAccuracyEnabled(), terrain = null, debugTrace = false) {
    const fighters = [
        buildFighter(playerPet, "player", 0, -5.6, 2.6, enemyPet.element, playerDamageMult * (0, pet_duel_sim_js_1.terrainPetMult)(terrain, playerPet.element), playerHpMult, playerReviveOnce, applyItems),
        buildFighter(enemyPet, "enemy", 0, 5.6, 2.6, playerPet.element, (0, pet_duel_sim_js_1.terrainPetMult)(terrain, enemyPet.element), 1, false, applyItems),
    ];
...
function runPetPartyDuelCinematic(playerLead, playerReserve, enemyLead, enemyReserve, seed, playerDamageMult = 1, playerHpMult = 1, playerReviveOnce = false, applyItems = true, accuracyEnabled = petAccuracyEnabled(), debugTrace = false) {
    const fighters = [buildFighter(playerLead, "player", 0, -5.6, 2.6, enemyLead.element, playerDamageMult, playerHpMult, playerReviveOnce, applyItems)];
    if (playerReserve)
        fighters.push(buildFighter(playerReserve, "player", 1, -5.0, -3.2, enemyReserve?.element ?? enemyLead.element, playerDamageMult, playerHpMult, false, applyItems));
    fighters.push(buildFighter(enemyLead, "enemy", 0, 5.6, 2.6, playerLead.element, 1, 1, false, applyItems));
    if (enemyReserve)
        fighters.push(buildFighter(enemyReserve, "enemy", 1, 5.0, -3.2, playerReserve?.element ?? playerLead.element, 1, 1, false, applyItems));
    return simulate(fighters, seed, accuracyEnabled, debugTrace);
}
...
function petCinematicArchetype(pet) {
