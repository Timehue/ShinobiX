...
const CAP_TICKS = pet_duel_sim_js_1.DUEL_TPS * 75; // ~75s cap; the sudden-death ramp guarantees a real KO before it
const TTK_HP = 3.0; // HP scale → long, meaty fights that show the kiting/repositioning (tune knob)
const LATE_T = pet_duel_sim_js_1.DUEL_TPS * 50; // sudden-death: past here damage ramps so fights END in a KO
const LATE_RAMP = pet_duel_sim_js_1.DUEL_TPS * 20; // dmg ×(1 + (t−LATE_T)/RAMP) → ×2.25 at cap
...
const maskAt = (c, r) => pet_arena_walkmask_js_1.WALK_MASK.charCodeAt(r * GCOLS + c) === 49;
...
const cellWalkable = (c, r) => c >= 0 && r >= 0 && c < GCOLS && r < GROWS && (maskAt(c, r) || maskAt(GCOLS - 1 - c, r)) && !cellBlockedByWall(c, r);
...
function hasLineOfSight(ax, ay, bx, by) {
    ...
    for (let i = 1; i < steps; i++) {
        const tt = i / steps;
        if (!walkableAt(ax + dx * tt, ay + dy * tt))
            return false;
    }
    return true;
}
...
function buildAbility(j) {
    ...
    return {
        ...
        castTicks: Math.round(pet_duel_sim_js_1.DUEL_TPS * (j.signature ? 0.5 : cls === "support" ? 0.25 : 0.3)),
        cdTicks: base + (j.signature ? Math.round(pet_duel_sim_js_1.DUEL_TPS * 1.5) : 0),
        cdLeft: j.signature ? Math.round(pet_duel_sim_js_1.DUEL_TPS * 2.5) : Math.round(pet_duel_sim_js_1.DUEL_TPS * 0.5),
...
function statusTicks(ab, rounds) {
    return Math.round(pet_duel_sim_js_1.DUEL_TPS * (rounds && rounds > 0 ? rounds * 0.8 : ab.signature ? 1.4 : 1.0));
}
...
function buildFighter(pet, team, slot, x, y, oppElement, atkMult, hpMult, reviveOnce, applyItems) {
    ...
    const moveSpeed = clamp(2.8 + speed * 0.02, 2.8, 6.6) / pet_duel_sim_js_1.DUEL_TPS; // units/tick
...
function applyDamage(att, tgt, ab, rng, t, events, viaProjectile) {
    ...
    let mult = (0, pet_duel_sim_js_1.elementMult)(att.element, tgt.element) * (crit ? 1.6 : 1) * Math.max(0.3, buff);
...
function resolveCast(f, fighters, projectiles, nextProjId, rng, t, events, accuracyEnabled) {
    ...
    if (ab && ab.cls === "ranged") {
        ...
        for (const tgt of targets)
            projectiles.push({ id: nextProjId.n++, ownerId: f.id, team: f.team, targetId: tgt.id, abilityIdx: idx, x: f.x, y: f.y, speed: 0.34, ttl: Math.round(pet_duel_sim_js_1.DUEL_TPS * 3), element: f.element, kind: ab.kind });
    ...
    if (!ab && f.basicRanged) {
        ...
        if (tgt) {
            projectiles.push({ id: nextProjId.n++, ownerId: f.id, team: f.team, targetId: tgt.id, abilityIdx: -1, x: f.x, y: f.y, speed: 0.34, ttl: Math.round(pet_duel_sim_js_1.DUEL_TPS * 3), element: f.element, kind: "damage" });
...
function steer(f, e, fighters, rStar, routeGoal) {
    for (let i = 0; i < N; i++) {
        _interest[i] = 0;
        _danger[i] = 0;
    }
    ...
    if (cl > 1e-3)
        writeMap(_interest, -f.x / cl, -f.y / cl, CENTER_BIAS, false);
...
function decide(f, fighters, projectiles, rng, t, events) {
    ...
    if (!desperate && f.dodgeCd <= 0 && f.stamina >= COST_DODGE && f.statuses.rootLeft <= 0
        ...
        if (d <= eRange + 1.2) {
            ...
            if (rng() < chance) { // rng() always drawn (order preserved); stall just lowers the threshold → no dodge
                ...
                f.dodgeCd = Math.round(pet_duel_sim_js_1.DUEL_TPS * 0.9);
                f.vx = 0;
                f.vy = 0;
    ...
    if (f.commit > Math.round(pet_duel_sim_js_1.DUEL_TPS * 2.5)) {
        if (!useRanged) {
        ...
        f.commit = Math.round(pet_duel_sim_js_1.DUEL_TPS * 1.2); // press into range
    }
    if ((enemyOpen || killShot) && !useRanged && d > moveRange)
        rStar = Math.min(rStar, moveRange * 0.9); // press the opening / go for the kill
    if (holdRepos)
        rStar += f.style.reposBack; // reposition beat: back out by the archetype's amount (kiter dances far, defender holds ground) then circle back in — the in-out cadence
...
function stepFighter(f, fighters, projectiles, nextProjId, rng, t, events, accuracyEnabled) {
    ...
    switch (f.state) {
        ...
        case "dodge": {
            const [nx, ny] = tryStep(f.x + f.moveDx * f.maxSpeed * 2.7, f.y + f.moveDy * f.maxSpeed * 2.7, f.x, f.y);
            f.x = nx;
            f.y = ny;
            if (--f.stateLeft <= 0)
                f.state = "idle";
            ...
            if (--f.stateLeft <= 0) {
                ...
                if (isMelee) {
                    ...
                    if (f.statuses.rootLeft > 0) {
                        ...
                        if (tgt) {
                            const ddx = tgt.x - f.x, ddy = tgt.y - f.y, dd = Math.max(1e-4, Math.sqrt(ddx * ddx + ddy * ddy));
                            f.moveDx = ddx / dd;
                            f.moveDy = ddy / dd;
                            f.faceX = ddx / dd;
                            f.faceY = ddy / dd;
        ...
        case "recover":
            if (--f.stateLeft <= 0) {
                f.state = "idle";
                f.reposLeft = Math.round(pet_duel_sim_js_1.DUEL_TPS * f.style.reposDur);
            }
            break; // reposition beat: per-archetype (rusher stays on you, kiter dances out) — the anime in-out cadence
        case "stagger":
            if (--f.stateLeft <= 0)
                f.state = "idle";
            break;
...
function snap(t, fighters, projectiles) {
    return {
        t,
        actors: fighters.map((f) => ({ id: f.id, team: f.team, slot: f.slot, x: f.x, y: f.y, faceX: f.faceX, faceY: f.faceY, hp: Math.max(0, f.hp), maxHp: f.maxHp, stamina: f.stamina, state: f.state, statuses: statusFlags(f.statuses) })),
...
function simulate(fighters, seed, accuracyEnabled) {
    ...
    if (winner === null && teamAlive(fighters, "player") && teamAlive(fighters, "enemy")) {
        const frac = (team) => { let hp = 0, max = 0; for (const f of fighters)
            if (f.team === team) {
                hp += Math.max(0, f.hp);
                max += f.maxHp;
...
function runPetDuelCinematic(playerPet, enemyPet, seed, playerDamageMult = 1, playerHpMult = 1, playerReviveOnce = false, applyItems = true, accuracyEnabled = petAccuracyEnabled(), terrain = null) {
    const fighters = [
...
function runPetPartyDuelCinematic(playerLead, playerReserve, enemyLead, enemyReserve, seed, playerDamageMult = 1, playerHpMult = 1, playerReviveOnce = false, applyItems = true, accuracyEnabled = petAccuracyEnabled()) {
