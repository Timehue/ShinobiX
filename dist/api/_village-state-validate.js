...
exports.validateVillageStateWrite = validateVillageStateWrite;
exports.loadAuthoritativeKage = loadAuthoritativeKage;
 ...
 */
async function validateVillageStateWrite(existing, incoming, ctx, 
...
kageState) {
    ...
    if (Array.isArray(incoming.noticePosts)) {
        ...
        const prevIds = new Set(prevPosts.map((p) => String(p.id ?? '')).filter(Boolean));
        ...
        const incomingIds = new Set(incomingPosts.map((p) => String(p.id ?? '')).filter(Boolean));
        ...
        if (removed.length > 0 && !callerIsSeatedKage) {
            ...
            const silence = ctx.isAdmin ? null : await (0, moderation_js_1.getActiveSilence)(ctx.callerName).catch(() => null);
            const cleaned = [];
            for (const raw of incomingPosts) {
                ...
                if (!ctx.isAdmin) {
                    if (typeof post.title === 'string') {
                        post.title = (0, _text_moderation_js_1.sanitizeUserText)(post.title, _text_moderation_js_1.TEXT_LIMITS.noticeTitle);
                    }
                    if (typeof post.body === 'string') {
                        post.body = (0, _text_moderation_js_1.sanitizeUserText)(post.body, _text_moderation_js_1.TEXT_LIMITS.noticeBody);
                    }
    ...
    if (Array.isArray(incoming.kageChallenges)) {
        ...
        const prevById = new Map(prevChals.map((c) => [String(c.id ?? ''), c]));
...
async function loadAuthoritativeKage(village) {
    const slug = village.toLowerCase().replace(/\s+/g, '-');
    return (await _storage_js_1.kv.get(`village:kage:${slug}`)) ?? null;
}
