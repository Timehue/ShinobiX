...
exports.evaluateLaunchControl = evaluateLaunchControl;
exports.newRegistrationsDisabled = newRegistrationsDisabled;
exports.scheduledJobsDisabled = scheduledJobsDisabled;
...
function isOperatorRecoveryPath(path) {
...
function evaluateLaunchControl(req, env = process.env) {
    ...
    if (isOperatorRecoveryPath(path))
        return ALLOWED;
    ...
    if (enabled(env, 'FREEZE_ECONOMY_REWARDS')
...
function newRegistrationsDisabled(env = process.env) {
    return enabled(env, 'DISABLE_NEW_REGISTRATIONS') || enabled(env, 'MAINTENANCE_MODE');
}
function scheduledJobsDisabled(env = process.env) {
