...
exports.buildPublicSaveDTO = buildPublicSaveDTO;
...
const PUBLIC_CHAR_FIELDS = new Set([
...
const PUBLIC_COMBAT_TOPLEVEL_FIELDS = ['savedBloodlines', 'creatorJutsus', 'creatorItems'];
...
function buildPublicSaveDTO(data, opts) {
    const char = data.character;
    const projectedChar = {};
    if (char && typeof char === 'object') {
        for (const k of PUBLIC_CHAR_FIELDS) {
            if (k in char)
                projectedChar[k] = char[k];
        }
    }
    const out = { character: projectedChar };
...
function combatProjection(data) {
    const out = { ...data };
    for (const f of COMBAT_STRIP_TOPLEVEL_FIELDS)
        delete out[f];
    const char = out.character;
    if (char && typeof char === 'object') {
        const trimmed = { ...char };
        for (const f of COMBAT_STRIP_CHAR_FIELDS)
            delete trimmed[f];
...
function rankFromXp(profession, xp) {
    ...
    for (let i = 1; i <= MAX_PROFESSION_RANK; i += 1) {
        if (xp >= t[i])
            rank = Math.min(MAX_PROFESSION_RANK, i + 1);
    }
...
async function writeGainsWindow(name, w) {
    try {
...
function sanitizeCharacterSave(incoming, existing) {
    ...
    if (typeof char.customTitle === 'string' && char.customTitle.trim()) {
        ...
        if (titleChanged && (0, _titles_registry_js_1.isServerCreditedTitle)(masked)) {
        ...
        else if (titleChanged && (0, _titles_registry_js_1.isKnownEarnedTitle)(masked)) {
            ...
            const storedServer = existing?.character?.serverTitles;
            const owned = new Set([
                ...(Array.isArray(exChar.earnedTitles) ? exChar.earnedTitles : []).map((t) => (0, _titles_registry_js_1.normalizeTitleKey)(String(t))),
                ...(Array.isArray(storedServer) ? storedServer : []).map((t) => (0, _titles_registry_js_1.normalizeTitleKey)(String(t))),
            ]);
            char.customTitle = owned.has(norm) ? masked : '';
        }
        else if ((0, _legacy_track_js_1.legacyEnabled)() && titleChanged) {
    ...
    if ((0, _legacy_track_js_1.legacyEnabled)()) {
        if ('customTitleStyle' in char) {
            char.customTitleStyle = (typeof char.customTitleStyle === 'string' && _titles_registry_js_1.TITLE_STYLE_IDS.has(char.customTitleStyle)) ? char.customTitleStyle : '';
        }
        if ('customTitleIcon' in char) {
            char.customTitleIcon = (typeof char.customTitleIcon === 'string' && _titles_registry_js_1.TITLE_ICON_SET.has(char.customTitleIcon)) ? char.customTitleIcon : '';
        }
    }
    else {
        const exChar = existing?.character;
        if (typeof exChar?.customTitleStyle === 'string')
            char.customTitleStyle = exChar.customTitleStyle;
        else
            delete char.customTitleStyle;
        if (typeof exChar?.customTitleIcon === 'string')
            char.customTitleIcon = exChar.customTitleIcon;
        else
            delete char.customTitleIcon;
    }
    // ── Legacy (server-owned) ───────────────────────────────────────
    ...
    {
        const exLegacy = existing?.character?.legacy;
        if (exLegacy !== undefined)
            char.legacy = exLegacy;
        else
            delete char.legacy;
    }
    ...
    if (!isFirstSave) {
        if (exChar.masterySpec !== undefined)
            char.masterySpec = exChar.masterySpec;
        else
            delete char.masterySpec;
    }
    if (!isFirstSave) {
        for (const field of ['customTitle', 'customTitleStyle', 'customTitleIcon']) {
            if (exChar[field] !== undefined)
                char[field] = exChar[field];
            else
                delete char[field];
    ...
    if (Array.isArray(exChar.redeemedTrainingTokens))
        ...
        delete char.redeemedTrainingTokens;
    if (Array.isArray(exChar.redeemedJutsuTrainingActions))
        char.redeemedJutsuTrainingActions = exChar.redeemedJutsuTrainingActions;
    else
        delete char.redeemedJutsuTrainingActions;
    if (Array.isArray(exChar.redeemedAiFightRewards))
        char.redeemedAiFightRewards = exChar.redeemedAiFightRewards;
    else
        delete char.redeemedAiFightRewards;
    ...
    for (const field of ['serverExploreDate', 'serverExploresToday', 'redeemedSectorExplorations']) {
        if (exChar[field] !== undefined)
            char[field] = exChar[field];
        else
            delete char[field];
    }
    for (const field of ['serverChestDate', 'serverChestsToday', 'redeemedAncientChests']) {
        if (exChar[field] !== undefined)
            char[field] = exChar[field];
        else
            delete char[field];
    }
    if (Array.isArray(exChar.redeemedShopPurchases))
        char.redeemedShopPurchases = exChar.redeemedShopPurchases;
    else
        delete char.redeemedShopPurchases;
    if (Array.isArray(exChar.redeemedShopSales))
        char.redeemedShopSales = exChar.redeemedShopSales;
    else
        delete char.redeemedShopSales;
    if (Array.isArray(exChar.redeemedCrafts))
        char.redeemedCrafts = exChar.redeemedCrafts;
    else
        delete char.redeemedCrafts;
    if (Array.isArray(exChar.redeemedNamedForges))
        char.redeemedNamedForges = exChar.redeemedNamedForges;
    else
        delete char.redeemedNamedForges;
    ...
    for (const field of ['unlockedAchievements', 'achievementUnlockedAt', 'claimedAchievementRewards', 'earnedTitles']) {
        if (exChar[field] !== undefined)
            char[field] = exChar[field];
        else
            delete char[field];
    ...
    for (const field of ['endlessTowerRun', 'endlessTowerBestWave', 'totalEndlessTowerWins', 'dailyTowerXp', 'dailyEndlessRuns', 'dailyEndlessDate', 'redeemedEndlessActions']) {
        if (exChar[field] !== undefined)
            char[field] = exChar[field];
        else
            delete char[field];
    ...
    if (Array.isArray(exChar.redeemedStoryBattles))
        char.redeemedStoryBattles = exChar.redeemedStoryBattles;
    else
        delete char.redeemedStoryBattles;
    // One-time Academy spar payout latch, written by /api/story/settle.
    if (exChar.academySparClaimed === true)
        char.academySparClaimed = true;
    else
        delete char.academySparClaimed;
    if (exChar.starterPetClaimed === true)
        char.starterPetClaimed = true;
    else
        delete char.starterPetClaimed;
    if (Array.isArray(exChar.redeemedPetEncounters))
        char.redeemedPetEncounters = exChar.redeemedPetEncounters;
    else
        delete char.redeemedPetEncounters;
    if (Array.isArray(exChar.claimedCreatorEvents))
        char.claimedCreatorEvents = exChar.claimedCreatorEvents;
    else
        delete char.claimedCreatorEvents;
    if (Array.isArray(exChar.claimedWarCrateIds))
        char.claimedWarCrateIds = exChar.claimedWarCrateIds;
    else
        delete char.claimedWarCrateIds;
    ...
    for (const [field, maxDelta] of Object.entries(LIFETIME_COUNTERS)) {
        const inV = Math.max(0, Number(char[field] ?? 0));
        const exV = Math.max(0, Number(exChar[field] ?? 0));
    ...
    const MONTHLY_CLAN_CONTRIB_CAPS = {
    ...
    if (Array.isArray(char.itemStacks)) {
        ...
        for (const s of char.itemStacks) {
    ...
    if (!isFirstSave)
        char.examsPassed = exExams.filter((exam, index) => KNOWN_EXAMS.has(exam) && exExams.indexOf(exam) === index).slice(0, 4);
    ...
    if (char.pendingCombatMissionClaims !== undefined) {
        ...
        for (const raw of rawPending) {
            ...
            if (!key || seenPending.has(key))
                continue;
            const def = (0, _mission_catalog_js_1.combatMissionByKey)(key);
            if (!def)
                continue; // not a real catalog mission key
            if (charLevel < def.min)
                continue; // below the mission's level gate
            validatedPending.push(key);
            seenPending.add(key);
            if (validatedPending.length >= PENDING_COMBAT_CLAIMS_CAP)
                break;
        }
    ...
    const BLOODLINE_RANK_ORDER = { 'B Rank': 0, 'A Rank': 1, 'S Rank': 2 };
    ...
    const pendingBloodlineForges = (0, _forge_js_1.readPendingBloodlineForges)(existing?.pendingBloodlineForges);
    const consumedBloodlineForgeIds = new Set();
    ...
    const normalizeBloodlineArray = (arr, existingArr, mayConsumeForge = false) => {
        if (!Array.isArray(arr))
            return arr;
        const existingRankById = new Map();
        if (Array.isArray(existingArr)) {
            for (const eb of existingArr) {
                if (eb && typeof eb === 'object') {
                    const eid = String(eb.id ?? '');
                    const er = String(eb.rank ?? '');
                    if (eid && KNOWN_BLOODLINE_RANKS.has(er))
                        existingRankById.set(eid, er);
                }
            }
        }
        return arr.slice(0, BLOODLINE_CAP).map((bl) => {
            ...
            if (!storedRank || isUpgrade) {
                const requestedRank = (0, _forge_js_1.parseBloodlineForgeRank)(rawRank);
            ...
            if (Array.isArray(out.jutsus)) {
                ...
                out.jutsus = (0, _jutsu_points_js_1.enforceBloodlineBudget)(out.jutsus, blRank);
                out.totalPoints = Math.min(20, (0, _jutsu_points_js_1.bloodlinePoints)(out.jutsus, blRank));
            }
            return out;
        }).filter((bl) => bl !== null);
    };
    ...
    if (Array.isArray(char.savedBloodlines))
        char.savedBloodlines = normalizeBloodlineArray(char.savedBloodlines, exChar.savedBloodlines, false);
    ...
    if (char.endlessTowerRun && typeof char.endlessTowerRun === 'object') {
        const run = char.endlessTowerRun;
        if (run.bankedRyo != null)
            run.bankedRyo = Math.max(0, Math.min(ET_BANKED_RYO_CAP, Number(run.bankedRyo) || 0));
        if (run.bankedXp != null)
            run.bankedXp = Math.max(0, Math.min(ET_BANKED_XP_CAP, Number(run.bankedXp) || 0));
        if (run.wave != null)
    ...
    if (char.hollowGateRun && typeof char.hollowGateRun === 'object') {
        ...
        if (run.runToken != null)
            run.runToken = String(run.runToken).slice(0, 64);
        if (run.serverSeed != null)
            run.serverSeed = String(run.serverSeed).slice(0, 64);
        if (run.earnedXp != null)
            run.earnedXp = Math.max(0, Math.min(200_000, Math.floor(Number(run.earnedXp) || 0)));
        if (run.earnedFragments != null)
            run.earnedFragments = Math.max(0, Math.min(40, Math.floor(Number(run.earnedFragments) || 0)));
        if (run.earnedVeils != null)
            run.earnedVeils = Math.max(0, Math.min(25, Math.floor(Number(run.earnedVeils) || 0)));
        if (run.activeCombat && typeof run.activeCombat === 'object') {
            const active = run.activeCombat;
        ...
        if (Array.isArray(run.augmentOffers) && run.augmentOffers.length > 8) {
            run.augmentOffers = run.augmentOffers.slice(0, 8);
        }
    }
    ...
    if (Array.isArray(char.defeatedAiIds) && char.defeatedAiIds.length > DEFEATED_AI_IDS_CAP) {
        char.defeatedAiIds = char.defeatedAiIds.slice(-DEFEATED_AI_IDS_CAP);
    }
    ...
    const entitledTileCards = (0, _entitlement_guard_js_1.preserveEntitledStringArray)(char.tileCards, exChar.tileCards, () => true);
    ...
    if ('battleHistory' in char) {
        if (!Array.isArray(char.battleHistory)) {
            delete char.battleHistory;
        }
        else {
            char.battleHistory = char.battleHistory
                .slice(0, BATTLE_HISTORY_CAP)
                .map((b) => {
                if (b && Array.isArray(b.actions) && b.actions.length > BATTLE_ACTIONS_CAP) {
                    return { ...b, actions: b.actions.slice(-BATTLE_ACTIONS_CAP) };
                }
                return b;
            });
        }
    ...
    const DAILY_CLAIM_DATE_FIELDS = ['claimedVillageAgendaDate', 'claimedMapControlDate', 'warGroundBountyDate'];
    for (const field of DAILY_CLAIM_DATE_FIELDS) {
        const incomingDate = char[field];
        if (typeof incomingDate !== 'string' || incomingDate === '')
            continue;
    ...
    for (const field of ['lastDailyReset', 'lastHuntReset']) {
        const stored = typeof exChar[field] === 'string' ? exChar[field] : '';
        const incoming = typeof char[field] === 'string' ? char[field] : '';
    ...
    if (Array.isArray(incoming.creatorItems)) {
        sanitizedCreatorItems = incoming.creatorItems
            ...
            if (out.weaponEp != null)
                out.weaponEp = Math.max(0, Math.min(60, Number(out.weaponEp) || 0));
    ...
    if (!isFirstSave)
        out.activeTraining = existing?.activeTraining ?? null;
    if (!isFirstSave)
        out.activeJutsuTraining = existing?.activeJutsuTraining ?? null;
...
async function recordMissingSaveVersion(playerName) {
    try {
        const key = (0, _save_version_js_1.saveVersionTelemetryKey)(new Date().toISOString());
        const cur = (await _storage_js_1.kv.get(key)) ?? {};
        await _storage_js_1.kv.set(key, { count: Number(cur.count ?? 0) + 1, lastPlayer: playerName, lastAt: Date.now() }, { ex: SAVE_NOVERSION_TELEMETRY_TTL_SEC });
    }
    catch {
        // Telemetry is best-effort and MUST NOT affect the save outcome.
    }
}
async function validateClanAndVillageIdentity(safeIncoming, existing, playerName) {
    ...
    if (req.method === 'POST') {
        try {
            ...
            if (isAdminSave) {
                ...
                if (!identity.admin && isClanSave) {
                    ...
                    try {
                        ...
                        if (!actorClan || actorClan !== targetClanSlug) {
                            ...
                            const matchesCaller = (entry) => (0, _utils_js_1.safeName)(String(entry?.name ?? '')) === identity.name;
                            const callerInRequests = Array.isArray(incomingBody?.joinRequests)
                                && incomingBody.joinRequests.some(matchesCaller);
                            const callerInMembers = Array.isArray(incomingBody?.members)
                                && incomingBody.members.some(matchesCaller);
                            ...
                            if (allowCreate) {
                                ...
                                if (!(await (0, _ratelimit_js_1.enforceRateLimitKv)(req, res, 'clan-create', 3, 60 * 60_000, identity.name)))
                                    return;
                            }
                            else {
                                ...
                                if (!(await (0, _ratelimit_js_1.enforceRateLimitKv)(req, res, 'clan-join-request', 20, 60 * 60_000, identity.name)))
                                    return;
                            }
                        }
                    }
                ...
                if (!isClanSave && !(await (0, _ratelimit_js_1.enforceRateLimitKv)(req, res, 'save-burst', 1, 3_000, identityName))) {
            ...
            const adminStoredVersion = Number(existing?._saveVersion ?? 0);
