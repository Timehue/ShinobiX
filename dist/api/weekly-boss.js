...
function defaultBossHp(weekKey) {
...
async function buildFreshBossState(weekKey) {
    ...
    const overrideId = await _storage_js_1.kv.get(WEEKLY_BOSS_OVERRIDE_KEY);
    let aiId = overrideId ?? '';
    let bossName;
    if (!aiId) {
...
async function loadOrInitBoss() {
    ...
    const existing = await _storage_js_1.kv.get(WEEKLY_BOSS_STATE_KEY);
    if (!existing)
        return null;
...
async function distributeRewardsIfExpired(boss) {
    ...
    await (0, _lock_js_1.withKvLock)(WEEKLY_BOSS_STATE_KEY, async () => {
        ...
        const entries = Object.entries(fresh.damageByPlayer)
            .sort(([, a], [, b]) => b - a);
        const totalDmg = entries.reduce((sum, [, dmg]) => sum + dmg, 0) || 1;
        const baseRyo = Math.floor(fresh.hpMax * 0.5);
        const baseXp = Math.floor(fresh.hpMax * 0.25);
        const computed = entries.map(([name, dmg], i) => {
    ...
    for (const entry of summary) {
        if (alreadyCredited.has(entry.name))
            continue;
        try {
            const did = await (0, _lock_js_1.withKvLock)(saveKeyCreditScope(entry.name), async () => {
                const saveKey = `save:${entry.name}`;
                const fresh = await _storage_js_1.kv.get(saveKey);
                const freshChar = fresh?.character;
                if (!fresh || !freshChar)
                    return true; // no save → nothing to credit; count as done
                ...
                const reserved = await _storage_js_1.kv.set(receiptKey, '1', { nx: true, ex: 35 * 24 * 60 * 60 });
                if (!reserved)
                    return true; // already credited by another pass
                try {
    ...
    if (newlyCredited.length > 0 || !finalBoss.rewardsDistributed) {
        await (0, _lock_js_1.withKvLock)(WEEKLY_BOSS_STATE_KEY, async () => {
            const fresh = await _storage_js_1.kv.get(WEEKLY_BOSS_STATE_KEY) ?? finalBoss;
            const credited = new Set([...(fresh.creditedPlayers ?? []), ...newlyCredited]);
...
function saveKeyCreditScope(name) {
    return `save:${name}`;
}
async function handler(req, res) {
    ...
    if (req.method === 'POST') {
        ...
        try {
            ...
            if (weekKey && weekKey !== boss.weekKey)
                return res.status(409).json({ error: 'Stale week — boss has reset.' });
            ...
            if (kind === 'damage') {
                ...
                if (!identity.admin) {
                    ...
                    const placed = await _storage_js_1.kv.set(cdKey, '1', { nx: true, ex: WEEKLY_BOSS_COOLDOWN_SECONDS });
                    if (!placed) {
