...
async function handler(req, res) {
    ...
    if (req.method === 'POST') {
        ...
        try {
            ...
            const boss = await loadOrInitBoss();
            if (!boss)
                return res.status(409).json({ error: 'No boss available — admin must set weeklyBossOverride.' });
            ...
            if (kind === 'claim') {
                ...
                const claimResult = await (0, _lock_js_1.withKvLock)(WEEKLY_BOSS_STATE_KEY, async () => {
                    const fresh = await _storage_js_1.kv.get(WEEKLY_BOSS_STATE_KEY) ?? boss;
                    if (fresh.hpRemaining > 0)
                        return { error: 'alive' };
                    const claimed = fresh.killRewardedTo ?? [];
                    if (claimed.includes(actorName))
                        return { error: 'already-claimed' };
                    const myDamage = fresh.damageByPlayer[actorName] ?? 0;
                    if (myDamage <= 0)
                        return { error: 'no-damage' };
                ...
                if ('error' in claimResult) {
                    if (claimResult.error === 'alive')
                        return res.status(409).json({ error: 'Boss is still alive.' });
