...
(0, node_test_1.describe)('snapshot run health', () => {
    ...
    (0, node_test_1.it)('recovers an interrupted run without duplicating completed snapshots', async () => {
        const originalKeys = _storage_js_1.kv.keys;
        const originalGet = _storage_js_1.kv.get;
        const originalSet = _storage_js_1.kv.set;
        ...
        Date.now = () => now;
        _storage_js_1.kv.keys = async (pattern) => {
            if (pattern === 'save:*')
                return [...records.keys()].filter((key) => key.startsWith('save:'));
            if (pattern === 'save-snapshot:*')
                return [...records.keys()].filter((key) => key.startsWith('save-snapshot:'));
            return [];
        };
        _storage_js_1.kv.get = async (key) => (records.get(key) ?? null);
        _storage_js_1.kv.set = async (key, value) => {
        ...
        try {
            const interrupted = await (0, snapshot_saves_js_1.runSnapshotSaves)(100);
            node_assert_1.strict.equal(interrupted.ok, false);
            node_assert_1.strict.equal(interrupted.truncated, true);
            node_assert_1.strict.equal(interrupted.processed, 8);
            node_assert_1.strict.equal(interrupted.snapshotted, 8);
            node_assert_1.strict.equal(records.has(snapshot_saves_js_1.SNAPSHOT_SUCCESS_KEY), false);
            now += 1_000;
            const recovered = await (0, snapshot_saves_js_1.runSnapshotSaves)(5_000);
            node_assert_1.strict.equal(recovered.ok, true);
            node_assert_1.strict.equal(recovered.truncated, false);
            node_assert_1.strict.equal(recovered.processed, 9);
            node_assert_1.strict.equal(recovered.skipped, 8);
            node_assert_1.strict.equal(recovered.snapshotted, 1);
            node_assert_1.strict.equal(snapshotWrites, 9, 'retry must not rewrite the first completed batch');
            node_assert_1.strict.equal(records.has(snapshot_saves_js_1.SNAPSHOT_SUCCESS_KEY), true);
        }
        finally {
