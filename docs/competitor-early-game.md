...
> **the genre leader** and **a sibling shinobi RPG** — both start a new Academy
...
### a sibling shinobi RPG — sibling design
- Every ninja starts as an **Academy Student**; the defining first task is
  ...
  each). **Rank-up requires all 3 starter jutsu at level 5.** (a sibling shinobi RPG)
...
### a rival shinobi browser game — best-executed scripted FTUE
- The **tutorial IS the Genin Exam**: mentor NPC teaches Attack/Charge/Run via a
  ...
  free**, and graduating promotes Academy→Genin. (a rival shinobi browser game Wiki — Tutorial)
...
### a rival shinobi MMO — the "what to avoid" monetization benchmark
- Heavily guided, **auto-pathing** story FTUE; companions unlock by clearing PvE
  ...
  **pay-to-win**, and auto-combat "feels less engaging." (MMOs.com review, Common Sense Media)
...
- **Torn** — Energy (5/15min, cap 100) + Nerve; **does not refill on level-up**,
  ...
  "what now?" (a browser MMO's energy system, Duke Missions)
- **a long-running browser RPG** — 40 adventures/day (cap 200, hoarding deleted so you
  ...
  the post-tutorial cliff** ("once done, you're on your own"). (KoL Adventures, [jayisgames](https://jayisgames.com/review/kingdom-of-loat.php))
...
- **a long-running browser RPG** — a fresh Game Day every 4h refills stamina, **plus 2
  ...
  overload** at start. (a long-running browser RPG newbie guide)
...
- **First 10 minutes = guaranteed win → instant reward → first upgrade.** AFK
  ...
  **level 1→27 in the first session** and loans powerful units; idle games
  un-grey the first cheap upgrade within seconds. ([GameRefinery](https://www.gamerefinery.com/first-impression-seals-the-deal-onboarding-best-practices-part-1/), DoF a leading idle RPG)
- **Progressive feature unlock keyed to account level.** a leading open-world RPG's **Adventure
  Rank ladder** (AR12 dailies, AR16 co-op, AR20 endgame…) and a leading gacha RPG's
  **Trailblaze Level ladder** reveal one system at a time and gray out the rest.
  "Dump it all in the first 10 minutes and players bail." (open-world RPG account rank, a leading gacha RPG beginner guide)
- **Offline progress as the return hook.** a leading idle RPG's chest fills and **caps**
  (overflow nags you back); a leading idle RPG banks up to **24h** offline. Framed as "your
  stuff kept working while you were away," and **idle yield scales with active
  progress** so the two reinforce each other. (Macmillan, a leading idle RPG)
- **Energy is generous and never blocks the fun.** a leading open-world RPG/a leading gacha RPG keep **story &
  ...
  overflows," not "you're locked out." (a leading gacha RPG, [Raid energy](https://hellhades.com/beginners-energy-guide-raid-shadow-legends/))
- **The "first 7 days" program.** A **login calendar** where ~3-7 logins grant a
  **guaranteed premium reward**, + a **beginner-mission track** that teaches each
  system and pays guaranteed premium currency (Dokkan "Greatest Warrior," a leading gacha RPG's
  9-part Operation Briefing), + dailies as a **single activity-points bar →
  premium** (a leading gacha RPG: 500 points → 60 jade), not a nagging checklist. (a leading gacha RPG, a leading gacha RPG)
- **Always-visible "next thing."** Trackable main quest that auto-paths;
  ...
  that kills D1.
- **First gacha pull is free and chosen, not gambled.** a leading gacha RPG's **Selective
  Summon** (10-pull guaranteeing a 5★, infinite rerolls until happy); a leading gacha RPG's
  first warp guarantees a 4★. Monetized RNG is introduced only after the player
  has a usable build. (a leading gacha RPG)
...
7. **Defer premium pressure;** first pull free/chosen. Day-one paywalls are the
   #1 thing players resent (a rival shinobi MMO).
...
- ✅ Auto-learn bloodline jutsu at creation — confirmed as the genre standard
  (the genre leader/a sibling shinobi RPG/a rival shinobi browser game). ShinobiX is the outlier; fix it first.
- ✅ Guided first win in the first minutes (a rival shinobi browser game's exam-as-tutorial).
...
- **Rank-up as a learn-and-level checklist.** Make the Genin objective include
  "raise your starter jutsu to mastery level N" (the genre leader/a sibling shinobi RPG model), so
...
- **Progressive unlock ladder keyed to Ninja Rank** (open-world RPG account rank / a leading gacha RPG TL model).
...
- **Reframe training timers as a "while you were away" payout**, capped (8-24h)
  so it overflows and nags a return (a leading idle RPG/a leading idle RPG). Make the **first**
...
- ❌ Day-one paywall / premium pressure (a rival shinobi MMO's #1 criticism).
- ❌ Full auto-combat in the FTUE — keep the first fight manual so verbs are
  taught.
- ❌ Text-wall onboarding (a long-running browser RPG) — progressive disclosure instead.
...
Shinobi RPGs: a sibling shinobi RPG ·
a rival shinobi browser game Tutorial ·
a rival shinobi MMO review (MMOs.com) ·
Common Sense Media

PBBG lineage: a browser MMO's energy system ·
a browser MMO's mission chains ·
KoL Adventures ·
...
a long-running browser RPG newbie guide ·
[Outwar Recommended Path](https://outwar.info/wiki/Recommended_Path) ·
Politics & War Fundamentals

Idle/gacha: DoF — a leading idle RPG ·
[GameRefinery — Onboarding](https://www.gamerefinery.com/first-impression-seals-the-deal-onboarding-best-practices-part-1/) ·
a leading gacha RPG Beginner Guide (KQM) ·
a leading open-world RPG Adventure Rank ·
an idle RPG beginners guide ·
Macmillan — a leading idle RPG idle mechanics ·
a leading gacha RPG New Player Guide ·
