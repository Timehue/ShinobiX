...
**Refreshed 2026-09-25.** Village and Sector War change who owns the map, which
...
| Garrison | After 2 hours with no live battle, the attacker may fight the sector's garrison. It scores at half weight, capped at 150 points (200 while the garrison is fed). | `GARRISON_UNLOCK_IDLE_MS`, `GARRISON_POINTS_CAP` |
...
| When it settles | On the next war-map status poll after the deadline (every 15 seconds while anyone has the map open), on the next declaration, or at the **03:00 UTC** daily pass, whichever comes first. Two of them racing still settle once. | `api/_sector-war-settle.ts` |
...
The war settles on its own. Staff it anyway:

1. Just after the deadline, open the war map. That poll settles the war within
   seconds. Without it, the 03:00 UTC daily pass settles it.
...
| --- | --- | --- |
| War scoring must stop now | Set `DISABLE_VILLAGE_WAR=1` and redeploy. | Every war route answers 404, and mercenaries, the daily war pass, taxes and seeding stop. World PvP no longer binds or scores (fixed 2026-09-25). Settlement pauses too, because the map poll and the daily pass are off. Recorded scores stay, and due wars settle after the switch is removed. |
