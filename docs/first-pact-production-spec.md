...
## The Arbiter, and what standing is for

The Court's antagonist is a person. The Arbiter stands in the High Court square at the mouth of the council annex, wears the Court's plain grey with no ornament, and carries a nameplate that is blank because the office required the name and the Arbiter gave it. That is the deliberate inverse of the continuity ruling above: the Withheld refused cession of a defining choice, and the Arbiter made the same choice the other way and can still defend it. The last four rounds of the Balancing are an argument with a position, not a fight with a roster.

Registrar Orin and the Arbiter must never say the same thing. Orin explains the threshold as an obstacle a claimant has to clear. The Arbiter explains why the Court built the threshold, which is that anyone who reaches it has already spent the years that make a claimant reasonable, and reasonable claimants are cheap to answer.

Court Standing has two uses, and both are the Arbiter's. It opens the Balancing at 1,600, and beyond that it buys a finding its place in the permanent record for 600. A finding can only be entered for a writ the player actually answered, only once, and never out of the standing the Balancing gate is still weighing: until the Court has sat, the threshold is reserved and only the surplus above it can be spent. Entering a finding is the campaign's one purchase of permanence, so the quarter it was written about says so afterwards and the epilogue names it.

## Defeat

Every encounter in all three tables carries defeat copy, spoken by the opponent that won. A campaign fight that ends in silence and a rematch button reads as a dead end rather than pressure, and thirteen of these fights are mandatory. The copy may not repeat between encounters and may not narrate finality, because a loss is retryable everywhere in this campaign: the Impound withdraws to the ward gate with the paper still in hand, and is there again when the player comes back.
