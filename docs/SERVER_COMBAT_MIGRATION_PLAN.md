...
The implemented route is `POST /api/missions/combat-start`. It seals the save and enemy into an embedded non-public Tower floor. `Missions.tsx` reuses `BattleTowerFight` without HUD/layout changes. A completed winning session is validated by `/api/missions/queue-combat-claim`, which marks the binding settled and mints an authority-tagged single-use claim token. `claim-mission` accepts C/B/A/S only with that server-combat token; a local pending flag or legacy token cannot pay it. E/D remain capped tutorial Arena fights.
