...
| Push `ae547299b` (fix `206df07f8` + the rows above) | CI run 34249203121 | Production Image green; CI red before any test ran: `server-build-security` → `Retain server build evidence` (`actions/upload-artifact@v7`) failed to finalize with a 403 from GitHub's blob intermediary after the build and the server-dist artifact had both succeeded, and every downstream job was skipped as failed. Infrastructure, not code; a rerun does not revive the cancelled rollout, so main was re-tipped with this row. |
| Re-tip `43bd34d5b` | same tree + the row above | Every test job green, including the combat-layout chromium job over the countdown fix (the first CI pass of that check since 09-08 01:48Z). CI still red: two more `Retain … evidence` uploads hit the same 403 on FinalizeArtifact. GitHub status showed no incident; the repo holds ~243 GB of unexpired Actions artifacts (7,214 of them, 12–35 GB added per day at 14-day retention), which is the likely throttle. The 14 evidence uploads are diagnostics nothing downloads, so `.github/workflows/ci.yml` marks them `continue-on-error: true`; the build/release artifacts that jobs depend on are unchanged. Recommended follow-up for the owner: shorten evidence retention and drop videos/traces from passing runs. |
| Push `afca91758` (evidence uploads non-fatal) | CI run 34252805868 | Every test job green again. One required artifact READ failed this time: `e2e-responsive / 1-of-2` got the same 403 from the intermediary on `ListArtifacts` while fetching the release build (44 artifact operations succeeded, 1 refused, so roughly a 2% per-operation refusal rate that day; the repo is public, so this is not a storage quota). Nothing in the tree can change that, so main was re-tipped once more with this row. |
| **Live** | `75eb4cd22` (re-tip of `afca91758`; product tree = review fix `c998682d3` + Tower countdown fix `206df07f8` on the owner's `aaee4a7af`) | CI, Production Image and CodeQL all green at ~18:00Z; production `/health` reported `commitShort` `75eb4cd2` at 2026-09-08 18:29:52Z. The seventh pass, the Tower header fix, and the evidence-upload CI change are all live. |
