...
It covers the 244 page slots, the 5 dungeon entrances, and 4 draft event avatars, which qualify only if their bytes equal a retired portrait (253 slots). Each slot is downloaded and hashed first, and deleted only if its current bytes still equal the retired hash. Every slot it will delete is backed up beside a `manifest.json` before anything is removed. Deletion uses the batch form of the app's own endpoint, `DELETE /api/images` with `{ ids }`, 100 per request (full admins only). It clears R2 and the per-image keys, the asset registry and the cache version, and it rewrites the category hash and legacy bundle once per request. A direct database delete would not be enough, because `/api/img` rebuilds a missing key from the legacy bundles (and writes it back). Each run backs up to its own timestamped folder, and the script finishes by checking that the image index no longer lists the removed slots. `--restore` uploads one image at a time, so it is limited to 20 chosen slots per run (`--slots`).
...
**Incident, 2026-09-19.** The first version of `--apply` sent one `DELETE /api/images` per image. Each of those rewrites the whole event category hash (3.7 MB) and the legacy bundle (6.5 MB), so 253 in a row wrote about 2.5 GB. That filled the production database disk at 19:56:44 UTC, and Postgres crash-looped until the owner enlarged the disk, recovering at 20:01:58 UTC. Save data was not affected. 29 of the 253 slots had been removed; the rest stayed stored and hidden. The batch form above replaced the per-image loop: removing all of them now rewrites each record once.
