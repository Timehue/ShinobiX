...
GitHub's full contract suite caught a touch-hover regression before deployment: six homepage hover rules were not limited to hover-capable devices. Those rules now use `@media (hover: hover)`; the shared ungated-hover ceiling tightens from 61 to 58 instead of relaxing the guard. The touch-hover and mobile-target contracts, rebuilt bundle and size gate, and six affected Chromium layout/navigation scenarios passed after correction. Desktop hover appearance is preserved.

## Wide-monitor framing

The 2553 × 1264 monitor report exposed a cover-crop problem: the hero height stopped at 850px while the artwork continued scaling with viewport width, cutting off the shinobi's head. At widths of 1700px and above, the scene now occupies a centered frame capped at 1800px and additionally constrained by viewport height. Its edges fade into the existing charcoal surface. The atmosphere shares the same frame, so canopy flecks and the fox glow remain aligned. The caption stays with the scene rather than drifting into the outer gutter. Mobile and standard desktop rules retain their existing presentation.

The wide-monitor browser regression checks the actual image bounds containing the hair, fox, and tail at 2553 × 1264, 3440 × 1440, 1920 × 800, and 1920 × 1080. It also verifies no horizontal overflow and matching artwork/particle widths. The three initial monitor comparisons were visually reviewed. No image generation, extra asset downloads, per-frame JavaScript, or new animation layers were introduced.

Monitor-fix validation passed: 18 Chromium landing scenarios, six Firefox/WebKit framing/motion/navigation scenarios, 15 preload/mobile/hover contracts, standalone browser-test TypeScript, production Vite build, and the unchanged download budget. The original local preview was rebuilt with the same source correction. Existing phone, tablet, and standard desktop screenshot baselines remain unchanged.
