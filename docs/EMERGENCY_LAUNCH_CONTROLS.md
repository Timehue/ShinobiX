...
| --- | --- | --- |
| `MAINTENANCE_MODE=1` | Returns non-cacheable HTTP 503 for every Express player API route, including reads and login. It does not stop scheduled, realtime, game-loop, or other in-process writers. | Health, restart, admin, manual snapshot, and KV-proxy paths remain available. |
...
| `FREEZE_ECONOMY_REWARDS=1` | Rejects unsafe-method player requests (`POST`, `PUT`, `PATCH`, `DELETE`) at the shared Express route boundary, including new registration. It does not stop mutation-on-GET settlement, scheduled jobs, realtime/game-loop activity, or storage writers. | GET gameplay traffic, existing-player credential operations, telemetry, admin, manual snapshot, and KV-proxy paths. |
...
| `DISABLE_PRESENCE_STATE_JOBS=1` | Prevents presence snapshot restore/write scheduling and the one-second game loop, including sleeper-camp/travel-lease materialization. | Express traffic and Socket.IO remain independent; use their controls when required. |
...
Neither `MAINTENANCE_MODE` nor `FREEZE_ECONOMY_REWARDS` is a complete write or
storage-quiescence fence. A procedure that requires all state to stop must also
stop and verify scheduled jobs, realtime/game-loop writers, mutation-on-read
settlement, and any external writer. Do not infer that a newly added GET handler
or non-HTTP worker is covered by the shared unsafe-method gate.
...
4. Verify the control with a harmless request:
   ...
   - presence state: startup logs state that presence snapshots and the game loop are disabled.
