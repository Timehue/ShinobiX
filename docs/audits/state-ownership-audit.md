...
| Inventory / itemStacks | `character.inventory` / `.itemStacks` | MIXED — `preserveOwnedItems` conserves counts; one net-new non-server-owned item/save carve-out (`:1462`); server-owned ids never mintable | shop, craft, drops | No mint; drops = spend semantics | lock | MEDIUM (carve-out) |
