...
group('pet-progression-and-custody', /api\/pet\//, {
  ...
  amountAuthority:'Server duration/XP, costs, offspring/trait/palette roll and deterministic hatch from sealed outcome. C2 fixed: owner-approved additive Den/Yard bonus for new training; old sealed rewards unchanged.',
...
group('clan-mission', /api\/clan\/(mission\/|_mission-catalog)/, {
  activitySource:'Eight weekly missions; clan XP/treasury plus personal Clan Points.', actorBinding:'Authenticated clan member (admin exemption preserved); server reservation seals eligible contributors.', activityBinding:'Clan slug + ISO week + mission key; random private receipt owner binds shared proof.', amountAuthority:'Canonical progress targets/rewards and member-count XP scaling sealed in the private primary receipt.', clientTrustedFields:'Requested clan/mission, never progress/amount.', eligibilityProof:'Canonical clan contribution/territory state meets target at reservation.', expiry:'Current-week admission only; committed primary10days, pending primary no expiry, applied shared/personal evidence21days.', singleUseReplayProtection:'NX reservation + CAS co-written shared credit/proof; dedicated personal mission receipts outlast display history.', atomicBoundary:'Clan credit+proof share one CAS; each personal point award+proof shares a versioned save CAS. Final primary commit follows personal writes.', durableReceipt:'clan:mission-claimed:<clan>:<week>:<mission>; protected clanMissionSettlements and clanMissionPointReceipts; listing/audit projections.', retrySemantics:'C3 FIXED for new protocol: expired pending resumes unpaid shared credit from sealed grant; positive shared proof resumes only private points; completed repeats do not pay again.', partialFailureRecovery:'Local fault tests cover before/after shared credit, personal interruption, history churn and server-proof spoofing. Historical pending remains untouched and cannot authorize personal points. Prior-week ambiguity requires operator evidence.', adminSearchability:'Manual mission receipt/clan/audit correlation; no new admin interface.', featureKillSwitch:'No new switch or player action.'
});
group('clan-exchange', /api\/clan\/(exchange\/|_exchange)/, {
  activitySource:'Personal Clan Points → catalog currency/items/caches/cosmetics or clan War Supply.', actorBinding:'Authenticated member; new debit requires matching stored clan. Reserved debit stays bound to its original recipient clan.', activityBinding:'Treasury purchases use stable player request ID + clan/item fingerprint and server-generated journal token; other purchases retain existing counters.', amountAuthority:'Server Exchange table, hall level, limits and costs. Journal seals catalog quote; allowance period is fixed when the debit actually commits.', clientTrustedFields:'Item ID/clan/request identity after validation; timestamp only bounds recovery admission, never amount or eligibility.', eligibilityProof:'Points, hall level and membership before treasury debit; personal purchases retain capacity checks.', expiry:'Treasury request90days, durable journal90days refreshed on update, applied proofs100days; replay cannot outlive proof retention.', singleUseReplayProtection:'Treasury debit+purchase counter+proof share player CAS; shared War Supply+proof share clan CAS. Updated client retains request identity across uncertain responses.', atomicBoundary:'Existing clan→player locks plus CAS fences on both records; journal reserved before any debit.', durableReceipt:'economy-settlement:clan-exchange-<digest> with server token and sealed quote; protected clanExchangeSettlements on source player and clan; existing audit.', retrySemantics:'C4 FIXED for updated clients/new protocol: finish missing side without refund or second charge. Legacy no-ID callers resume unfinished debit; lost final success cannot be distinguished from a genuine new legacy purchase.', partialFailureRecovery:'Local tests cover each applied-side write, journal completion, lock expiry/concurrency, rollover, forged identity/value and pre-seeded proof fields. No reconstruction from an unbound debit or automatic historical refund.', adminSearchability:'Existing Economy Settlements journal discovery and audit correlation; no admin auth/UI change.', featureKillSwitch:'No new switch, modal or player action.'
});
...
write('value-path-registry.json',{auditedCommit:'ecd8d6ccaba017a6791ec0ec94f822c10658984d',sourceState:'Audited base plus approved C1–C5 corrections; see recovery-validation.json for final source hashes.',scope:'All mounted routes and syntactically discovered API value mutations. Every row has the required fields. Family evidence is explicitly distinguished from an individual endpoint certificate; computed/dynamic writes, socket-only handlers, schedules and legacy compatibility require callsite review. This is a risk inventory, not a claim every path is crash-safe.',families:groups.map(({id,detail})=>({id,...detail})),rows});
...
Object.assign(evidence, {
 'clan-mission': {
  'normal success':['clan/_reward-recovery.test.ts','new weekly mission claims'],
  'exact retry':['clan/_reward-recovery.test.ts','mission lost shared-write'],
  'simultaneous duplicate':['clan/_reward-recovery.test.ts','simultaneous mission'],
  'forged outcome':['clan/_reward-recovery.test.ts','pre-seeded before'],
  'response lost after commit':['clan/_reward-recovery.test.ts','mission lost shared-write'],
  'server/process interruption':['clan/_reward-recovery.test.ts','mission resumes a new-protocol'],
  'partial settlement':['clan/_reward-recovery.test.ts','mission point interruption'],
  'repeat claim after success':['clan/_reward-recovery.test.ts','display history is full']
 },
 'clan-exchange': {
  'normal success':['clan/_reward-recovery.test.ts','two genuine purchases'],
  'exact retry':['clan/_reward-recovery.test.ts','two genuine purchases'],
  'simultaneous duplicate':['clan/_reward-recovery.test.ts','simultaneous Exchange'],
  'wrong activity/run/battle':['clan/_reward-recovery.test.ts','another item or clan'],
  'expired proof':['clan/_reward-recovery.test.ts','expired and future'],
  'forged amount':['clan/_reward-recovery.test.ts','ignores forged price'],
  'forged outcome':['clan/_reward-recovery.test.ts','pre-seeded Exchange'],
  'response lost after commit':['clan/_reward-recovery.test.ts','Exchange recovers ${boundary} clan'],
  'server/process interruption':['clan/_reward-recovery.test.ts','suspended Exchange writer'],
  'partial settlement':['clan/_reward-recovery.test.ts','Exchange resumes ${boundary}'],
  'reconnect':['clan-exchange-retry.test.ts','lost Exchange response'],
  'repeat claim after success':['clan/_reward-recovery.test.ts','two genuine purchases']
 }
});
const failures = {'clan-and-village-treasury-transfers':{'forged outcome':'C5 FIXED: the new clan receipt tests reject forged/cleared source proof; before/after evidence retained'}};
