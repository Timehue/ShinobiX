...
| World-context village crisis | `world-crisis` lifecycle/observation, then `missions/ai-fight-start` with identity-only `world-crisis`; Solo action/state | `WorldCrisis` → `AiFightHost` | Solo session/token + World active pointer + global crisis run, village fronts, contributor ledger, and sealed proof ids | Server reconstructs the canonical village warden and level scaling from the player's saved village. Only a settled sealed win can add one defense, and retrying the same proof cannot add another. | solo-PvE → solo-PvE | Migrated at introduction. The level-37 awakening is global, while combat and contribution remain on the canonical Solo authority. |
