...
The new Rally adapter clones skeletons and animation clips, strips whole-skeleton root translation while preserving local joint motion, fits different body proportions and crossfades gameplay-driven ready/start/run/sprint/jump/airborne/land/stagger/technique/victory/defeat states. Existing guard, gallop, gallop-jump, hit reaction, victory and rest clips provide morphology-appropriate equivalents. Technique locomotion continues while element effects communicate activation. Finish positions separate all four pets, face them toward the viewer and widen the camera for phone aspect ratios. After the finish the canvas keeps drawing for 2.5 s of animation time, long enough for the pets to reach those spaces, the camera to pull back and the longest victory clip to play, and then renders only on demand. Results take focus at once but scroll into view only after that, so the podium stays on screen. With reduced motion the pets and camera cut straight to the finish framing instead of gliding. Mixers, cloned materials and skeletons are disposed when unloaded; immutable source assets remain cached.
...
The new code is modular and lazy loaded, with no new package dependency. Race simulation stays in refs; React HUD updates are limited to 10 Hz, and the memoised Canvas does not re-render for them. Rendering reuses existing model LODs/atlases, instanced scenery, capped DPR (1–1.5, or 1 without MSAA on the shared lite-fx gate), 1,024-pixel PCF shadows and a small dust pool. Owned WebGL resources, animation mixers, listeners, timers and input captures are cleaned up. Caravan mutates a persisted run instead of rerolling the map on each click, uses one request per choice and avoids routine polling.
