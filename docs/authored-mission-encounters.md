...
The first GitHub release run flagged an explicit `any` in the browser fixture's saved-record type. It was replaced with `unknown`; the fixture's executable behavior is unchanged. The full client lint transcript is retained as `mission-encounter-evidence/live-main-lint.log`.
