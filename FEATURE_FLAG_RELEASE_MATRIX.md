...
| Clan Boss Operations | Enabled with Clan Boss/admin monitored | Server-owned 1–4 player clan parties seal legal loadouts into the existing N-actor Tower encounter; a leased self-healing registry sweeper and cursor-paged diagnostics cover lifecycle drift; `DISABLE_CLAN_BOSS_PARTIES=1` preserves the legacy solo-compatible assault path. | High | Run `npm run certify:clan-boss-operation` and `npm run audit:clan-boss-balance`; then stage Postgres/packet-loss/human-duration checks, review queue and AFK aggregates, and keep the party kill switch ready. |
