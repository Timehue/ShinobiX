...
import type { StoryLane } from './_story-record.js';

export type StoryRoadEventDef = {
    id: string;
    levelReq: number;
    minProgress: number;
    traits: Record<string, StoryLane>;
};

function def(slug: string, levelReq: number, traits: Record<string, StoryLane>): StoryRoadEventDef {
    return { id: `story-road-${slug}`, levelReq, minProgress: levelReq >= 100 ? 9 : 0, traits };
}

const DEFS: StoryRoadEventDef[] = [
...
export const STORY_ROAD_EVENT_DEFS: Record<string, StoryRoadEventDef> = Object.fromEntries(DEFS.map((d) => [d.id, d]));
