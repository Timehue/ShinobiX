/*
 * PvE difficulty curve — the only copy.
 *
 * It started as a hand port of shinobij.client/src/lib/pve-difficulty.ts in
 * step 3b of the AI-fight migration (docs/runbooks/combat-mode-migration.md).
 * Once PvE resolved on the server, nothing in the client imported that file, so
 * it was deleted and its design notes moved here. Tune the curve in this file.
 *
 * It is the layer that makes early PvE survivable: enemy stats and HP are scaled
 * down by band, and — the load-bearing part — every enemy→player hit passes a
 * per-hit cap, a per-turn cap and an easy-band mercy floor.
 *
 ...
 * fighting another player": because aiStatsForLevel emits the full level-budget
 * block, peer's ×1.0 already mirrors a maxed PvP fighter (uncapped).
 *
 ...
 * The shared damage math is never touched.
 *
...
// Retuned for the unified stat model: aiStatsForLevel emits the FULL
...
// pool at level 100 so endgame PvE still reads like a real duel. Applied to
// standard PvE only (no live opponentCharacter, not endless/ranked) — exactly
// like the stat multiplier. STARTING values — tune from kill-time playtests.
...
// mirroring a maxed player. Combat math is unchanged — this only feeds it an
// honest mastery number. It also obeys the per-rank jutsu cap (the same rule
// players get), so a mid-band enemy can't out-master its own rank — e.g. a
// level-29 Genin foe is capped at mastery 20, not 29.
...
// is clamped to a learnable chunk of the bar. Max-level peer combat is
// intentionally UNCAPPED so endgame PvE hits as hard as a real duel. STARTING
// values — tune from kill-time playtests.
...
// `playerMaxHp`, by the encounter band. Returns Infinity (no cap) for the
// max-level peer. Callers gate this to standard PvE only (no live
// opponentCharacter, not endless/ranked) — exactly like the stat multiplier.
...
// caps the turn too (it used to be uncapped, so a two-action hard foe could
// chain ~120% of the bar into a kill — the main "they hit too hard" complaint in
// the 50–90 range); the max-level peer stays uncapped so endgame PvE plays like
// a real duel.
 ...
 * widening caps. Hard applies its per-hit/per-turn caps with no mercy floor.
 * Callers gate this to standard PvE (no live opponentCharacter, not
 * endless/ranked), exactly like the stat multiplier.
 */
...
// pure policy helpers; the server PvE engine applies them to its move selection.
...
// sets HOW STRONG it is; this sets HOW SMART it is. Pure data — the server PvE
// engine applies it.
//
// IMPORTANT: `usesSmartScorer` preserves the EXACT historical threshold
// (masterAi || level >= 30), so it never moves the basic→smart boundary or
// touches a combat number. Every other field gates behaviour that is off in the
// lower bands, so onboarding (easy) is unchanged.
...
/** Scale every numeric stat by the difficulty factor, clamped to the stat cap so
 *  the peer band tops out at a maxed-player-like profile rather than
 *  overshooting. A factor of exactly 1 returns the input unchanged (no
 *  allocation). Takes a plain record, since sealed stat blocks are already
 *  normalized. */
...
// ─── Weekly boss ─────────────────────────────────────────────────────────────
// Kept beside the standard band because both feed the same engine clamp, but
// they are SEPARATE mechanics — see the note at the top of this file. (This
// started as a port of client helpers that no longer exist; this is the only
// copy.)
