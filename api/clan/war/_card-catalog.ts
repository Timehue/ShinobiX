/*
 * Canonical Shinobi Card Clash stats — the server's source of truth for the
 * clan-war duel (audit #8).
 *
 * The clan-war duel is server-authoritative, but the client used to submit the
 * derived Clash stats (cost/power/element/rarity/ability) and the server only
 * range-checked them — so a player could field an OWNED weak card with maxed
 * stats. The api/ bundle can't import the client engine
 * (shinobij.client/src/lib/card-clash.ts — tsconfig.cpanel excludes the client),
 * and the save stores tileCards as bare IDS (no base stats), so we resolve the
 * canonical stats two ways:
 *
 *   • BUILT-IN cards (the 150-card catalog) → BUILTIN_CLASH, a table GENERATED
 *     from the client's own deriveCardClashCard (scripts/_card-probe.ts). Zero
 *     hand-port risk — it IS the client output. _card-catalog.test.ts re-derives
 *     from the live client catalog and fails if this table drifts.
 *   • CREATOR cards (admin-authored, stored at the save's top-level creatorCards
 *     with full base stats) → deriveClashStats(), a faithful port of the client
 *     derivation, applied at runtime to the card's authoritative base stats. The
 *     same drift test runs this port over all 150 built-in cards and asserts it
 *     matches the client exactly, so a port bug can't ship.
 *
 * The handler (tilecards.ts) overrides every submitted card's stats with the
 * canonical resolution and keeps only the id + ownership check — so deck strength
 * is tied to the cards a player actually earned, regardless of what the client
 * sends.
 */
import type { ClashAbility, ClashCard, ClashRarity } from './_card-clash-engine.js';

export type CanonClashStats = Omit<ClashCard, 'id'>;

// Minimal authoritative card identity needed to derive Clash stats — matches the
// client TileCard shape (shinobij.client/src/data/tile-cards.ts) for the fields
// the derivation reads.
export type TileBase = {
    id: string; name: string; element: string; rarity: string;
...
export const BUILTIN_CLASH: Record<string, CanonClashStats> = {
...
const LEGENDARY_ABILITY_OVERRIDES: Record<string, ClashAbility> = {
...
type Role = 'fighter' | 'defender' | 'support' | 'assassin' | 'summoner' | 'control';

const cardAverage = (c: TileBase): number => (c.top + c.right + c.bottom + c.left) / 4;
const cardSpread = (c: TileBase): number =>
    Math.max(c.top, c.right, c.bottom, c.left) - Math.min(c.top, c.right, c.bottom, c.left);

function deriveRole(card: TileBase, spread: number): Role {
    const name = card.name.toLowerCase();
    if (name.includes('clone') || name.includes('spirit') || name.includes('wisp') || name.includes('summon')) return 'summoner';
    if (name.includes('guard') || name.includes('turtle') || name.includes('golem') || name.includes('behemoth') || name.includes('titan')) return 'defender';
    if (name.includes('spy') || name.includes('assassin') || name.includes('thief') || name.includes('stalker') || name.includes('reaper')) return 'assassin';
    if (name.includes('monk') || name.includes('sage') || name.includes('keeper') || name.includes('messenger')) return 'support';
    if (card.element === 'Ice' || card.element === 'Shadow') return 'control';
    if (spread >= 30) return 'fighter';
    return 'fighter';
}

function deriveAbility(card: TileBase, role: Role): ClashAbility {
...
export function deriveClashStats(card: TileBase): CanonClashStats {
    const avg = cardAverage(card);
    const spread = cardSpread(card);
    let cost = 1;
    let power = Math.round(avg / 8);
    if (card.rarity === 'common') cost = avg < 24 ? 1 : avg < 32 ? 2 : 3;
    else if (card.rarity === 'rare') cost = avg < 45 ? 2 : avg < 56 ? 3 : 4;
    else if (card.rarity === 'epic') cost = avg < 68 ? 3 : avg < 78 ? 4 : 5;
    else cost = avg < 84 ? 4 : avg < 91 ? 5 : 6; // legendary
    ...
    return { element: card.element, rarity: card.rarity as ClashRarity, cost, power, ability };
}
...
export function buildCreatorBaseMap(creatorCards: unknown): Map<string, TileBase> {
    const map = new Map<string, TileBase>();
    if (!Array.isArray(creatorCards)) return map;
 ...
 */
export function canonicalClashStats(id: string, creatorBaseById: Map<string, TileBase>): CanonClashStats | null {
    const builtin = BUILTIN_CLASH[id];
    if (builtin) return builtin;
    const creator = creatorBaseById.get(id);
    if (creator) return deriveClashStats(creator);
    return null;
}
