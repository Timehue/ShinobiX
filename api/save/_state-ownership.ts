...
export const SAVE_FIELD_CONTRACT: readonly SaveFieldDef[] = [
    ...
    // ── Wallet & currencies ─────────────────────────────────────────────────
    f('ryo', 'character', 'server-clamped', 'currency', ['strict-ledger-char', 'combat-strip-char'], 'generic saves may decrease stored ryo for legacy client sinks; every increase must come from a server domain command'),
    ...
    f('settledHollowGateCombatIds', 'character', 'server-payout-stamp', 'hollow-gate', ['strict-ledger-char', 'always-ledger-char', 'combat-strip-char'], 'lost-ack recovery ledger written only by Hollow Gate combat settlement'),
    f('hollowGateCombatSettlements', 'character', 'server-payout-stamp', 'hollow-gate', ['strict-ledger-char', 'always-ledger-char', 'combat-strip-char'], 'fingerprinted save-atomic Hollow Gate payout receipts preserved through their recovery horizon'),
    f('soloPveCompanionSettlements', 'character', 'server-payout-stamp', 'receipts', ['strict-ledger-char', 'always-ledger-char', 'combat-strip-char'], 'bounded dedicated summon-cost recovery markers written atomically with companion gear and consumable charges'),
    f('soloPveItemSettlements', 'character', 'server-payout-stamp', 'receipts', ['strict-ledger-char', 'always-ledger-char', 'combat-strip-char'], 'bounded per-move Solo PvE item-charge recovery markers; generic saves cannot add, replace, or clear them'),
    f('aiFightRewardSettlements', 'character', 'server-payout-stamp', 'missions', ['strict-ledger-char', 'always-ledger-char', 'combat-strip-char'], 'token-horizon AI-fight redemption receipts and atomic daily counters; generic saves cannot add, replace, or clear them'),
    f('weeklyBossStartSettlements', 'character', 'server-payout-stamp', 'weekly-boss', ['strict-ledger-char', 'always-ledger-char', 'combat-strip-char'], 'run-bound stamina charges retained through prepared Weekly Boss start recovery and finalized only after the run is durably ready'),
    f('weeklyBossUsageSettlements', 'character', 'server-payout-stamp', 'weekly-boss', ['strict-ledger-char', 'always-ledger-char', 'combat-strip-char'], 'run-bound damage/usage journal committed atomically with the authoritative save cost before shared damage becomes distributable'),
    f('weeklyBossPayoutSettlements', 'character', 'server-payout-stamp', 'weekly-boss', ['strict-ledger-char', 'always-ledger-char', 'combat-strip-char'], 'dedicated save-atomic Weekly Boss payout proofs retained until the boss state acknowledges each credited player'),
    f('combatMissionClaimSettlements', 'character', 'server-payout-stamp', 'missions', ['strict-ledger-char', 'always-ledger-char', 'combat-strip-char'], 'run-bound mission combat payout receipt committed atomically with its exact server reward response'),
    f('bountySagaStamp', 'character', 'server-payout-stamp', 'pvp', ['strict-ledger-char', 'always-ledger-char', 'combat-strip-char'], 'single latest immutable PLACE/CLAIM saga stamp written atomically with the ryo debit/credit by /api/pvp/bounty; generic/admin autosaves must never add, replace, or clear it'),
    ...
    f('battleTowerSpireWeeklyBest', 'character', 'server-owned', 'towers', ['progression-entitlement-char']),
    f('battleTowerSpireWeekKey', 'character', 'server-owned', 'towers', ['progression-entitlement-char']),
    ...
    f('equippedJutsuIds', 'character', 'server-clamped', 'combat-loadout', [], 'active combat use capped at Base 12 / Supporter 15; a lapsed 13-15 preference tail is retained, and strict mode still requires learned ids'),
    f('equippedBloodlineId', 'character', 'server-clamped', 'bloodlines', [], 'client-selected custom bloodline; must remain inside the active Base 1 / Supporter 2 stored projection'),
