...
export interface MercenaryTier {
    /** stable id (also the per-war once-only marker suffix) */
    id: string;
    /** the merc band's shinobi level — the flavor "tier" the owner asked for */
    level: number;
    name: string;
    blurb: string;
    /** Honor Seals spent to hire (deducted server-side, recomputed at hire) */
    costSeals: number;
    /** war damage dealt to the enemy village on hire (sealed, floored so it can't end a war) */
    warDamage: number;
}
...
const BY_ID: Record<string, MercenaryTier> = Object.fromEntries(MERCENARY_TIERS.map(t => [t.id, t]));

export function isMercenaryTierId(id: string): boolean {
    return Object.prototype.hasOwnProperty.call(BY_ID, id);
}

export function mercenaryById(id: string): MercenaryTier | null {
 ...
 */
export function applyMercenaryDamage(prevEnemyHp: number, warDamage: number): { nextHp: number; dealt: number } {
