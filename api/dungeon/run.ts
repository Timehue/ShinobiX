...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        if (!identity.admin && !(await enforceRateLimitKv(req, res, 'dungeon-run', 20, 60_000, identity.name))) return;
        const result = await mutatePlayerSave(playerName, ({ character }) => {
            ...
            return { ok: true as const, character: out.character, value: { token: out.token, alreadyApplied: out.alreadyApplied } };
        });
        if (!result.ok) return res.status(result.status).json({ error: result.error });
        return res.status(200).json({ ok: true, ...result.value, character: result.character, _saveVersion: result._saveVersion });
    } catch (error) { console.error('[dungeon/run]', error); return res.status(500).json({ error: 'Internal server error.' }); }
}
