...
describe('clan-boss player-combat parity guard', () => {
    ...
    it('reuses PvP combat resolution while documenting remaining tower-local drift', () => {
        ...
        assert.match(
            engine,
            /applyJutsu as applyPvpJutsu,\s*applyDoTs,\s*tickStatuses,\s*applyGroundEffectToFighter,\s*tickGroundEffects,\s*characterOwnsElement,\s*poisonSpendDamage\s*}\s*from '..\/pvp\/move\.js'/,
