...
import { canonicalTagName, CAPPED_AMP_TAGS, jutsuHasFixedEffectPower } from './pvp/_tags.js';

export type RawTag = { name?: unknown; percent?: unknown };
export type RawJutsu = {
    tags?: unknown;
    ap?: unknown;
    range?: unknown;
    target?: unknown;
...
export function pointBudgetForRank(rank: string | null | undefined): number {
...
export function tagPointValue(tag: RawTag, rank: string | null | undefined): number {
    if (typeof tag?.name !== 'string' || !tag.name) return 0;
    ...
    if (name === 'Copy' || name === 'Mirror') return 3;
    if (['Stun', 'Bloodline Seal', 'Lag', 'Overclock', 'Debuff Prevent', 'Buff Prevent'].includes(name)) return 2;
    if (['Reflect', 'Cleanse Prevent', 'Clear Prevent', 'Heal', 'Elemental Seal'].includes(name)) return 1.5;
    if (['Shield', 'Pierce', 'Wound', 'Barrier', 'Drain'].includes(name)) return 1;
    if (name === 'Push') return 1;
    if (name === 'Pull') return 0.75;
    if (['Move', 'Poison', 'Ignition'].includes(name)) return 0.5;
    return 1;
}

/** Rolled-up point cost of one jutsu at a given rank. Mirror of jutsuPoints. */
export function jutsuPoints(jutsu: RawJutsu, rank: string | null | undefined): number {
    const tags = Array.isArray(jutsu?.tags) ? (jutsu.tags as RawTag[]) : [];
    let sum = 0;
...
export function bloodlinePoints(jutsus: RawJutsu[], rank: string | null | undefined): number {
 ...
 */
export function enforceBloodlineBudget(jutsus: RawJutsu[], rank: string | null | undefined): RawJutsu[] {
    if (!Array.isArray(jutsus) || jutsus.length === 0) return jutsus;
    const budget = pointBudgetForRank(rank);
    // Shallow-clone each jutsu + its tag array so we can splice without mutating input.
    const out: RawJutsu[] = jutsus.map((j) =>
        j && typeof j === 'object'
            ? { ...(j as RawJutsu), ...(Array.isArray((j as RawJutsu).tags) ? { tags: [...((j as RawJutsu).tags as RawTag[])] } : {}) }
            : j,
    );
    if (bloodlinePoints(out, rank) <= budget) return out;
    ...
    while (bloodlinePoints(out, rank) > budget && guard++ < 500) {
        let best: { ji: number; ti: number; pts: number } | null = null;
        for (let ji = 0; ji < out.length; ji++) {
            const tags = Array.isArray(out[ji]?.tags) ? (out[ji]!.tags as RawTag[]) : null;
            if (!tags) continue;
        ...
        if (!best) break; // no removable tags remain
        (out[best.ji]!.tags as RawTag[]).splice(best.ti, 1);
    }
    return out;
}
