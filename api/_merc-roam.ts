...
export const MERC_TARGET_COOLDOWN_MS = 15 * 60 * 1000;

export interface RoamTarget {
    name: string;
    /** the player's home village — must equal the merc's enemy village to be a mark */
    village: string;
    hp: number;
    maxHp: number;
}
...
export function pickMercTarget(candidates: readonly RoamTarget[], enemyVillage: string): RoamTarget | null {
...
function mercTargetCdKey(targetPlayer: string): string {
    return `merc:target-cd:${safeName(targetPlayer).toLowerCase()}`;
}

/** True if a merc fought this player within the last MERC_TARGET_COOLDOWN_MS. */
export async function isMercTargetOnCooldown(targetPlayer: string, now: number): Promise<boolean> {
    const until = await kv.get<number>(mercTargetCdKey(targetPlayer));
...
export function mercNpcId(village: string, tierId: string, index: number): string {
