...
export function cashOutEndless(character: Record<string, unknown>, run: EndlessRun, day: string) {
    const todayRaw = character.lastDailyReset === day ? whole(character.dailyTowerXp) : 0;
    const raw = whole(run.bankedXp); const cap = 450 + Math.max(1, whole(character.level)) * 60;
    const under = Math.min(raw, Math.max(0, cap - todayRaw)); const creditedXp = under + Math.floor((raw - under) * 0.2);
    const leveled = gainXp(character, creditedXp) as Record<string, unknown>;
