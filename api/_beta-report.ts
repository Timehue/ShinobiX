import { betaLevelBand, type BetaMetricsSnapshot } from './_beta-metrics.js';

export interface BetaPopulationSnapshot {
    savesScanned: number;
    ...
    examHolds: Record<string, number>;
    walletRyoPercentiles: Record<string, number>;
    bankRyoPercentiles: Record<string, number>;
    hospitalizedPlayers: number;
    hospitalSoftLockRisk: number;
    academyTrialClaims: number;
    academyChecklistClaims: number;
    towerPlayers: number;
}

export interface DailyBetaReport {
    schemaVersion: 'shinobix.beta-daily-report.v1';
    generatedAt: number;
    metrics: BetaMetricsSnapshot;
    population?: BetaPopulationSnapshot;
    alerts: string[];
}

function inc(map: Record<string, number>, rawKey: unknown): void {
...
export function buildBetaPopulationSnapshot(records: readonly unknown[]): BetaPopulationSnapshot {
    const levelBands: Record<string, number> = {};
    const ranks: Record<string, number> = {};
    const professions: Record<string, number> = {};
    const villages: Record<string, number> = {};
    const examHolds: Record<string, number> = {};
    ...
    for (const record of records) {
        ...
        const char = (record as Record<string, unknown>).character;
        if (!char || typeof char !== 'object' || Array.isArray(char)) {
            malformedSaves += 1;
            continue;
        }
        const player = char as Record<string, unknown>;
        ...
        inc(levelBands, betaLevelBand(level));
        ...
        if (level === 20) inc(examHolds, 'level-20-genin-exam');
        if (level === 39) inc(examHolds, 'level-39-chunin-exam');
        if (player.hospitalized === true) {
            hospitalizedPlayers += 1;
            if (wallet < 10) hospitalSoftLockRisk += 1;
        }
        if (player.academyTrialClaimed === true) academyTrialClaims += 1;
        if (player.academyChecklistClaimed === true) academyChecklistClaims += 1;
        if (finiteNonNegative(player.battleTowerBestFloor) > 0) towerPlayers += 1;
    }
...
export function buildDailyBetaReport(
    metrics: BetaMetricsSnapshot,
    population?: BetaPopulationSnapshot,
): DailyBetaReport {
    const alerts: string[] = [];
    ...
    if (duplicateAttempts > 0) alerts.push(`${duplicateAttempts} duplicate reward attempt(s) rejected.`);
    if (failedClaims > 0) alerts.push(`${failedClaims} reward claim failure(s) recorded.`);
    if (unresolvedSessions > 0) alerts.push(`${unresolvedSessions} unresolved combat session(s) recorded.`);
    if (population?.malformedSaves) alerts.push(`${population.malformedSaves} malformed save record(s) require inspection.`);
    if (population?.hospitalSoftLockRisk) alerts.push(`${population.hospitalSoftLockRisk} hospitalized player(s) have under 10 wallet ryo.`);
    return {
        schemaVersion: 'shinobix.beta-daily-report.v1',
...
function mapLine(label: string, value: Record<string, number> | undefined): string {
