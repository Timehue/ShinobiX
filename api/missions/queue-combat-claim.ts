...
type QueueOutcome =
    ...
    | { queued: true; saveVersion: number };
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        const outcome = await withKvLock<QueueOutcome>(saveKey, async () => {
            ...
            await kv.set(tokenKey, '1', { ex: COMBAT_CLAIM_TOKEN_TTL_SECONDS }).catch(() => undefined);
