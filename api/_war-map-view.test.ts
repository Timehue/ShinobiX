...
import { normalizeVillageWarRecord, SECTOR_CONTROL_HP_MAX } from './_war-state.js';
...
describe('war-map-view: villageWarMapView', () => {
    it('assembles a fresh village view', () => {
        ...
        assert.equal(v.sectors[0].controlHpMax, SECTOR_CONTROL_HP_MAX);  // no Watchtower
    ...
    it('reflects Watchtower (Control HP cap) and Supply Depot (WR/sector)', () => {
        ...
        assert.equal(v.sectors[0].controlHpMax, Math.round(SECTOR_CONTROL_HP_MAX * 1.15));  // +15% at Watchtower L10
    ...
    it('suspends Control-HP/WR bonuses while dormant but still owes upkeep', () => {
        ...
        assert.equal(v.sectors[0].controlHpMax, SECTOR_CONTROL_HP_MAX);  // Watchtower suspended
