...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        const result = await mutatePlayerSave<{ replayed: boolean; trait: WildPetTrait | null }>(playerName, async ({ character }) => {
            const receipts = Array.isArray(character.redeemedPetEncounters) ? character.redeemedPetEncounters as string[] : [];
            if (receipts.includes(token)) return { ok: true as const, character, value: { replayed: true, trait: null } };
            ...
            return { ok: true as const, character: { ...granted.character, redeemedPetEncounters: [...receipts.slice(-49), token] }, value: { replayed: false, trait: granted.trait } };
        });
        if (!result.ok) return res.status(result.status).json({ error: result.error });
        await kv.del(key).catch(() => undefined);
        return res.status(200).json({ ok: true, ...result.value, character: result.character, _saveVersion: result._saveVersion });
    } catch (error) { console.error('[pet/befriend]', error); return res.status(500).json({ error: 'Internal server error.' }); }
}
