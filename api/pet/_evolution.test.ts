...
function fireStarter(over: Partial<PetLike> = {}): PetLike {
    return {
        ...
        defense: 24,
        speed: 32,
        growthBaseStats: { hp: 300, attack: 46, defense: 24, speed: 32 },
...
describe('evolvePet stat math', () => {
    it('applies the standard→rare delta and preserves identity + element', () => {
        ...
        assert.deepEqual(evolved.growthBaseStats, { hp: 350, attack: 54, defense: 30, speed: 38 });
        assert.equal(evolved.hp, 479);                     // evolved base + 49 levels of core growth
        assert.equal(evolved.attack, 74);
        assert.equal(evolved.defense, 41);
        assert.equal(evolved.speed, 52);
    ...
    it('applies the rare→legendary delta and bumps moveRange', () => {
        const rare = fireStarter({ rarity: 'rare', evolutionStage: 1, level: 90, hp: 800, attack: 120, defense: 90, speed: 100, growthBaseStats: { hp: 350, attack: 54, defense: 30, speed: 38 }, moveRange: 3 });
        ...
        assert.deepEqual(evolved.growthBaseStats, { hp: 396, attack: 60, defense: 34, speed: 43 });
        assert.equal(evolved.hp, 660);
        assert.equal(evolved.attack, 100);
    ...
    it('recalculates evolution stats from the authoritative base, not forged live stats', () => {
        ...
        assert.equal(evolved.hp, 505);
        assert.equal(evolved.attack, 78);
        assert.equal(evolved.defense, 43);
        assert.equal(evolved.speed, 55);
        const overDef = fireStarter({ level: 60, defense: 238 });
        const e2 = evolvePet(overDef, 1, evolutionLineFor('starter-fire')!);
        assert.equal(e2.defense, 43);
    });
});
