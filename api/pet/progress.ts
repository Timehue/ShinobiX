    ...
    try {
        ...
        const result = await mutatePlayerSave<{ action: string; pet: Record<string, unknown> | null }>(playerName, ({ character }) => {
            ...
            if (action === 'start-training') {
                ...
                if (pet.expedition || (pet.training && now < Number((pet.training as Record<string, unknown>).endsAt))) return { ok: false as const, status: 409, error: 'Pet is already busy.' };
                ...
                nextPet = gainServerPetXp({ ...pet, training: undefined }, Math.min(5000, Math.max(0, Number(training.sealedXp) || 0)), String(training.type ?? ''));
