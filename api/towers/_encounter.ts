...
function makeSpawnRng(seed: number): () => number {
...
function placeRandomInBand(
    rnd: () => number, used: Set<number>, blocked: Set<number>,
    w: number, h: number, colMin: number, colMax: number, tries = 80,
): number {
    ...
    for (let i = 0; i < tries; i++) {
        ...
        const tile = r * w + c;
        if (!used.has(tile) && !blocked.has(tile)) { used.add(tile); return tile; }
    }
...
export type BuildEncounterParams = {
    ...
    seed: number;
    partySize: number;
    /** Endless Spire — sealed ascension modifiers; when present the run is FULL-STRENGTH
     *  (no party downscaling) and the seal is written onto the session. Absent for story. */
