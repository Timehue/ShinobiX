    ...
    if (floor.dynamicHazards != null) {
        if (!Array.isArray(floor.dynamicHazards)) {
            ...
            floor.dynamicHazards.forEach((hz, i) => {
                ...
                if ((hz as { kind?: string })?.kind !== 'geyser') errs.push(`${w2}.kind invalid`);
                if (!Number.isInteger(hz.count) || hz.count < 1 || hz.count > 12) errs.push(`${w2}.count must be an integer in [1,12]`);
                if (typeof hz.pct !== 'number' || hz.pct <= 0 || hz.pct > 25) errs.push(`${w2}.pct out of (0,25]`);
                if (!Number.isInteger(hz.everyRounds) || hz.everyRounds < 2) errs.push(`${w2}.everyRounds must be an integer ≥2`);
                if (hz.firstRound != null && (!Number.isInteger(hz.firstRound) || hz.firstRound < 1)) errs.push(`${w2}.firstRound must be a positive integer`);
            });
        }
    }
