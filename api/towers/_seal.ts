 ...
 */
import { sanitizeJutsuList, sanitizePvpItems, stripNonCombatFields } from '../pvp/session.js';
...
export function sealTowerFighter(saveChar: Record<string, unknown>): Record<string, unknown> {
    const src = (saveChar.stats && typeof saveChar.stats === 'object') ? (saveChar.stats as Record<string, unknown>) : {};
    const stats: Record<string, number> = {};
    for (const f of STAT_FIELDS) stats[f] = clampNum(src[f], 0, MAX_STAT, 0);
    ...
    const sealed: Record<string, unknown> = {
        ...
        jutsu: sanitizeJutsuList(saveChar.jutsu),
        pvpItems: sanitizePvpItems(saveChar.pvpItems),
    ...
    return stripNonCombatFields(sealed);
}
